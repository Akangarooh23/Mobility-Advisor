-- Neon SQL editor: ejecuta cada bloque por separado (uno a uno).
-- No uses BEGIN/COMMIT en este editor para este caso.
...
-- 7) Seed brand self aliases from catalog brands
...
-- 8) Auto brand aliases from offers vs catalog by normalized equality
...
-- 9) Manual critical brand aliases
WITH resolved_ds AS (
  SELECT COALESCE(
...
INSERT INTO moveadvisor_brand_aliases (alias_name, canonical_name, source, is_active)
SELECT d.alias_name, d.canonical_name, 'manual-critical', TRUE
FROM (
  SELECT
    v.alias_name,
    v.canonical_name,
    ROW_NUMBER() OVER (
      PARTITION BY normalize_alias_token(v.alias_name)
      ORDER BY v.alias_name
    ) AS rn
  FROM (
...
-- 10) Seed model self aliases from catalog models
...
-- 11) Auto model aliases from offers vs catalog by normalized equality and resolved brand
WITH offers_raw AS (
...
-- 12) Manual critical model aliases
INSERT INTO moveadvisor_model_aliases (brand_canonical_name, canonical_name, alias_name, source, is_active)
SELECT d.brand_canonical_name, d.canonical_name, d.alias_name, 'manual-critical', TRUE
FROM (
  SELECT
    v.brand_canonical_name,
    v.canonical_name,
    v.alias_name,
    ROW_NUMBER() OVER (
      PARTITION BY normalize_alias_token(v.brand_canonical_name), normalize_alias_token(v.alias_name)
      ORDER BY v.alias_name
    ) AS rn
  FROM (
    VALUES
...
-- 13) Validation brand count
SELECT count(*) AS total_brand_aliases
FROM moveadvisor_brand_aliases
WHERE is_active = TRUE;

-- 14) Validation model count
SELECT count(*) AS total_model_aliases
FROM moveadvisor_model_aliases
WHERE is_active = TRUE;

-- 15) Validation DS case
SELECT alias_name, canonical_name, source
FROM moveadvisor_brand_aliases
WHERE normalize_alias_token(alias_name) IN (normalize_alias_token('DS'), normalize_alias_token('DS Automobiles'))
ORDER BY alias_name;
