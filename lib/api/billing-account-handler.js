...
async function getProfileFromPostgres(email) {
  ...
  try {
    const result = await pool.query(
      `SELECT name, apellidos, phone, company_name, tax_id, billing_address, iban
       FROM moveadvisor_users WHERE lower(email) = lower($1) LIMIT 1`,
      [email]
    );
    if (!result.rows.length) return null;
    const row = result.rows[0];
    const fullName = [normalizeText(row.name), normalizeText(row.apellidos)].filter(Boolean).join(" ");
    return {
      fullName,
    ...
    await pool.query(
      `UPDATE moveadvisor_users
       SET name = $2, apellidos = $3, phone = $4, company_name = $5, tax_id = $6,
           billing_address = $7, iban = $8, profile_updated_at = NOW()
      ...
      [
        ...
        normalizeText(profile?.billingAddress),
