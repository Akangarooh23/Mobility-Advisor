/**
 * POST /api/user-saved  – upsert a saved comparison
 * GET  /api/user-saved  – list saved comparisons
 * DELETE /api/user-saved?id=<id> – remove a saved comparison
 *
 * Requires active session. Falls back to localStorage on the client side.
 */
...
module.exports = async function userSavedHandler(req, res) {
  ...
  if (!shouldUseSqlServerMobility()) {
    // Backend no disponible – el cliente usa localStorage como fallback.
