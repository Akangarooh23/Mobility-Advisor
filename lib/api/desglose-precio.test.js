...
describe("el precio partido", () => {
  ...
  test("son tres, y no cinco", () => {
    ...
    assert.deepEqual(d.lineas.map((l) => l.concepto), [
      "Precio del coche",
      "Transporte desde Alemania hasta tu casa",
  ...
  test("el transporte es el mismo número que se usa para el coste", () => {
    const d = desgloseParaElCliente(6000, 10000);
    const transporte = d.lineas.find((l) => l.concepto === "Transporte desde Alemania hasta tu casa");
 ...
 *
 * «Transporte desde Alemania» decía de dónde y no decía hasta dónde. El viaje
 * cubre el trayecto entero, de la ciudad alemana a Zaragoza —donde se homologa
 * y se prepara— y de Zaragoza a su casa. Los dos tramos van en esta línea: el
 * cliente paga un viaje, no dos.
 *
 * Es la diferencia entre un precio completo y un precio que crece después.
 */
describe("hasta dónde llega el transporte incluido", () => {
  test("la línea dice dónde acaba el viaje, no solo dónde empieza", () => {
    const d = desgloseParaElCliente(6000, 10000);
    const linea = d.lineas.find((l) => /transporte/i.test(l.concepto));
    assert.match(linea.concepto, /hasta tu casa/i,
      "sin decir hasta dónde, no se sabe si el viaje acaba en la campa o en su puerta");
  });

  test("la entrega a domicilio no se cobra aparte: va dentro", () => {
    const d = desgloseParaElCliente(6000, 10000);
    assert.ok(!d.aparte.some((x) => /entrega/i.test(x)),
      "cobrarla aparte sería cobrar dos veces el mismo viaje");
  });

  test("y lo que sí se contrata aparte son dos cosas, no tres", () => {
    const d = desgloseParaElCliente(6000, 10000);
    assert.ok(d.aparte.some((x) => /seguro/i.test(x)));
    assert.ok(d.aparte.some((x) => /reacondicionad/i.test(x)));
    assert.equal(d.aparte.length, 2);
  });
});
