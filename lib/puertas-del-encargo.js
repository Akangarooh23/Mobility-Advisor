...
function aDondeSeHace(clave, matricula) {
  ...
  // La ficha y los papeles se editan en la pantalla del IDCar; el informe y las
  // franjas, en el panel. Son dos sitios porque son dos pantallas distintas, no
  // porque se haya elegido así.
  const enSuFicha = (ancla, texto) => ({ texto, url: `/mis-coches${suyo}#${ancla}` });
  const enSuPanel = (ancla, texto) => ({ texto, url: `/panel/vehiculos${suyo}#${ancla}` });
  /*
   * Las franjas tienen pantalla propia.
   *
   * Esto apuntaba a `/panel/vehiculos#franjas`, y allí las franjas vivían
   * dentro de un botón de la tarjeta del coche: el enlace bajaba hasta el
   * botón, sin abrir nada, y quien lo seguía se quedaba mirando su lista de
   * coches. El aviso decía «pendiente indicar franjas horarias» y llevaba a un
   * sitio donde no se indicaban.
   */
  const enSusVisitas = (texto) => ({ texto, url: `/panel/visitas${suyo}` });
  switch (clave) {
    case 'idcar':    return enSuFicha('datos', 'Completar la ficha y las fotos');
    case 'papeles':  return enSuFicha('documentos', 'Subir los documentos');
    case 'tasacion': return { texto: 'Hacer la tasación gratuita', url: `/panel/tasaciones${suyo}` };
    case 'informe':  return enSuPanel('informe', 'Hacer el informe de estado');
    case 'franjas':  return enSusVisitas('Elegir cuándo y dónde puedes enseñarlo');
    // Las dos viven en la ficha del IDCar, cada una en su sección.
    case 'seguro':        return enSuFicha('seguros', 'Subir el papel del seguro');
    case 'mantenimiento': return enSuFicha('mantenimientos', 'Subir una factura de revisión');
    default:         return null;
  }
}
...
function lasPuertas(hay, ahora = new Date()) {
  ...
  const puertas = [
     ...
     * sobre un coche que todavía no sabemos cuál es, y ese número es del que
     * se habla luego con el cliente.
     *
     * No deja de ser obligatoria: cambia de sitio en la cola.
     */
    {
      clave: 'tasacion',
      nombre: 'La tasación',
      ...
      falta: faltanPapeles.length > 0
        ? 'Primero sube la ficha técnica: de ahí sacamos la versión y las características'
        : 'No te la has hecho todavía. Es gratis y se hace desde aquí',
    },
    {
      clave: 'informe',
     ...
     *
     * Estaban como obligatorias y no lo son. Un coche se anuncia y se vende
     * sin el historial de revisiones —peor, pero se vende—, y el papel del
     * seguro no hace falta hasta el día del traspaso. Tenerlas cerrando la
     * puerta dejaba un encargo parado por una factura de hace tres años que a
     * lo mejor el cliente ni tiene, y le decía en su panel que no podíamos
     * publicar por eso, lo cual no era verdad.
     *
     * Siguen en la lista porque siguen mereciendo la pena: el historial es lo
     * primero que pregunta quien compra y de lo poco que sube el precio. Lo
     * que cambia es que no cuentan para publicar.
     */
    {
      clave: 'seguro',
      nombre: 'El seguro',
      ...
      falta: 'Sube la póliza o el recibo si lo tienes a mano. Hace falta el día del traspaso, no para publicar',
    },
    {
      clave: 'mantenimiento',
      ...
      falta: 'Sube alguna factura de las revisiones. No es obligatorio, pero es lo primero que pregunta quien compra',
    },
  ];
  ...
  return puertas.map((p) => ({
    ...
    //
    // Y la bloqueada lleva a lo que hay que hacer antes, no a ella misma: la
    // tasación sin la ficha técnica mandaba al tasador a poner un número sobre
    // un coche del que no sabemos la versión.
    donde: p.abierta
      ? null
      : aDondeSeHace(p.bloqueada ? 'papeles' : p.clave, hay.matricula),
  }));
}

/**
 * Cuántas le quedan **para publicar**. Es el número de la campana y el del resumen.
 *
 * Las opcionales no cuentan: decían «te quedan 4 cosas para que podamos
 * publicarlo» metiendo dentro una factura de revisión que no para nada.
 */
function cuantasLeFaltan(puertas) {
  return (puertas || []).filter((p) => !p.abierta && !p.opcional).length;
}

/** Las que no paran el anuncio. Gemela de `LAS_OPCIONALES` del ERP. */
const LAS_OPCIONALES = ['seguro', 'mantenimiento'];
...
async function loQueHayDe(pool, vehicleId) {
   ...
   * por la tabla de en medio.
   */
  const seguros = await pide(
  ...
  const mantenimientos = await pide(
    `SELECT COUNT(*)::int AS n
  ...
  return {
    ...
    seguros: (seguros.rows[0] && seguros.rows[0].n) || 0,
    mantenimientos: (mantenimientos.rows[0] && mantenimientos.rows[0].n) || 0,
  };
}
