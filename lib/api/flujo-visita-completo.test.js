...
// Relativas a hoy, como en `elegir-hora.test.js`: con fechas fijas la prueba
// dejaba de pasar el día en que esas horas quedaban atrás, porque una hora
// propuesta que ya ha pasado no vale y el servidor contesta 409.
const enDias = (d, h) => { const x = new Date(Date.now() + d * 86400000); x.setUTCHours(h, 0, 0, 0); return x.toISOString(); };

const HUECO_PEDIDO = { id: "s-1", starts_at: enDias(3, 8), ends_at: enDias(3, 9) };
const HUECO_OTRO = { id: "s-2", starts_at: enDias(8, 8), ends_at: enDias(8, 9) };
const PROPUESTAS = [enDias(5, 8), enDias(6, 14)];
