...
const { plantilla, parrafo, datos, aviso, boton, esc, textoPlano } = require("../correo");
...
const LUGAR = require("../lugar-de-la-visita");
...
async function sendEmail({ to, subject, html, attachments, movil }) {
  ...
  const body = { from: FROM_EMAIL, to, subject, html, text: textoPlano(html) };
...
async function laVisitaParaElVendedor(bookingId, token) {
   ...
   *
   * Si puso una dirección para ese coche, esa manda: es la calle y el número.
   * Lo otro es `vehicle_location`, que es lo que sale en el anuncio —«Madrid»—
   * y a quien ya tiene hora no le lleva a ninguna parte.
   */
  const suyo = LUGAR.comoLoVeSuDuenno(await elLugarDelCoche(b.offer_id));
  return {
    booking: {
    ...
    donde_sugerido: normalize(suyo?.enUnaLinea) || normalize(b.vehicle_location),
    contacto_sugerido: normalize(suyo?.contacto),
...
async function vendedorConfirma(bookingId, token, { donde, preguntarPor } = {}) {
  const pool = getPool();
  if (!pool) throw new Error("No DB");
  let lugar = normalize(donde).slice(0, 200);
  let quien = normalize(preguntarPor).slice(0, 120);
  ...
  try {
    ...
    /*
     * Si no escribió dónde, se usa el sitio que tiene puesto para ese coche.
     *
     * Escribirlo en cada visita era la única manera que había, y con tres
     * compradores lo escribía tres veces. Ahora lo pone una vez junto a las
     * franjas y esto lo recoge; lo que teclee aquí sigue mandando, porque un
     * día puede quedar en otro sitio.
     */
    if (!lugar) {
      const suyo = await elLugarDelCoche(antes.offer_id);
      lugar = normalize(LUGAR.comoSeLee(LUGAR.comoLoVeSuDuenno(suyo) || {})).slice(0, 200);
      if (!quien) quien = normalize(suyo?.contacto).slice(0, 120);
    }
...
/**
 * ¿Las franjas de esa oferta las puede tocar quien lo pide?
 *
 * Añadir, añadir en bloque y borrar no pedían **nada**: ni sesión ni
 * comprobación. Y el identificador de la oferta no es ningún secreto —va en la
 * dirección del anuncio—, así que cualquiera podía llenar de horas el calendario
 * del coche de otro, o dejarlo sin ninguna y que nadie pudiera pedir visita.
 * Sin dejar rastro de quién fue.
 *
 * Solo se gestionan franjas de un IDCar (`idcar-<coche>`), que es lo único que
 * hacen las dos pantallas que llaman aquí. Y el coche tiene que ser suyo: que el
 * identificador sea difícil de teclear es un obstáculo, no un permiso.
 */
async function esSuOferta(offerId, email) {
  const m = /^idcar-(.+)$/.exec(String(offerId || ""));
...
/**
 * El portero de las tres rutas que cambian el calendario.
 *
 * Contesta él mismo cuando dice que no, para que cada ruta se lea de un vistazo.
 * Devuelve true si se puede seguir.
 */
async function dejaTocarLasFranjas(req, body, offerId, res) {
  const { email } = await identidadDeLaPeticion(req, { cuerpo: body });
  if (!email) {
    res.status(401).json({ ok: false, error: "sin_sesion" });
    return false;
  }
  if (!(await esSuOferta(offerId, email))) {
    res.status(403).json({ ok: false, error: "no_es_tu_coche" });
    return false;
  }
  return true;
}
...
/**
 * El lugar donde enseña ese coche, si lo tiene puesto.
 *
 * Devuelve `null` cuando no hay ninguno, y también cuando la tabla todavía no
 * existe: esto se llama desde la confirmación de una visita, y una visita no se
 * puede quedar sin confirmar porque falte una dirección que es opcional.
 */
async function elLugarDelCoche(offerId) {
  const pool = getPool();
  if (!pool) return null;
  try {
    await pool.query(LUGAR.SQL_ASEGURA_TABLA);
    const r = await pool.query(LUGAR.SQL_LEE, [String(offerId || "")]);
    return r.rows[0] || null;
  } catch (e) {
    console.error("[visitas] lugar:", e.message);
    return null;
  }
}
...
module.exports = async function visitAvailabilityHandler(req, res) {
  ...
  /*
   * ── El lugar donde enseña el coche ──────────────────────────────────────
   *
   * Las dos rutas piden sesión y comprueban que el coche es suyo, igual que
   * las franjas: esta dirección es el portal de su casa y las horas a las que
   * queda con desconocidos. No sale por ninguna ruta pública.
   */
  if (method === "GET" && route === "lugar") {
    const offerId = normalize(req.query?.offerId);
    if (!offerId) return res.status(400).json({ ok: false, error: "offerId required" });
    const { email } = await identidadDeLaPeticion(req);
    if (!email) return res.status(401).json({ ok: false, error: "Hace falta iniciar sesión" });
    try {
      if (!(await esSuOferta(offerId, email))) return res.status(403).json({ ok: false, error: "no_es_tu_coche" });
      return res.status(200).json({ ok: true, lugar: LUGAR.comoLoVeSuDuenno(await elLugarDelCoche(offerId)) });
    } catch (e) {
      return res.status(500).json({ ok: false, error: e.message });
    }
  }

  if (method === "POST" && route === "guarda_lugar") {
    const { offerId } = body;
    if (!offerId) return res.status(400).json({ ok: false, error: "offerId required" });
    if (!(await dejaTocarLasFranjas(req, body, offerId, res))) return undefined;
    const falta = LUGAR.queLeFaltaAlLugar(body);
    if (falta) return res.status(400).json({ ok: false, error: falta });
    const pool = getPool();
    if (!pool) return res.status(500).json({ ok: false, error: "No DB" });
    try {
      await pool.query(LUGAR.SQL_ASEGURA_TABLA);
      const r = await pool.query(LUGAR.SQL_GUARDA, LUGAR.losDatosDeGuardar(offerId, body));
      return res.status(200).json({ ok: true, lugar: LUGAR.comoLoVeSuDuenno(r.rows[0]) });
    } catch (e) {
      return res.status(500).json({ ok: false, error: e.message });
    }
  }
