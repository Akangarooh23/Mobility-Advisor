...
const { plantilla, parrafo, datos, aviso, esc, textoPlano } = require("../correo");
...
async function sendInternalAlert(userEmail, request) {
  ...
  await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      ...
      // Ya es texto: sacarlo del HTML seria el camino al reves.
      text: JSON.stringify({ userEmail, ...request }, null, 2),
    }),
  }).catch(() => {});
}
