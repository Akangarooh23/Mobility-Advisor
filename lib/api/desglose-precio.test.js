...
describe("el precio partido", () => {
  ...
  test("el transporte es el mismo número que se usa para el coste", () => {
    const d = desgloseParaElCliente(6000, 10000);
    const transporte = d.lineas.find((l) => l.concepto === "Transporte hasta nuestras instalaciones");
 ...
 *
 * «Transporte desde Alemania» decía de dónde y no decía hasta dónde, y quien lo
 * leía entendía que el coche le llegaba a casa. No: lo incluido lo trae hasta
 * nuestras instalaciones de Madrid, que es donde tiene que estar para la ITV de
 * homologación y para matricularlo. Llevárselo desde ahí se contrata aparte.
 *
...
describe("hasta dónde llega el transporte incluido", () => {
  test("la línea dice dónde acaba el viaje, no solo dónde empieza", () => {
    ...
    assert.match(linea.concepto, /nuestras instalaciones/i,
      "sin decir hasta dónde, el cliente entiende que se lo llevan a casa");
  });

  test("y la entrega a domicilio está entre lo que se contrata aparte", () => {
    const d = desgloseParaElCliente(6000, 10000);
    assert.ok(d.aparte.some((x) => /entrega/i.test(x)),
      "si no sale aquí, el único sitio donde se dice es el teléfono");
  });
});
