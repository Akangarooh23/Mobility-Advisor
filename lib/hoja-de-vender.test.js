...
describe("el botón lleva a un formulario propio", () => {
  test("y no al formulario de contacto general", () => {
    // Los demás atributos se dejan abiertos: lo que aquí importa es que la
    // página monte su propio formulario y le pase quién es, no la lista exacta
    // de cosas que le da.
    assert.match(PAGINA, /<FormularioEncargoVenta[^>]*userEmail=\{userEmail\}[^>]*\/>/);
  ...
  test("y que crea un lead de verdad", () => {
     ...
     *
     * Se mira la ruta y no la forma de llamarla. La aserción pedía
     * `fetch("/api/leads"` literal y se quedó en rojo el día que las llamadas
     * pasaron por `rutaApi()`, que es la misma petición con la base delante —
     * lo que hay que proteger es que el formulario cree el lead, no con qué
     * ayudante se hace la llamada.
     */
...
describe("que coche es lo dice quien lo sabe", () => {
  /*
   * En el ERP salian «sus coches» y habia que adivinar cual. Con un coche se
   * adivina; con tres, no. Y quien lo tiene que adivinar es el que coge el
   * telefono, que es justo quien menos lo sabe.
   *
   * El cliente si lo sabe, asi que lo elige el.
   */
  test("si ha entrado y tiene coches, elige de una lista", () => {
    assert.match(FORM, /const tieneCoches = haySesion && Array\.isArray\(misCoches\)/);
    assert.match(FORM, /<select id="fev-coche-lista"/);
  });

  test("y cada control tiene su propio id, con la etiqueta apuntando al suyo", () => {
    /*
     * La lista y la matrícula compartían `id="fev-coche"`. Son excluyentes, así
     * que en el DOM no chocaban — pero el id cambiaba de dueño según el caso y
     * la etiqueta apuntaba a lo que hubiera. Dos controles distintos, dos ids.
     */
    assert.match(FORM, /id="fev-coche-matricula"/);
    assert.match(FORM, /htmlFor=\{tieneCoches \? "fev-coche-lista" : "fev-coche-matricula"\}/);
    assert.doesNotMatch(FORM, /id="fev-coche"/);
  });

  test("y el coche elegido viaja con el lead", () => {
    // Sin esto el ERP volveria a adivinar aunque el lo hubiera elegido.
    assert.match(FORM, /vehicle_id: datos\.vehicleId \|\| undefined/);
  });

  test("y si no, la matricula: ni texto libre ni registro", () => {
    /*
     * Era texto libre y valia «un Golf del 15», que no identifica ningun coche:
     * hay miles, y el que tiene que adivinar cual es es el que coge el
     * telefono. La matricula identifica el coche y ademas deja mirar si ya
     * tiene ficha, que es lo que se pretendia sacar de obligar a registrarse.
     */
    assert.match(FORM, /value=\{datos\.matricula\}/);
    assert.match(FORM, /placeholder="8888 LXR"/);
    assert.ok(
      !/registr[ao]/i.test(soloCodigo(FORM)),
      "se le esta pidiendo que se registre para poder pedirlo",
    );
  });

  test("y el que ha entrado sin coches ya no se come un muro", () => {
    /*
     * Habia un bloqueo -«primero da de alta tu coche»- que no dejaba ni
     * preguntar. Sin cuenta si se podia, o sea que registrarse te lo ponia mas
     * dificil.
     *
     * Y no conseguia lo que buscaba: la ficha la hace el con el coche delante,
     * y eso no lo hace nadie antes de que le digan por cuanto se esta vendiendo
     * su coche. La ficha sigue haciendo falta para publicar -esas puertas estan
     * en el ERP-, pero no para preguntar.
     */
    assert.doesNotMatch(FORM, /Primero da de alta tu coche/);
    assert.doesNotMatch(FORM, /sinCoches/);
  });

  test("pero se le dice donde se crea, y se le lleva con la matricula puesta", () => {
    // No es un muro, es una puerta abierta: el que quiera adelantarlo, puede.
    assert.match(FORM, /elAlta\(datos\.matricula\)/);
