...
/**
 * Llama a la tarea como la llama Vercel: con el secreto en la cabecera.
 *
 * Antes llamaba con el `user-agent`, que es lo que bastaba cuando la puerta se
 * abría sola al faltar el secreto. Ahora sin secreto no pasa nadie.
 */
async function corre({ cabeceras = { authorization: `Bearer ${process.env.CRON_SECRET}` } } = {}) {
...
describe("quién puede dispararla", () => {
  ...
  test("y decir que eres Vercel no vale", async () => {
    // El agente lo escribe quien llama: es una linea de texto, no una llave.
    filas = [{ fuente: "autocasion", ultimo: HACE_MUCHO, ofertas: 10 }];
    const conAgente = await corre({ cabeceras: { "user-agent": "vercel-cron/1.0" } });
    assert.equal(conAgente.codigo, 401);
    const conElSecreto = await corre({ cabeceras: { authorization: "Bearer s3cr3t0" } });
    assert.equal(conElSecreto.codigo, 200);
  });
});
