...
module.exports = async function conditionReportHandler(req, res) {
  ...
  try {
    ...
    const creada = await carswiseCheck.crearSesionDeCaptura({
      ...
      // A dónde vuelve el usuario al terminar. Sin esto acabaría en una pestaña
      // suelta de otro dominio, sin saber si su expediente ha quedado
      // registrado. Al otro lado se valida contra una lista de dominios.
      returnUrl: `${normalizeText(process.env.PUBLIC_SITE_URL) || "https://www.carswiseai.com"}/?volver=idcar`,
      snapshot: {
