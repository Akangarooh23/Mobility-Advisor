  ...
  try {
    ...
    if (safeTokens.length > 0) {
      ...
      const targetedQuery = unaFilaPorCoche(
        selectCols,
        `(${conditions.join(" OR ")}) AND COALESCE(is_active, TRUE) = TRUE`,
        "$1",
      );
      try {
    ...
    try {
      ...
      if (delConsejero.condiciones.length) {
        const sinCondiciones = await pool.query(
          unaFilaPorCoche(selectCols, "COALESCE(is_active, TRUE) = TRUE", "$1"),
          [boundedLimit],
        );
...
async function readInventoryUniverse(options = {}) {
  ...
  const hayCriterios = Object.values(criterios).some((v) => v !== null && v !== undefined && v !== '');
