...
/**
 * Los suplidos en la factura.
 *
 * De los 21.310 € que ingresa el cliente, solo 3.000 son la factura. El coche es
 * del concesionario alemán y el impuesto de Hacienda: dinero que pasa por
 * nuestra cuenta y **no es ingreso nuestro**, así que va fuera de la base de IVA
 * y aparte en la factura.
 *
 * Sin ese bloque, la factura diría 3.000 € al lado de una transferencia de
 * 21.310 y la diferencia no tendría explicación en ningún papel.
 */
describe("los suplidos", () => {
  const fs = require("fs");
  const path = require("path");
  const FUENTE = fs.readFileSync(
    path.join(__dirname, "invoice-pdf-handler.js"), "utf8"
  ).replace(/\r\n/g, "\n");

  test("se pintan cuando los hay", () => {
    assert.match(FUENTE, /SUPLIDOS/);
    assert.match(FUENTE, /Array\.isArray\(row\.suplidos\)/);
  });

  test("y se dice que no son ingreso nuestro", () => {
    // Un cliente que ve 21.310 € en una factura de servicios pensaría que
    // cobramos eso. Y Hacienda también.
    assert.match(FUENTE, /No son ingreso de PopCar/);
    assert.match(FUENTE, /fuera de la base imponible/);
  });

  test("no entran en la base de IVA", () => {
    // La base sale del importe de la factura y de nada más. Si los suplidos
    // entraran en esa línea, se estaría repercutiendo IVA sobre el coche de otro.
    const lineas = FUENTE.split("\n");
    const base = lineas.find((l) => l.includes("const baseAmount"));
    const iva = lineas.find((l) => l.includes("const ivaAmount"));
    // Del importe de la factura y del tipo que le toque a ese cliente. El tipo
    // ya no está clavado: una empresa de la UE con NIF-IVA comprobado va al 0 %.
    assert.match(base, /Number\(amount\) \/ \(1 \+ tipo \/ 100\)/);
    assert.ok(!/suplido/i.test(base), "los suplidos están entrando en la base");
    assert.ok(!/suplido/i.test(iva), "se estaría repercutiendo IVA sobre los suplidos");
  });

  test("y el total pagado cuadra con lo que transfirió", () => {
    assert.match(FUENTE, /Total pagado \(factura \+ suplidos\)/);
    assert.match(FUENTE, /Number\(amount\) \+ sumaSuplidos/);
  });

  test("sin suplidos, ese bloque no sale", () => {
    // Una factura de tasación de 10 € no tiene suplidos ni tiene por qué
    // explicar nada. Se mira la suma y no la longitud de la lista: una lista
    // con una línea a cero tampoco tiene nada que explicar.
    assert.match(FUENTE, /if \(sumaSuplidos\) \{/);
  });

  test("y los suplidos van debajo del cuadro del total, no a su lado", () => {
    // Iban los dos en la misma banda: el bloque empezaba a la altura del
    // recuadro amarillo, así que «21500.00 EUR» se imprimía encima de
    // «16890.00 EUR» y no se leía ninguno de los dos.
    const cuadro = /doc\.rect\(330, totY \+ 28, 215, 22\)/;
    assert.match(FUENTE, cuadro, "el cuadro del total se ha movido de sitio");
    assert.match(FUENTE, /let suplidosY = totY \+ 72;/,
      "los suplidos tienen que empezar por debajo del cuadro");
  });

  test("y el sello de pagada no se come la última línea", () => {
    // Con tres suplidos, el bloque baja: poner «PAGADA» en un sitio fijo lo
    // dejaba encima del total pagado.
    assert.match(FUENTE, /Math\.max\(totY \+ 78, suplidosY\)/);
  });
});
