 ...
 * PopCar Check la dirección firmada del objeto y se reenvía al navegador
...
/**
 * El coche que hay detrás del identificador de una oferta.
 *
 * Los dos prefijos con los que puede llegar, y el identificador pelado si no
 * lleva ninguno: quien pregunte por el coche directamente también acierta.
 */
function elCocheDeLaOferta(id) {
  const t = normalizeText(id);
  if (t.startsWith("user_")) return t.slice(5);
  if (t.startsWith("idcar-")) return t.slice(6);
  return t;
}
...
module.exports = async function vehicleModelPublicHandler(req, res) {
  ...
  /*
   * El mismo coche se llama de dos maneras según quién lo publicara.
   *
   * `user_<id>` cuando sale del garaje del dueño y `idcar-<id>` cuando lo
   * publicamos nosotros desde el ERP. Aquí solo se quitaba el primero, así que
   * en la ficha de un coche que vendemos nosotros —justo los que llevan
   * informe, porque se lo exigimos— la pregunta se hacía con
   * `idcar-veh-123`, no encontraba nada y el informe no se ofrecía.
   *
   * Es el peor sitio donde podía pasar: el informe de estado es lo que separa
   * nuestro anuncio de uno de Milanuncios, y el que llegaba de coches.net no
   * lo veía.
   */
  const bruto = normalizeText(req.query?.vehicleId);
  const vehicleId = elCocheDeLaOferta(bruto);
  ...
  try {
    ...
    const modelo = await popcarCheck.urlDelModelo3d(fila.capture_session_id, formato);
    if (!modelo.ok) {
    ...
    // Sin modelo propio, PopCar Check genera el esquema al vuelo y sí cabe por aquí.
    const esquema = await popcarCheck.descargarModelo3d(fila.capture_session_id, formato);
    if (!esquema.ok) {
      return res.status(502).json({ error: "No se ha podido cargar la vista en 3D." });
    }
...
/** Para poder probar la regla de los prefijos sin montar una base. */
module.exports.elCocheDeLaOferta = elCocheDeLaOferta;
