    ...
    for (const r of (Array.isArray(row.leads) ? row.leads : [])) {
      ...
      try {
        ...
        const tal = await pool.query(
          `SELECT taller, direccion, cita_at, cliente_pidio
             FROM erp_revisiones_taller
            WHERE vehicle_id = $1 AND estado <> 'Hecha' AND avisado_at IS NOT NULL
            ORDER BY created_at DESC LIMIT 1`,
          [cocheId]
        ).catch(() => ({ rows: [] }));
        const cita = tal.rows[0];
        if (cita && cita.cita_at) {
          tallerPorLead[s(r.id)] = {
            taller: s(cita.taller),
            direccion: s(cita.direccion),
            cita_at: new Date(cita.cita_at).toISOString(),
            /*
             ...
             */
            cliente_pidio: s(cita.cliente_pidio),
          };
        }
      } catch (e) {
