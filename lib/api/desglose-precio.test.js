...
const {
  ...
  FEE_POPCAR_CON_IVA, impuestoDeMatriculacion, sqlTipoDelImpuesto,
...
describe("el precio partido", () => {
  ...
  test("el fee va suelto y con su nombre", () => {
    ...
    assert.equal(fee.importe, FEE_POPCAR_CON_IVA);
...
describe("el mismo precio en SQL que en pantalla", () => {
  test("la consulta lleva el fee", () => {
    assert.ok(sqlPrecioPuestoAqui().includes(String(FEE_POPCAR_CON_IVA)));
  });
