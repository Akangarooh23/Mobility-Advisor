...
describe("lo que paga el cliente", () => {
  ...
  test("el coche, el fee con su IVA y el impuesto: nada más", () => {
    // 18.000 del coche + 3.630 de servicio + 4,75 % de 24.000
    assert.equal(precioPuestoAqui(18000, 24000), 18000 + 3630 + 0.0475 * 24000);
  });

  test("el fee que se le enseña lleva el IVA dentro", () => {
    // 3.000 son nuestros y 630 son de Hacienda. Enseñar 3.000 y cobrar 3.630
    // sería enseñar un precio que no existe.
    assert.equal(FEE_POPCAR, Math.round(FEE_POPCAR_BASE * 1.21));
  });
