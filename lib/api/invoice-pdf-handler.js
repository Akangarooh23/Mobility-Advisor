...
const { TITULO_RECTIFICATIVA, referenciaALaOriginal } = require("../series-de-factura.js");
const { tipoDeIvaDelServicio, POR_QUE_SIN_IVA } = require("../iva-del-servicio.js");
const { SSL_POSTGRES } = require("../postgres-ssl");

const getPool = (() => {
  let pool;
  return () => {
    if (!pool) pool = new Pool({ connectionString: process.env.DATABASE_URL || process.env.POSTGRES_URL, ssl: SSL_POSTGRES });
    return pool;
  };
})();
...
/**
 * La marca de que alguien comprobó el NIF-IVA del cliente en VIES.
 *
 * Nadie corre migraciones a mano aquí. Y hasta que exista la pantalla donde
 * marcarla, la columna vale para lo que tiene que valer: está a nulo, así que
 * a todo el mundo se le repercute el 21 %.
 */
const ENSURE_NIF_IVA = `
...
async function getInvoice(invoiceId, email) {
  const pool = getPool();
  await pool.query(ENSURE_NIF_IVA).catch(() => {});
  const result = await pool.query(
    ...
    `SELECT i.id, i.number, i.cw_invoice_number, i.date, i.amount, i.status, i.description,
            ...
            i.rectifica_numero, i.rectifica_fecha, i.rectifica_motivo,
            u.name, u.apellidos, u.phone, u.tax_id, u.nif_iva_verificado,
...
function buildPdf(PDFDocument, row) {
  return new Promise((resolve, reject) => {
    ...
    /*
     * El tipo lo decide quién recibe la factura, no el generador.
     *
     * Casi siempre el 21 %. Una empresa de otro país de la UE con NIF-IVA
     * comprobado va sin IVA, y el impuesto lo autoliquida ella: sin la
     * comprobación en VIES se le repercute igual, porque si la exención no se
     * sostiene el IVA lo debemos nosotros.
     */
    const tipo = tipoDeIvaDelServicio(row);
    const baseAmount = (Number(amount) / (1 + tipo / 100)).toFixed(2);
    ...
    /*
     * Y si rectifica a otra, se dice arriba y en grande.
     *
     * Las tres cosas que pide Hacienda y que confirmó el asesor: el texto
     * «Factura Rectificativa» visible, a qué factura corrige —número y fecha—
     * y por qué. Una rectificativa que parece una factura normal no se puede
     * casar con la que corrige, y quien la recibe se queda con las dos.
     */
    const rectifica = nt(row.rectifica_numero);
    doc.fontSize(rectifica ? 16 : 22).font("Helvetica-Bold").fillColor(COLOR.texto)
      .text(rectifica ? TITULO_RECTIFICATIVA : "FACTURA", 300, rectifica ? 54 : 50, { align: "right", width: 245 });
    ...
    let trasCabecera = 115;
    if (rectifica) {
      const referencia = referenciaALaOriginal({ numero: rectifica, fecha: row.rectifica_fecha });
      const motivo = nt(row.rectifica_motivo);
      doc.fontSize(9).font("Helvetica").fillColor(COLOR.textoSuave)
        .text(referencia, 50, 106, { width: 495 });
      trasCabecera = 122;
      if (motivo) {
        doc.text(`Motivo: ${motivo}`, 50, 118, { width: 495 });
        trasCabecera = 134;
      }
    }

    doc.moveTo(50, trasCabecera).lineTo(545, trasCabecera).strokeColor(COLOR.linea).lineWidth(1).stroke();
    ...
    doc.fontSize(9).font("Helvetica").fillColor(COLOR.textoSuave)
      ...
      .text(`IVA (${tipo}%):`, 380, totY + 14)
      .text(`${ivaAmount} EUR`, 480, totY + 14, { align: "right", width: 55 });

    // Una factura exenta sin decir por qué está incompleta.
    if (tipo === 0) {
      doc.fontSize(8).font("Helvetica").fillColor(COLOR.textoSuave)
        .text(POR_QUE_SIN_IVA, 50, totY + 32, { width: 320 });
    }

    /**
     * Los suplidos, si los hay, **debajo del cuadro del total**.
     *
     * Dinero que hemos pagado en nombre del cliente: el coche, que es del
     * concesionario alemán; el impuesto, que es de Hacienda; y la garantía, que
     * es de la aseguradora. Ninguno es ingreso nuestro, así que van fuera de la
     * base de IVA y aparte.
     *
     * Sin este bloque, la factura diría 3.000 € al lado de una transferencia de
     * 21.500, y la diferencia no tendría explicación en ningún papel.
     *
     * Iban los dos en la misma banda —el bloque empezaba a la altura del
     * recuadro amarillo— así que las cifras se imprimían unas encima de otras y
     * no se leía ninguna.
     */
    const suplidos = Array.isArray(row.suplidos) ? row.suplidos : [];
    const sumaSuplidos = suplidos.reduce((t, x) => t + (Number(x && x.importe) || 0), 0);

    doc.rect(330, totY + 28, 215, 22).fill(COLOR.amarillo);
    doc.fontSize(11).font("Helvetica-Bold").fillColor(COLOR.negro)
      .text(sumaSuplidos ? "TOTAL FACTURA:" : "TOTAL:", 340, totY + 34)
      .text(`${amount} EUR`, 440, totY + 34, { align: "right", width: 95 });

    let suplidosY = totY + 72;
    if (sumaSuplidos) {
      ...
      for (const su of suplidos) {
        ...
        doc.fontSize(9).font("Helvetica").fillColor(COLOR.texto)
          ...
          .text(`${importe.toFixed(2)} EUR`, 440, suplidosY, { align: "right", width: 105 });
        suplidosY += 15;
      }
      doc.moveTo(50, suplidosY + 2).lineTo(545, suplidosY + 2).strokeColor(COLOR.linea).stroke();
      suplidosY += 12;
      // Y lo que ha transferido de verdad: la factura son 3.000 y el ingreso
      // 21.500, y sin esta línea esa diferencia no cuadra en ningún papel.
      doc.fontSize(9).font("Helvetica-Bold").fillColor(COLOR.texto)
        .text("Total pagado (factura + suplidos):", 50, suplidosY)
        .text(`${(Number(amount) + sumaSuplidos).toFixed(2)} EUR`, 440, suplidosY, { align: "right", width: 105 });
      suplidosY += 22;
    }
    ...
    doc.fontSize(10).font("Helvetica-Bold").fillColor(paid ? COLOR.negro : COLOR.amarilloTexto)
      .text(paid ? "PAGADA" : status.toUpperCase(), 50, Math.max(totY + 78, suplidosY));
