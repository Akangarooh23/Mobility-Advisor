...
module.exports = async function vehiclePublishHandler(req, res) {
  ...
  const action    = normalizeText(body.action).toLowerCase() || "publish"; // publish | unpublish
  ...
  try {
    ...
    const imageUrl = photoRes.rows[0]?.file_url || "";

    // Get all photo URLs for image_urls column
    ...
    const imageUrls = JSON.stringify(allPhotosRes.rows.map((r) => r.file_url));

    const title = normalizeText(v.title) || [v.brand, v.model, v.year].filter(Boolean).join(" ");

    const existing = await pool.query(
      `SELECT id FROM moveadvisor_marketplace_vo_offers WHERE id = $1`,
      [offerId]
    );
    ...
    if (existing.rows.length) {
      await pool.query(
        `UPDATE moveadvisor_marketplace_vo_offers SET
           ...
           seller_type = 'particular', is_active = TRUE,
           updated_at = NOW()
         WHERE id = $12`,
      ...
      await pool.query(
        ...
        [offerId, title, v.brand||'', v.model||'', Number(v.year)||0, price,
         Number(v.mileage)||0, v.fuel||'', v.color||'', v.notes||'',
         imageUrl, imageUrls, sessionEmail,
         `${v.cv||''} CV`.trim()]
      );
    }
