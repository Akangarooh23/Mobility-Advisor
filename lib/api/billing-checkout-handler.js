...
const { resolvePlanById, getCheckoutPlansCatalog } = require("../billingCatalog");
...
module.exports = async function billingCheckoutHandler(req, res) {
  ...
  const planLabel = normalizeText(selectedPlan?.label) || "Plan MoveAdvisor";
  const priceId = normalizeText(selectedPlan?.stripePriceId);
