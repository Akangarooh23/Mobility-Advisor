...
describe("y el catálogo no vuelve a publicar un coche muerto", () => {
  ...
  // Por encima de los dos listones: 12.000 € de precio mínimo y 8.000 € de
  // ahorro mínimo. Aquí había un 16.000 -> 26.000 que ahorra 5.610 € y dejó de
  // pasar el 21-sep, cuando el suelo en euros entró; no es que la prueba
  // fallara, es que le cambiaron el listón debajo.
  const bueno = { precioAleman: 30000, precioEspanol: 48000, comparables: 30 };
...
/**
 * Y tampoco un coche que ya es de otro cliente.
 *
 * Un anuncio de AutoScout24 sigue vivo y sigue saliendo a cuenta después de que
 * se lo compremos a alguien: el alemán tarda en quitarlo, y el precio no cambia.
 * Así que las dos banderas de arriba no lo apartan, y otro cliente podía
 * encargar exactamente el mismo coche y pagar su fianza.
 *
 * Pasó con el Kia Sorento: entregado el 1 de septiembre y en el escaparate una
 * semana después.
 */
const RESERVADA = /COALESCE\(import_locked, FALSE\) = FALSE/;

describe("ni un coche que ya es de otro cliente", () => {
  const OFERTAS = lee("import-offers-handler.js");
  const ALTA = lee("import-lead-handler.js");

  test("el listado, la ficha y el enlace lo filtran", () => {
    const cuantas = (OFERTAS.match(new RegExp(RESERVADA.source, "g")) || []).length;
    assert.ok(cuantas >= 4, `solo ${cuantas} consultas apartan los reservados; hacen falta el listado, las dos fichas y las marcas`);
  });

  test("y el desplegable de marcas no los cuenta", () => {
    const marcas = OFERTAS.slice(OFERTAS.indexOf("GROUP BY brand") - 500, OFERTAS.indexOf("GROUP BY brand"));
    assert.match(marcas, RESERVADA);
  });

  test("encargar uno reservado no llega ni a empezar", () => {
    // Es la puerta que importa: sin ella, el segundo cliente paga la fianza de
    // un coche que ya se le compró al primero.
    assert.match(ALTA, RESERVADA);
  });

  test("con COALESCE: sin marcar no es lo mismo que reservado", () => {
    // Los 800.000 anuncios rastreados tienen la columna a nulo. Sin el COALESCE
    // no saldría ninguno.
    assert.ok(!/import_locked = FALSE(?!\))/.test(OFERTAS), "compara sin COALESCE y deja fuera todo lo no marcado");
  });
});

describe("ni un coche dañado, ni uno sin mirar", () => {
  const { sePublica } = require("../coste-importacion.js");
  const bueno = { precioAleman: 30000, precioEspanol: 48000, comparables: 30 };

  test("dañado, no se publica", () => {
    assert.equal(sePublica({ ...bueno, danado: true }), false);
  });

  test("sin comprobar los daños, tampoco", () => {
    // El comprobador se bloquea a veces y deja coches sin mirar. Mientras no
    // se sepa, no se ofrece: «los dañados no los publiques» incluye a los que
    // podrían serlo.
    assert.equal(sePublica({ ...bueno, danosComprobados: false }), false);
  });

  test("mirado y limpio, se publica", () => {
    assert.equal(sePublica({ ...bueno, danado: false, danosComprobados: true }), true);
  });

  test("el recálculo sabe distinguir «mirado y limpio» de «sin mirar»", () => {
    /*
     * La prueba de que un coche se ha mirado es `is_damaged IS NOT NULL`, no la
     * fecha. Cuando AutoScout24 nos bloquea, el comprobador estampa
     * damage_checked_at y deja is_damaged en NULL para volver a intentarlo: la
     * fecha solo dice «se intentó». Tomándola por veredicto, esto publicaría
     * los que nadie ha podido mirar.
     *
     * Y se miran las LÍNEAS DE SQL, no el fichero entero: la consulta explica
     * todo esto en sus comentarios, así que buscar en el texto completo da por
     * bueno el comentario en vez del código. Esta misma prueba pasó en verde
     * un día entero por eso.
     */
    const fs = require("fs");
    const path = require("path");
    const SQL = fs.readFileSync(
      path.join(__dirname, "..", "..", "scripts", "recalcula-publicacion.cjs"), "utf8");
    const consulta = SQL.slice(SQL.indexOf("SELECT id, price"), SQL.indexOf("WHERE country = 'DE'"))
      .split("\n").filter((l) => l.trim().indexOf("--") !== 0).join("\n");
    assert.match(consulta, /is_damaged IS NOT NULL/,
      "la consulta no comprueba que los daños se hayan MIRADO de verdad");
    assert.match(SQL, /danosComprobados:/, "no se le pasa el dato a sePublica");
  });
});
