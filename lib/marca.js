...
function respuestaA() {
  const nt = (v) => String(v || "").trim();
  return nt(process.env.REPLY_TO_EMAIL) || MARCA.correoSoporte;
}
...
function correoInterno() {
  const nt = (v) => String(v || "").trim();
  return nt(process.env.INTERNAL_EMAIL) || MARCA.correoSoporte;
}

module.exports = { MARCA, COLOR, remitente, respuestaA, correoInterno };
