...
describe("una visita recién pedida", () => {
  ...
  test("con la hora y su enlace, pero sin el teléfono ni el correo del comprador", async () => {
    await sendBookingEmails(visita());
    const html = enviados.find((e) => e.to === "vendedor@example.com").html;
    assert.match(html, />10:00</);
    assert.doesNotMatch(html, /de 10:00 a/, "una visita es a una hora, no un rango");
  ...
  test("al comprador, la hora concreta: no un rango", async () => {
    await sendBookingEmails(visita());
    const html = enviados.find((e) => e.to === "comprador@example.com").html;
    assert.match(html, />10:00</);
    assert.doesNotMatch(html, /de 10:00 a/);
  });
});
