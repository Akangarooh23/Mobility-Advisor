/**
 * Buscador de ofertas — Comprar › Buscar coche.
 *
 * Sirve el listado completo de `moveadvisor_market_offers` y lo va recortando
 * segun los filtros. Dos cosas quedan siempre fuera y no son negociables desde
 * el cliente: las ofertas inactivas y las de importacion alemana. Van en la
 * base de la consulta, no como filtro, para que no puedan colarse pasando un
 * parametro.
 *
 * Tiene dos modos:
 *   ?facets=brands            → las marcas con cuantas ofertas tiene cada una
 *   ?facets=models&brand=...  → los modelos de esa marca, igual
 *   (sin facets)              → la pagina de ofertas y el total que cumple
 *
 * Los desplegables se construyen con lo que hay en las ofertas, no con el
 * catalogo maestro: asi nunca se ofrece una marca que no devuelve nada.
 */

const { getPostgresPool } = require("../inventoryStore");

/*
 * Lo que este buscador enseña SIEMPRE, pase el cliente los filtros que pase.
 * Nunca se toca desde fuera.
 *
 * Hasta el 24-sep-2026 esto era solo `is_active AND country = 'ES'`, y se
 * notaba: en una búsqueda de Volkswagen T-Roc salían tarjetas con el hueco de
 * la foto, coches «a 301 €» y el mismo coche repetido seis veces.
 *
 * ── Sin foto no se enseña ──────────────────────────────────────────────────
 *
 * Una tarjeta sin foto es un hueco gris. Son 360.848 ofertas del pool, el 23 %,
 * y no están repartidas: wallapop tiene el 64 % sin foto y clicars el 72 %,
 * mientras coches.net y AutoScout24 andan por el 1-2 %.
 *
 * Esto NO se aplica a la tasación: un comparable sin foto sigue siendo un
 * precio válido. Solo a lo que se le enseña a una persona.
 *
 * ── Por debajo de 4.500 € no son coches, son cuotas ────────────────────────
 *
 * Ordenando por precio ascendente salían T-Roc de 2023 «a 301 €». No es un
 * chollo: es la cuota mensual publicada como precio. Wallapop tiene 6.500
 * ofertas por debajo de 1.000 € y 71.699 más entre 1.000 y 4.499.
 *
 * El suelo lo puso Ana en 4.500. Por debajo hay coches de verdad, pero no los
 * suficientes como para que compense enseñar el ruido.
 *
 * ── Un coche, una tarjeta ──────────────────────────────────────────────────
 *
 * Las cadenas publican su stock central bajo la cuenta de cada sucursal: el
 * mismo T-Roc de 15.890 € llega a estar diecisiete veces, en Alicante,
 * Córdoba, Granada, Madrid... Se enseña solo el anuncio canónico, que decide
 * scripts/agrupar-duplicados.js y guarda moveadvisor_offer_duplicates.
 *
 * Se filtra por AUSENCIA -no existe una fila que diga que este anuncio es copia
 * de otro- en vez de por presencia en la tabla: así un anuncio que el agrupador
 * todavía no ha visto se sigue enseñando. El agrupador corre cada mañana a las
 * 07:30, y con la regla al revés todo lo raspado después sería invisible hasta
 * el día siguiente.
 */
const BASE = `is_active AND country = 'ES'
  AND COALESCE(image_url, '') <> ''
  AND price >= 4500
  AND NOT EXISTS (
    SELECT 1 FROM moveadvisor_offer_duplicates d
     WHERE d.offer_id = moveadvisor_market_offers.id
       AND d.canonical_id <> d.offer_id)`;

const MAX_LIMITE = 48;

/**
 * Hasta donde se cuenta el total.
 *
 * Contar las 1.579.000 ofertas que cumplen «ninguno» tardaba 15 segundos, y
 * la pagina no salia hasta tenerlo. Nadie necesita saber si hay 1.579.109 o
 * 1.579.112: lo que dice algo es «mas de 10.000», y en cuanto los filtros
 * aprietan por debajo del tope la cifra vuelve a ser exacta.
 *
 * El LIMIT es lo que lo hace barato: Postgres para de contar al llegar.
 */
const TOPE_DEL_TOTAL = 10000;
const LIMITE_POR_DEFECTO = 24;

const ORDENES = {
  recientes: `last_seen_at DESC NULLS LAST`,
  precio_asc: `price ASC NULLS LAST`,
  precio_desc: `price DESC NULLS LAST`,
  km_asc: `mileage ASC NULLS LAST`,
  anio_desc: `"year" DESC NULLS LAST`,
};

function miles(n) {
  const s = String(Math.round(Number(n)));
  let out = "";
  for (let i = 0; i < s.length; i += 1) {
    if (i > 0 && (s.length - i) % 3 === 0) out += ".";
    out += s[i];
  }
  return out;
}

/* ── Como se escribe el nombre de una marca ────────────────────────────────
 *
 * Los portales mandan la marca a voces —«ASTON MARTIN», «LAMBORGHINI»— o en
 * minusculas —«smart», «touran»—, y en el desplegable eso se lee como un
 * grito o como un descuido. Se escribe como un nombre propio: cada palabra con
 * la primera letra en mayuscula y el resto en minuscula.
 *
 * Con una excepcion: las siglas. BMW, BYD, MG y DS no son palabras, y «Bmw» o
 * «Ds» no es como se escriben. Se reconocen por la forma —una sola palabra, en
 * mayusculas, de cuatro letras o menos— mas una lista corta para las que la
 * forma no atrapa: OMODA tiene cinco letras, y GiottiLine y SsangYong llevan
 * una mayuscula en medio a proposito.
 *
 * Esto cambia lo que se lee, nunca lo que se busca: el filtro compara siempre
 * en minusculas, asi que «Aston Martin» encuentra las ofertas guardadas como
 * «ASTON MARTIN». Las tildes no se tocan aqui.
 */
const NOMBRES_PROPIOS = new Map([
  // Siglas que la forma no atrapa: o son largas, o llegan ya en minusculas.
  ["omoda", "OMODA"],
  ["dfsk", "DFSK"],
  ["evo", "EVO"],
  ["swm", "SWM"],
  ["levc", "LEVC"],
  ["man", "MAN"],
  ["vw", "VW"],
  ["ac propulsion", "AC Propulsion"],
  ["dr automobiles", "DR Automobiles"],
  // Palabras de cuatro letras que la regla confundiria con siglas.
  ["ebro", "Ebro"],
  // Mayuscula en medio a proposito.
  ["giottiline", "GiottiLine"],
  ["ssangyong", "SsangYong"],
  ["kgm / ssangyong", "KGM / SsangYong"],
  ["mclouis", "McLouis"],
]);

function nombrePropio(nombre) {
  const n = String(nombre ?? "").trim();
  if (!n) return "";
  const escrito = NOMBRES_PROPIOS.get(n.toLowerCase());
  if (escrito) return escrito;

  // Una sola palabra, en mayusculas y corta: es una sigla.
  const unaPalabra = !/[\s/]/.test(n);
  if (unaPalabra && n.length <= 4 && n === n.toUpperCase() && /[A-Z]/.test(n)) return n;

  return n.replace(/[A-Za-zÀ-ÿ]+/g, (p) => p[0].toUpperCase() + p.slice(1).toLowerCase());
}

function alfabetico(lista) {
  return [...lista].sort((a, b) =>
    String(a.nombre).localeCompare(String(b.nombre), "es", { sensitivity: "base" }));
}

function texto(v) {
  return typeof v === "string" ? v.trim() : "";
}
function entero(v) {
  const n = Number.parseInt(String(v ?? "").replace(/[^0-9-]/g, ""), 10);
  return Number.isFinite(n) ? n : null;
}

/* ── Marcas escritas de varias formas ──────────────────────────────────────
 *
 * El volcado de portales trae la misma marca escrita de muchas maneras, y son
 * dos problemas distintos que se arreglan en dos sitios distintos.
 *
 * Uno son las erratas y los nombres comerciales: «Yoodoo» por Yudo, «bwm» por
 * BMW, «DS Automobiles» por DS. Ahi no hay regla que valga, hace falta que
 * alguien diga cual es la buena, y quien lo dice es `moveadvisor_brand_aliases`
 * —la misma tabla que ya consulta el tasador—. Aqui no se inventa criterio: se
 * lee el suyo.
 *
 * El otro son tildes, guiones, espacios y mayusculas: «Citroën» y «Citroen»,
 * «Mercedes-Benz» y «Mercedes Benz», «Land Rover» y «LAND-ROVER». Eso si es una
 * regla, y declararlo alias a alias seria una lista infinita que hay que ir
 * ampliando cada vez que un portal escribe distinto. Se agrupa por la clave que
 * calcula `normalize_alias_token`, la misma funcion SQL con la que la tabla de
 * alias genera las suyas, para que las dos formas de agrupar no se separen.
 *
 * El nombre que se ensena es el de la grafia con mas ofertas, salvo que un
 * alias diga otra cosa: entre 29.865 «Citroen» y 606 «Citroën» manda la mayoria.
 *
 * Nada de esto toca las ofertas. La columna sigue guardando lo que mando el
 * portal, y el filtro busca todas las grafias del grupo a la vez.
 */
/**
 * Las marcas que abren el desplegable, antes que las demas.
 *
 * Es una decision de producto, no algo que se deduzca de los datos: no es «las
 * que mas ofertas tienen» —Abarth tiene 733 y Ebro 543, y estan; Jaecoo y
 * Leapmotor acaban de llegar al mercado— sino las que se espera encontrar
 * arriba al abrir la lista. Por eso vive aqui, a la vista en un diff, y no en
 * una fila de la base que nadie repasa.
 *
 * Se cruzan por clave normalizada, no por texto: aqui pone «Citroën» y en las
 * ofertas la mayoria escribe «Citroen»; aqui «Škoda» y alli «Skoda». La clave
 * es la misma y por eso casan.
 *
 * SsangYong no esta en la lista aunque sea una marca principal: sus ofertas se
 * unieron a «KGM / SSANGYONG», que si esta, y ponerla dos veces la sacaria
 * repetida.
 */
const MARCAS_PRINCIPALES = [
  "Abarth", "Aiways", "Alfa Romeo", "Alpine", "Aston Martin", "Audi", "Bentley",
  "BMW", "BYD", "Chevrolet", "Chrysler", "Citroën", "Cupra", "Dacia", "Daewoo",
  "DFSK", "Dodge", "Dongfeng", "DR Automobiles", "DS", "Ebro", "EVO", "Ferrari",
  "Fiat", "Ford", "Honda", "Hyundai", "Infiniti", "Iveco", "Jaecoo", "Jaguar",
  "Jeep", "KGM / SSANGYONG", "Lamborghini", "Lancia", "Land Rover", "Leapmotor",
  "Lexus", "Lynk & Co", "Maserati", "Mercedes-Benz", "MG", "MINI", "Mitsubishi",
  "Nissan", "OMODA", "Opel", "Peugeot", "Polestar", "Porsche", "Renault", "Saab",
  "SEAT", "Škoda", "Smart", "Subaru", "Suzuki", "Tata", "Tesla", "Toyota",
  "Volkswagen", "Volvo",
];

const CACHE_TTL_MS = 5 * 60 * 1000;
let aliasCache = null;
let aliasCaduca = 0;
let grafiasCache = null;
let grafiasCaduca = 0;
let principalesCache = null;
let principalesCaduca = 0;

async function aliasDeMarcas(pool) {
  const ahora = Date.now();
  if (aliasCache && ahora < aliasCaduca) return aliasCache;

  const r = await pool.query(
    `SELECT alias_name, canonical_name
       FROM moveadvisor_brand_aliases
      WHERE is_active AND alias_key <> canonical_key`
  );

  const mapa = new Map();
  for (const fila of r.rows) {
    const de = texto(fila.alias_name).toLowerCase();
    const a = texto(fila.canonical_name);
    if (de && a) mapa.set(de, a);
  }

  aliasCache = mapa;
  aliasCaduca = ahora + CACHE_TTL_MS;
  return mapa;
}

/**
 * Las grafias que hay en las ofertas, agrupadas por clave normalizada, con el
 * nombre de la que mas ofertas tiene.
 *
 * La cuenta va sobre las ofertas que se enseñan —activas y de España— y no
 * sobre lo que quede tras los filtros del usuario. Las dos cosas importan:
 *
 *   - Si dependiera de los filtros, «Mercedes-Benz» podria pasar a llamarse
 *     «Mercedes Benz» por acotar los kilometros. El nombre de una marca no
 *     cambia porque muevas un filtro.
 *   - Y si contara la tabla entera, mandarian ofertas que nadie ve: sobre el
 *     total gana «Byd» 524 a 166, pero entre las que se enseñan gana «BYD» 155
 *     a 91. El nombre lo deciden las ofertas que existen para quien mira.
 *
 * Las grafias del grupo si salen de la tabla entera: el filtro tiene que
 * encontrarlas todas, y una grafia de mas no hace dano —no habra ofertas que
 * la lleven— mientras que una de menos deja ofertas fuera.
 *
 * La funcion normalizadora se ejecuta una vez por grafia distinta —unas 470—,
 * no una vez por oferta: agrupar primero por `brand` deja el trabajo caro
 * dentro del indice.
 */
async function mapaGrafias(pool) {
  const ahora = Date.now();
  if (grafiasCache && ahora < grafiasCaduca) return grafiasCache;

  /*
   * De `mmo_facetas`, no del pool.
   *
   * Esto recorria los 2,36 millones de filas en cada peticion de listado
   * -35 segundos- y la pagina no salia hasta tenerlo. La vista trae las dos
   * cuentas ya hechas: `marca` es lo visible, que decide el nombre, y
   * `marca_todas` son todas las grafias, que es lo que el filtro necesita.
   */
  const r = await pool.query(
    `SELECT normalize_alias_token(valor) AS clave,
            (array_agg(valor ORDER BY visibles DESC, cnt DESC))[1] AS nombre,
            array_agg(valor) AS grafias
       FROM (SELECT valor,
                    SUM(n) FILTER (WHERE tipo = 'marca_todas')::int AS cnt,
                    SUM(n) FILTER (WHERE tipo = 'marca')::int AS visibles
               FROM mmo_facetas
              WHERE tipo IN ('marca', 'marca_todas')
              GROUP BY valor) x
      GROUP BY 1`
  );

  const porClave = new Map();
  const porGrafia = new Map();
  for (const fila of r.rows) {
    const clave = texto(fila.clave);
    if (!clave) continue;
    const grafias = (fila.grafias || []).filter(Boolean);
    porClave.set(clave, { nombre: texto(fila.nombre), grafias });
    for (const g of grafias) porGrafia.set(String(g).toLowerCase(), clave);
  }

  grafiasCache = { porClave, porGrafia };
  grafiasCaduca = ahora + CACHE_TTL_MS;
  return grafiasCache;
}

/**
 * La clave normalizada de cada marca principal, calculada por la base para que
 * sea exactamente la misma con la que se agrupan las ofertas. Reimplementarla
 * en JavaScript seria la forma segura de que un dia dejaran de casar.
 */
async function clavesPrincipales(pool) {
  const ahora = Date.now();
  if (principalesCache && ahora < principalesCaduca) return principalesCache;

  const r = await pool.query(
    `SELECT v AS nombre, normalize_alias_token(v) AS clave FROM unnest($1::text[]) v`,
    [MARCAS_PRINCIPALES]
  );

  // Se conserva el orden de la lista, no el que devuelva la base.
  const porNombre = new Map(r.rows.map((x) => [x.nombre, texto(x.clave)]));
  principalesCache = MARCAS_PRINCIPALES
    .map((nombre) => ({ nombre, clave: porNombre.get(nombre) }))
    .filter((x) => x.clave);
  principalesCaduca = ahora + CACHE_TTL_MS;
  return principalesCache;
}

/**
 * El nombre con el que se ensena una marca. Manda el alias si lo hay; si no, la
 * grafia mayoritaria de su grupo; y si la marca no aparece en ninguna oferta,
 * lo que venga.
 */
function canonica(nombre, alias, grafias) {
  const n = String(nombre ?? "");
  const declarado = alias.get(n.toLowerCase());
  if (declarado) return declarado;
  if (!grafias) return n;
  const clave = grafias.porGrafia.get(n.toLowerCase());
  const grupo = clave ? grafias.porClave.get(clave) : null;
  return (grupo && grupo.nombre) || n;
}

/**
 * Todas las grafias que hay que buscar para la marca elegida, en minusculas.
 *
 * Se parte de la canonica —si llega un alias, un enlace antiguo por ejemplo, se
 * resuelve primero—, se anaden las grafias que un alias manda a esa marca, y de
 * cada una se anade su grupo normalizado entero. Asi elegir «BMW» encuentra
 * tambien las guardadas como «bwm», y elegir «Citroen» las guardadas con
 * dieresis.
 */
async function grupoDeMarca(pool, marca) {
  const alias = await aliasDeMarcas(pool);
  const grafias = await mapaGrafias(pool);

  const canon = canonica(marca, alias, grafias);
  const grupo = new Set([canon.toLowerCase(), String(marca ?? "").toLowerCase()]);

  for (const [de, a] of alias) {
    if (a.toLowerCase() === canon.toLowerCase()) grupo.add(de);
  }
  for (const g of [...grupo]) {
    const clave = grafias.porGrafia.get(g);
    const grupoNorm = clave ? grafias.porClave.get(clave) : null;
    if (grupoNorm) for (const otra of grupoNorm.grafias) grupo.add(String(otra).toLowerCase());
  }

  return [...grupo].filter(Boolean);
}

/**
 * Suma en una sola entrada las filas que son la misma marca.
 *
 * La consulta ya llega agrupada por clave normalizada, asi que aqui solo queda
 * juntar lo que ademas una la tabla de alias, que si cruza claves distintas:
 * «bwm» y «bmw» no se parecen para la funcion normalizadora, y son la misma
 * marca porque alguien lo ha dicho.
 */
function agrupar(filas, alias, grafias) {
  const por = new Map();
  for (const f of filas) {
    const nombre = String(f.nombre ?? "");
    if (!nombre.trim()) continue;
    const canon = canonica(nombre, alias, grafias);
    const clave = canon.toLowerCase();
    const previo = por.get(clave);
    if (previo) previo.n += Number(f.n) || 0;
    else por.set(clave, { nombre: canon, n: Number(f.n) || 0 });
  }
  return [...por.values()];
}

/**
 * Los modelos se parten igual que las marcas, y peor: «Leon» y «León» son 7.812
 * ofertas de Seat, y el C-HR de Toyota llega de cinco maneras. Se resuelve en el
 * momento y no en un mapa guardado como el de marcas: hay decenas de miles de
 * modelos distintos, y siempre se pregunta por uno dentro de una marca ya
 * elegida, asi que la consulta sale barata.
 */
async function grupoDeModelo(pool, grupoMarca, modelo) {
  const m = texto(modelo);
  if (!m) return null;

  const marcas = (grupoMarca && grupoMarca.length ? grupoMarca : null);
  /*
   * De `mmo_modelos`, no del pool.
   *
   * Antes esto recorria el millon y medio de ofertas para juntar las grafias
   * de un modelo. Y ademas solo juntaba las que se escriben igual salvo
   * tildes y mayusculas, asi que elegir «A3» dejaba fuera las 673 grafias
   * con la version pegada —«A3 Sportback 30 TFSI 116CV»— que los volcados de
   * wallapop y milanuncios metieron en el campo del modelo.
   *
   * La vista ya las tiene pegadas a su modelo del catalogo, asi que aqui se
   * pregunta por el modelo limpio y salen todas sus grafias.
   */
  const r = await pool.query(
    `SELECT array_agg(grafia) AS grafias
       FROM mmo_modelos
      WHERE lower(modelo) = lower($1)
        ${marcas ? "AND marca = ANY($2)" : ""}`,
    marcas ? [m, marcas] : [m]
  );

  const grafias = (r.rows[0] && r.rows[0].grafias) || [];
  const grupo = new Set([m.toLowerCase()]);
  for (const g of grafias) if (g) grupo.add(String(g).toLowerCase());
  return [...grupo];
}

/**
 * Traduce los filtros de la peticion a condiciones SQL con parametros.
 * Los grupos de marca y modelo son las grafias equivalentes de lo elegido;
 * llegan resueltos desde fuera porque calcularlos necesita la base y esta
 * funcion es sincrona.
 */
function construirFiltros(q, grupoMarca, grupoModelo) {
  const condiciones = [BASE];
  const valores = [];
  const añadir = (sql, valor) => {
    valores.push(valor);
    condiciones.push(sql.replace("$?", `$${valores.length}`));
  };

  const marca = texto(q.brand);
  const modelo = texto(q.model);
  const busqueda = texto(q.query);

  // Marca y modelo van por igualdad, no por LIKE: la columna esta limpia y un
  // LIKE haria que «Seat» arrastrase «Seat Comerciales».
  if (marca) {
    if (grupoMarca && grupoMarca.length > 1) añadir("lower(brand) = ANY($?)", grupoMarca);
    else añadir("lower(brand) = $?", marca.toLowerCase());
  }
  if (modelo) {
    if (grupoModelo && grupoModelo.length > 1) añadir("lower(model) = ANY($?)", grupoModelo);
    else añadir("lower(model) = $?", modelo.toLowerCase());
  }
  if (busqueda) {
    valores.push(`%${busqueda.toLowerCase()}%`);
    condiciones.push(
      `(lower(COALESCE(title,'')) LIKE $${valores.length}` +
      ` OR lower(COALESCE(brand,'')) LIKE $${valores.length}` +
      ` OR lower(COALESCE(model,'')) LIKE $${valores.length}` +
      ` OR lower(COALESCE(version,'')) LIKE $${valores.length})`
    );
  }

  for (const [clave, sql] of [
    ["minPrice", "price >= $?"], ["maxPrice", "price <= $?"],
    ["minYear", '"year" >= $?'], ["maxYear", '"year" <= $?'],
    ["minMileage", "mileage >= $?"], ["maxMileage", "mileage <= $?"],
    ["minPower", "power_cv >= $?"], ["maxPower", "power_cv <= $?"],
  ]) {
    const n = entero(q[clave]);
    if (n !== null) añadir(sql, n);
  }

  for (const [clave, columna] of [
    ["fuel", "fuel"], ["transmission", "transmission"],
    ["bodyType", "body_type"], ["color", "color"],
    ["province", "province"], ["sellerType", "seller_type"],
  ]) {
    const v = texto(q[clave]);
    if (v) añadir(`lower(${columna}) = $?`, v.toLowerCase());
  }

  return { where: condiciones.join(" AND "), valores };
}

function mapear(f, alias, grafias) {
  let imagenes = [];
  try {
    const p = JSON.parse(f.images || "[]");
    if (Array.isArray(p)) imagenes = p.filter((x) => typeof x === "string" && x);
  } catch { /* el campo puede venir vacío o mal formado; no es motivo de error */ }

  const cv = f.power_cv != null ? Number(f.power_cv) : null;
  const imagen = f.image_url || imagenes[0] || "";
  // La marca se muestra con el mismo nombre que en el desplegable: si la lista
  // dice «Citroen», la ficha no puede decir «Citroën». El titulo no se toca:
  // son las palabras del anuncio original, y de esas responde el portal.
  const marca = f.brand ? nombrePropio(canonica(f.brand, alias, grafias)) : "";

  return {
    id: f.id,
    portal: f.portal || "",
    url: f.url || "",
    searchUrl: f.url || "",
    title: f.title || [marca, f.model, f.version].filter(Boolean).join(" "),
    brand: marca,
    model: f.model || "",
    version: f.version || "",
    price: f.price === null ? null : Number(f.price),
    priceText: f.price === null ? "" : miles(f.price) + " €",
    monthlyPrice: f.monthly_price === null ? null : Number(f.monthly_price),
    financePrice: f.finance_price === null ? null : Number(f.finance_price),
    year: f.year != null ? Number(f.year) : null,
    mileage: f.mileage != null ? Number(f.mileage) : null,
    fuel: f.fuel || "",
    transmission: f.transmission || "",
    bodyType: f.body_type || "",
    body: f.body_type || "",
    color: f.color || "",
    powerCv: cv,
    powerKw: f.power_kw != null ? Number(f.power_kw) : null,
    power: cv != null ? cv + " CV" : "",
    province: f.province || "",
    city: f.city || "",
    location: [f.city, f.province].filter(Boolean).join(", "),
    dealerName: f.dealer_name || "",
    sellerType: f.seller_type || "",
    environmentalLabel: f.environmental_label || "",
    label: f.environmental_label || "",
    displacement: f.displacement || "",
    image: imagen,
    imageUrl: imagen,
    images: imagenes.length ? imagenes : (imagen ? [imagen] : []),
  };
}

/**
 * Las marcas del desplegable.
 *
 * Salen de `mmo_facetas`, que se refresca cada hora, y no de un GROUP BY
 * sobre el millon y medio de ofertas: eso tardaba 41 segundos y era la razon
 * de que el desplegable no se abriera hasta que cargaba la pagina entera.
 *
 * EL RECUENTO ES EL TOTAL DE LA MARCA, no el de la marca con tus otros
 * filtros puestos. Antes era lo segundo, y costaba ese recorrido entero cada
 * vez que tocabas cualquier cosa. Por eso la pantalla solo ensena el numero
 * cuando no hay mas filtros: una cifra que no corresponde a lo que ves es
 * peor que ninguna.
 */
async function facetaMarcas(pool) {
  const agrupadasQ = await pool.query(
    `SELECT normalize_alias_token(valor) AS clave,
            (array_agg(valor ORDER BY n DESC))[1] AS nombre,
            SUM(n)::int AS n
       FROM mmo_facetas WHERE tipo = 'marca'
      GROUP BY 1`
  );
  const agrupadas = agrupadasQ.rows.map((r) => ({
    clave: texto(r.clave), nombre: texto(r.nombre), n: Number(r.n) || 0,
  }));

  const porClave = new Map(agrupadas.map((c) => [c.clave, c]));
  const conClave = new Set(agrupadas.map((c) => c.clave));

  /* Las principales salen siempre, con un cero si hace falta: son principales
     por decision, no por tener stock. */
  const principales = await clavesPrincipales(pool);
  const principalesClaves = new Set(principales.map((p) => p.clave));
  const principalesSalida = principales.map((p) => {
    const g = porClave.get(p.clave);
    return { nombre: g ? g.nombre : p.nombre, n: g ? g.n : 0 };
  });

  const otras = agrupadas.filter((c) => !principalesClaves.has(c.clave));

  const catalogo = await pool.query(
    `SELECT name, normalize_alias_token(name) AS clave
       FROM moveadvisor_vehicle_brands WHERE is_active`
  );
  const vistas = new Set();
  const sinOfertas = [];
  for (const fila of catalogo.rows) {
    const nombre = texto(fila.name);
    const clave = texto(fila.clave) || nombre.toLowerCase();
    if (!nombre || conClave.has(clave) || principalesClaves.has(clave) || vistas.has(clave)) continue;
    vistas.add(clave);
    sinOfertas.push({ nombre, n: 0 });
  }

  // El nombre propio se pone al final, sobre lo que ya esta agrupado. Antes
  // cambiaria el texto con el que se cruzan claves y alias, y ahi cualquier
  // retoque de mayusculas es una fuente de fallos silenciosos.
  const comoNombre = (l) => l.map((x) => ({ ...x, nombre: nombrePropio(x.nombre) }));

  return {
    principales: alfabetico(comoNombre(principalesSalida)),
    conOfertas: alfabetico(comoNombre(otras)),
    sinOfertas: alfabetico(comoNombre(sinOfertas)),
  };
}
/**
 * Los modelos de la marca elegida.
 *
 * De `mmo_modelos`, donde cada grafia ya esta pegada a su modelo del
 * catalogo. Antes esto salia del pool agrupando por el texto tal cual, y por
 * eso el desplegable de Audi tenia 806 entradas: «A1» con 8.511 ofertas y
 * debajo «A1 1.0 TFSI 95CV Adrenalin» con una. Ahora son 155.
 */
async function facetaModelos(pool, q, grupoMarca) {
  const marca = texto(q.brand);
  if (!marca) return { conOfertas: [], sinOfertas: [] };

  const marcas = (grupoMarca && grupoMarca.length ? grupoMarca : [marca.toLowerCase()]);
  const conOfertas = await pool.query(
    /*
     * Lo que no pega con el catalogo solo sale si pesa algo.
     *
     * Wallapop manda un modelo llamado «1.8 TFSI 180CV SLine» y milanuncios
     * uno llamado «.», con una oferta cada uno. La lista va alfabetica, asi
     * que esos encabezaban el desplegable de Audi por delante del A1.
     * Siguen estando si pesan -un modelo raro de verdad tiene ofertas-,
     * pero una grafia suelta no es un modelo.
     */
    `SELECT (array_agg(modelo ORDER BY n DESC))[1] AS nombre, SUM(n)::int AS n
       FROM mmo_modelos
      WHERE marca = ANY($1)
      GROUP BY lower(modelo)
     HAVING bool_or(del_catalogo) OR SUM(n) >= 20`
    , [marcas]
  );

  // Igual que con las marcas: el catalogo se compara por clave, o «Leon»
  // saldria arriba y «León» otra vez abajo en «Mas modelos».
  const conClave = new Set(
    conOfertas.rows.map((r) => texto(r.nombre).toLowerCase()).filter(Boolean)
  );
  const catalogo = await pool.query(
    `SELECT m.name
       FROM moveadvisor_vehicle_models m
       JOIN moveadvisor_vehicle_brands b ON b.id = m.brand_id
      WHERE m.is_active AND b.is_active AND lower(b.name) = ANY($1)`
    , [marcas]
  );
  const vistos = new Set();
  const sinOfertas = [];
  for (const fila of catalogo.rows) {
    const nombre = texto(fila.name);
    const clave = nombre.toLowerCase();
    if (!nombre || nombre === '-' || conClave.has(clave) || vistos.has(clave)) continue;
    vistos.add(clave);
    sinOfertas.push({ nombre, n: 0 });
  }

  const lista = conOfertas.rows.map((r) => ({ nombre: texto(r.nombre), n: Number(r.n) || 0 }));
  return { conOfertas: alfabetico(lista), sinOfertas: alfabetico(sinOfertas) };
}
/**
 * Combustible, cambio, carroceria y provincia.
 *
 * Los cuatro salian de un GROUP BY sobre el pool entero —entre 8 y 15
 * segundos cada uno, y la pantalla pedia los cuatro a la vez—. Ahora se leen
 * de `mmo_facetas`, que son once filas, dos, cuarenta y ocho y tres mil.
 */
async function facetaCampo(pool, tipo) {
  const r = await pool.query(
    `SELECT (array_agg(valor ORDER BY n DESC))[1] AS nombre, SUM(n)::int AS n
       FROM mmo_facetas WHERE tipo = $1
      GROUP BY lower(valor)
     HAVING SUM(n) >= 20
      ORDER BY n DESC
      LIMIT 40`,
    [tipo]
  );
  return r.rows;
}
module.exports = async function handler(req, res) {
  const pool = getPostgresPool();
  if (!pool) {
    return res.status(503).json({ ok: false, error: "La base de ofertas no está disponible." });
  }

  const url = new URL(req.url, "http://local");
  const q = Object.fromEntries(url.searchParams.entries());

  try {
    const facets = texto(q.facets);

    // Se resuelve una vez y viaja a todo lo que consulte con la marca puesta.
    const marcaPedida = texto(q.brand);
    const grupoMarca = marcaPedida ? await grupoDeMarca(pool, marcaPedida) : null;
    const grupoModelo = texto(q.model) ? await grupoDeModelo(pool, grupoMarca, q.model) : null;

    if (facets === "brands") {
      return res.status(200).json({ ok: true, ...(await facetaMarcas(pool)) });
    }
    if (facets === "models") {
      return res.status(200).json({ ok: true, ...(await facetaModelos(pool, q, grupoMarca)) });
    }
    if (facets === "extra") {
      const [combustible, cambio, carroceria, provincia] = await Promise.all([
        facetaCampo(pool, "fuel"),
        facetaCampo(pool, "transmission"),
        facetaCampo(pool, "body_type"),
        facetaCampo(pool, "province"),
      ]);
      return res.status(200).json({ ok: true, combustible, cambio, carroceria, provincia });
    }

    const limite = Math.min(MAX_LIMITE, Math.max(1, entero(q.limit) ?? LIMITE_POR_DEFECTO));
    const desde = Math.max(0, entero(q.offset) ?? 0);
    const orden = ORDENES[texto(q.sort)] || ORDENES.recientes;

    const { where, valores } = construirFiltros(q, grupoMarca, grupoModelo);

    // El total y la pagina van en paralelo: son dos consultas independientes y
    // esperarlas en serie duplicaba el tiempo de respuesta.
    const [total, pagina, alias, grafias] = await Promise.all([
      pool.query(
        `SELECT COUNT(*)::int AS n FROM (
           SELECT 1 FROM moveadvisor_market_offers WHERE ${where} LIMIT ${TOPE_DEL_TOTAL + 1}
         ) x`,
        valores
      ),
      pool.query(
        `SELECT id, portal, url, brand, model, version, title, price, monthly_price,
                "year", mileage, fuel, transmission, body_type, color, power_cv,
                province, seller_type, image_url, images
           FROM moveadvisor_market_offers
          WHERE ${where}
          ORDER BY ${orden}
          LIMIT ${limite} OFFSET ${desde}`,
        valores
      ),
      aliasDeMarcas(pool),
      mapaGrafias(pool),
    ]);

    const contadas = total.rows[0].n;

    return res.status(200).json({
      ok: true,
      total: contadas > TOPE_DEL_TOTAL ? TOPE_DEL_TOTAL : contadas,
      // Para que la pantalla diga «mas de 10.000» y no una cifra inventada.
      hayMas: contadas > TOPE_DEL_TOTAL,
      limite,
      desde,
      ofertas: pagina.rows.map((f) => mapear(f, alias, grafias)),
    });
  } catch (error) {
    return res.status(500).json({ ok: false, error: error?.message || "Error al buscar ofertas." });
  }
};
