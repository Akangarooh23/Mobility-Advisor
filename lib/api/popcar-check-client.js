...
/**
 * Cliente de servicio contra PopCar Check (check.popcar.com.es).
 *
 ...
 * PopCar Check, donde el esquema impide afirmar mecánica sin verificación
 ...
 * PopCar Check exige UUID. Un UUID v5 sobre este espacio de nombres da
...
/**
 * A donde vive PopCar Check.
 *
 * La reserva era captura.carswiseai.com, de dos marcas atras, y hoy devuelve
 * 404: ningun proyecto de Vercel reclama ese nombre. Con ella puesta, faltar la
 * variable no se notaba como "falta configurar" sino como capture_unreachable,
 * que suena a caida de red y manda a mirar donde no es.
 *
 * Sin variable devuelve cadena vacia y `estaConfigurado()` lo trata como
 * apagado, igual que cuando falta la clave.
 */
function baseUrl() {
  return String(process.env.CARSWISE_CHECK_URL || "").trim().replace(/\/+$/, "");
}
...
function estaConfigurado() {
  return Boolean(claveInterna()) && Boolean(baseUrl());
}
 ...
 *
 * Manda lo que el perfil tenga: en PopCar Check solo `vehicle_id` es
 ...
 *
 * Por aquí y no directo a PopCar Check por lo mismo que el PDF: el navegador
 ...
 *
 * PopCar Check responde a la petición del modelo con un 302 a una URL firmada
...
async function urlDelModelo3d(sessionId, formato) {
  ...
  try {
    ...
    // Sin modelo propio, PopCar Check devuelve el esquema generado al vuelo. Eso sí
