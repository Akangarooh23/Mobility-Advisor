...
const {
  tipoDelImpuesto, bandaPorCo2, bandaEstimada, impuestoDeMatriculacion, co2Estimado,
...
/**
 * La base es el precio de la factura, y con eso el número deja de ser una
 * estimación.
 *
 * Es la comprobación más importante de este fichero, porque es la que ata el
 * cálculo a un hecho: al Sorento le cobraron 2.491 € y su factura son 16.890.
 * 16.890 × 14,75 % = 2.491,28. Al céntimo.
 *
 * Aquí se aproximó la base con el 60 % del precio de un usado comparable en
 * España, y por ese camino salían 2.646. No estaba mal por poco: estaba
 * resolviendo un problema que no existe, porque el precio de compra lo sabemos
 * desde que se publica el anuncio.
 */
test("el impuesto sale de la factura de compra, no del comparable español", () => {
  const FACTURA = 16890;
  const COMPARABLE_ESPANOL = 29899;
  assert.equal(impuestoDeMatriculacion(FACTURA, SORENTO).toFixed(2), "2491.28");
  assert.notEqual(
    Math.round(impuestoDeMatriculacion(FACTURA, SORENTO)),
    Math.round(impuestoDeMatriculacion(COMPARABLE_ESPANOL, SORENTO)),
    "la base ya no puede ser el comparable español"
  );
});

test("y lo que costó de verdad cuadra con lo que se le habría cobrado", () => {
  // 2.491,00 € cobró la gestoría. Un euro de diferencia es el redondeo de
  // Hacienda, no un error del cálculo.
  const nuestro = impuestoDeMatriculacion(16890, SORENTO);
  assert.ok(Math.abs(nuestro - 2491) < 1, `salen ${nuestro.toFixed(2)} €`);
});

test("un utilitario no paga como un SUV", () => {
  // Era el otro lado del mismo fallo: con un tipo único, o el SUV va corto o el
  // utilitario va caro.
  const golf = impuestoDeMatriculacion(18000, {
    fuel: "Gasolina", power_cv: 110, displacement: "999", title: "VW Golf 1.0 TSI",
  });
  const x5 = impuestoDeMatriculacion(45000, {
    fuel: "Diesel", power_cv: 265, displacement: "2993", title: "BMW X5 xDrive30d",
  });
  assert.equal(Math.round(golf), 855, "el utilitario ya no paga como un SUV");
  assert.ok(x5 > golf * 5, `el SUV grande sale a ${Math.round(x5)} y el utilitario a ${Math.round(golf)}`);
});

test("sin precio no sale un impuesto negativo ni NaN", () => {
  assert.equal(impuestoDeMatriculacion(null, SORENTO), 0);
  assert.equal(impuestoDeMatriculacion("lo que sea", SORENTO), 0);
});
