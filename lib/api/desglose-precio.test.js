...
const {
  ...
  FEE_POPCAR, impuestoEstimado, sqlTipoDelImpuesto,
...
describe("el precio partido", () => {
  ...
  test("el impuesto va sobre el precio español, no sobre el alemán", () => {
    ...
    assert.equal(imp.importe, Math.round(impuestoEstimado(AQUI, null)));
  });
...
describe("el mismo precio en SQL que en pantalla", () => {
  ...
  test("y el impuesto sobre el precio español", () => {
    // El mismo tipo que en pantalla, y sobre el mismo valor fiscal: si el
    // listado y la ficha no coinciden, quien lo ve deja de fiarse de los dos.
    assert.ok(sqlPrecioPuestoAqui().includes(sqlTipoDelImpuesto()));
    assert.ok(sqlPrecioPuestoAqui().includes(`*0.6*COALESCE(market_price_es,0)`));
  });
