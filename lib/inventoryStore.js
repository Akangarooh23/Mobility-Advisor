...
function computeUsageImpact(offers, userKm, userYear, medianPrice, priceField, brand) {
  ...
  if (!medianPrice || !userKm || !userYear) {
    ...
    return { usageImpact: 0, rawUsageImpact: 0, usedDefault: null, slopeKm: null, slopeYear: null };
  }
  ...
  const defaultImpact = () => {
    ...
    return { usageImpact: Math.round(Math.max(-cap, Math.min(cap, raw))), rawUsageImpact: Math.round(raw), usedDefault: true, slopeKm: null, slopeYear: null };
  };
...
async function getMarketPriceSnapshot(options = {}) {
  ...
  const { usageImpact, rawUsageImpact, usedDefault: usageUsedDefault, slopeKm, slopeYear } =
