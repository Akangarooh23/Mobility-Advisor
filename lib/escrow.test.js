...
describe("lo que deposita el cliente", () => {
  test("el coche y nuestro fee", () => {
    const d = loQuePagaAhora({ precioCoche: 18000, fee: 2999 });
    assert.equal(d.coche, 18000);
    assert.equal(d.fee, 2999);
    assert.equal(d.total, 20999);
  });
  ...
  test("y la garantía si la ha elegido", () => {
    const d = loQuePagaAhora({ precioCoche: 18000, fee: 2999, garantia: 590 });
    assert.equal(d.garantia, 590);
    assert.equal(d.total, 21589);
  });
