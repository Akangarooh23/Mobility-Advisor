...
// Relativas a hoy: con fechas fijas la prueba dejaba de pasar el día en que
// esas horas quedaban atrás, porque una hora propuesta que ya ha pasado no vale.
const enDias = (d, h) => { const x = new Date(Date.now() + d * 86400000); x.setUTCHours(h, 0, 0, 0); return x.toISOString(); };
const PROPUESTAS = [enDias(5, 10), enDias(6, 16)];
...
before(() => {
  Pool.prototype.query = function (sql, params, cb) {
    ...
    // La comprobacion de si esa hora ya la tiene otro.
    if (/SELECT id FROM vehicle_visit_bookings/i.test(t) && /id != /i.test(t)) {
      return responde(horaOcupada ? [{ id: "b-otra" }] : []);
    }
    if (/FROM vehicle_visit_bookings/i.test(t) && /SELECT/i.test(t)) {
      // La base sí mira el WHERE, así que aquí también: si no, un test pasaría
      // igual con la consulta pidiendo un estado que la reserva no tiene.
      if (/status = .confirmed./i.test(t) && estado !== "confirmed") return responde([]);
      if (/status != .cancelled./i.test(t) && estado === "cancelled") return responde([]);
    ...
    if (/FROM vehicle_visit_availability/i.test(t) && /FOR UPDATE/i.test(t)) {
      return responde([{ id: "s-otro", offer_id: RESERVA.offer_id, starts_at: PROPUESTAS[0], ends_at: PROPUESTAS[0], status: "available" }]);
    }
...
describe("lo que no se deja hacer", () => {
  test("una hora que no se le propuso", async () => {
    const { codigo } = await elige(enDias(9, 8));
...
describe("mover y cancelar desde la cita del cliente", () => {
  /** Llama a una ruta cualquiera del handler y devuelve código y cuerpo. */
  async function llama(route, body) {
    const req = { method: "POST", headers: {}, query: { route }, body: { route, ...body } };
    let codigo = 200;
    let cuerpo = null;
    const res = {
      status(c) { codigo = c; return res; },
  ...
  test("una visita pendiente también se puede cambiar de hora", async () => {
    estado = "pending";
    const { codigo, cuerpo } = await llama("reschedule", {
      bookingId: RESERVA.id, token: "t-buena", newSlotId: "s-otro",
    });
    assert.equal(codigo, 200, "toda visita nace pendiente: si solo se pudiera mover una confirmada, no se podría mover ninguna recién pedida");
    assert.equal(cuerpo.ok, true);
  });

  test("cancelar deja rastro de que lo hizo el cliente", async () => {
    const { codigo } = await llama("cancel", { bookingId: RESERVA.id, token: "t-buena" });
    assert.equal(codigo, 200);
    const cancelada = pasos.find((p) => p.evento === "cancelada");
    assert.ok(cancelada, "una cancelada desaparece de la Agenda; sin rastro nadie sabe quién la quitó");
    assert.equal(cancelada.actor, "cliente");
  });
});

describe("dos personas a la misma hora", () => {
  test("no se puede coger una hora que ya tiene otro", async () => {
    horaOcupada = true;
    const { codigo } = await elige(PROPUESTAS[0]);
    assert.equal(codigo, 409, "el hueco se crea si no existe, así que sin comprobarlo se presentan los dos");
    assert.equal(guardado, null);
  });
});
