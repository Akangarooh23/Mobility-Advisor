...
const { plantilla, parrafo, datos, aviso, textoPlano } = require("../correo");
const { identidadDeLaPeticion } = require("./identidad");
const { precioPuestoAqui, desgloseParaElCliente, FEE_POPCAR_CON_IVA } = require("../coste-importacion");
...
function getPool() {
  ...
  _pool = new Pool({ connectionString: connString, ssl: SSL_POSTGRES });
  return _pool;
}
...
async function sendEmails({ name, email, title, price, deposit, deposito }) {
  ...
  const manda = async (quien, cuerpo) => {
    const resp = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify(cuerpo.html ? { ...cuerpo, text: textoPlano(cuerpo.html) } : cuerpo),
    });
    if (!resp.ok) {
...
module.exports = async function importLeadHandler(req, res) {
  ...
  try {
    ...
    const offerRes = await pool.query(
      // Con el combustible, la potencia, la cilindrada y el CO₂: son los que
      // deciden la banda del impuesto de matriculación. Sin ellos se estima con
      // el tipo más bajo, y a un SUV grande le faltan mil euros.
      `SELECT title, price::numeric AS price, import_cost, market_price_es, year, mileage,
              fuel, power_cv, co2, displacement, body_type
       FROM moveadvisor_market_offers WHERE id = $1 AND country = 'DE' AND import_published = TRUE
         AND COALESCE(is_active, TRUE) = TRUE
         -- Y que no sea ya de otro cliente. Un anuncio aleman sigue vivo y
         -- sigue saliendo a cuenta despues de que se lo compremos a alguien,
         -- asi que sin esto dos clientes pueden encargar el mismo coche.
    ...
    const importPrice = Math.round(
      precioPuestoAqui(offer.price, offer.market_price_es, 0, offer) + (elegida.precio || 0)
    );
    /**
    ...
    const partes = desgloseParaElCliente(offer.price, offer.market_price_es, null, offer);
