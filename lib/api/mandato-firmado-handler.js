...
module.exports = async function mandatoFirmadoHandler(req, res) {
  ...
  try {
    ...
    /*
     * Y al cajón privado, no al de las fotos.
     *
     * Un mandato firmado lleva su nombre, su matrícula y su firma. En el bucket
     * público es alcanzable por quien tenga la dirección, saltándose la sesión
     * que pide el ERP para enseñarlo — que era justo lo que se creía que
     * protegía el papel.
     */
    ...
    /*
     * Se guarda con un nombre que se lee, no con el del móvil.
     *
     * Lo que sube se llama `documento (1).docx` o `scan_0007.pdf`. En la ficha
     * del encargo, al lado del mandato en blanco, eso no dice cuál de los dos
     * es el firmado — y ésa es toda la diferencia.
     */
    const comoSeLlama = comoSeLlamaElPapel(M.PAPEL, e.plate, nombre);

    await pool.query(M.SQL_GUARDA_DOCUMENTO, [encargoId, comoSeLlama, tipo, camino, tamano, email]);
