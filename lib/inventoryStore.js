...
const { inferFuelFromVersion, isFuelCompatible, getFuelCompatTokens } = require("./inferFuelFromVersion");
...
// Compatible fuel values for PostgreSQL IN clauses (lowercase DB form, accents preserved).
// 'híbrido' includes 'híbrido enchufable' so PHEV appears in a Híbrido search.
// gas/gnc/glp are grouped because 900 total offers — splitting leaves pools too thin.
// 1:1 cases (gasolina, diésel, eléctrico, híbrido enchufable) fall through to exact =.
...
function pgFuelCondition(fuelValue, values, colExpr) {
  ...
  if (compat) {
    ...
    const phs = compat.map((_, i) => `$${startIdx + i}`).join(', ');
    return `${colExpr} IN (${phs})`;
  }
...
function getPostgresPool() {
  ...
  if (!postgresPool) {
    postgresPool = new Pool({ connectionString, ssl: SSL_POSTGRES });
  }
...
/**
 * Dónde está este coche, a efectos de filtrar y de puntuar.
 *
 * Antes esto era `province + city` de la fila. Con la deduplicación eso pasó a
 * ser una respuesta incompleta: la fila que sobrevive es una de varias, y las
 * demás pueden estar en otras ciudades. El coche no se ha movido, sólo hemos
 * dejado de enseñar sus otros anuncios.
 *
 * Se busca por subcadena —igual que antes— porque los portales escriben
 * "CLICARS SEVILLA (Madrid)" y cosas peores. Devuelve una sola cadena para que
 * las tres llamadas (filtro de ciudad, filtro de ubicación y puntuación) no
 * puedan divergir: si alguna se queda con la definición vieja, un coche saldría
 * en el listado y no en el orden, o al revés.
 */
function ubicacionesDe(offer = {}) {
  const delGrupo = Array.isArray(offer.locations) ? offer.locations.join(" ") : "";
  return normalizeToken(`${offer.province || ""} ${offer.city || ""} ${delGrupo}`);
}
...
function normalizeOffer(raw = {}) {
  ...
  // Todas las ciudades donde este coche está publicado, no sólo la del anuncio
  // que sobrevivió a la deduplicación. Un concesionario con delegaciones publica
  // el mismo coche una vez por ciudad —el Dacia Dokker aparecía en 36—, y sin
  // esto filtrar por Córdoba escondería un coche que también está en Córdoba
  // sólo porque la canónica dice Zaragoza. Viene de moveadvisor_offer_duplicates
  // ya normalizado (minúsculas, sin acentos). Vacío si el coche no tiene grupo.
  const locationsRaw = firstDefined(raw, ["_ubicaciones", "ubicaciones"]);
  const locations = Array.isArray(locationsRaw)
    ? locationsRaw.map((u) => normalizeText(u)).filter(Boolean)
    : [];
  /*
   * De donde viene. Uno de cada siete anuncios es un importado aleman, y eso
   * cambia lo que vale: la mediana se calcula solo con coches ya matriculados
   * en Espana. Ver lib/lo-que-vale-en-el-mercado.js.
   */
  const country = normalizeText(firstDefined(raw, ["country", "pais"])).toUpperCase();
...
function buildSqlServerWhereClause(filters = {}) {
  ...
  if (normalizedFuel) {
    ...
    // Exact match per compatible token — LIKE '%gas%' matchea 'gasolina', LIKE '%gasolina%' matchea 'gasolina gas'.
    const compatTokens = getFuelCompatTokens(normalizedFuel) ?? [normalizedFuel];
    const fuelOr = compatTokens.map(t => `LOWER(Fuel) = N'${escapeSqlString(t)}'`).join(' OR ');
    conditions.push(`(Fuel IS NULL OR Fuel = N'' OR ${fuelOr})`);
  }
  ...
  if (normalizedSellerType) {
    conditions.push(`(SellerType IS NULL OR LOWER(SellerType) LIKE N'%${escapeSqlString(normalizedSellerType)}%')`);
  }
...
/**
 * **La tasación deduplica comparables antes de seleccionar el pool; una misma
 * oferta no puede contribuir varias veces a la valoración.**
 *
 * Esa frase es la propiedad, no el comentario de un arreglo. El pool cuenta
 * coches, no anuncios. Quien toque estas consultas tiene que poder seguir
 * diciéndola en voz alta.
 *
 * ── Antes esto eran 140 líneas de SQL ───────────────────────────────────────
 *
 * La primera versión agrupaba aquí mismo, con dos funciones de ventana y la
 * huella escrita a mano. Funcionaba, y tenía dos defectos que solo se ven desde
 * fuera:
 *
 *  1. **La regla vivía en dos sitios.** La misma huella estaba aquí y en el
 *     proceso nocturno, y el día que una se afine y la otra no, la tasación y
 *     el marketplace dirían cosas distintas del mismo coche.
 *  2. **Elegían canónicos distintos.** Ésta se quedaba con el anuncio
 *     actualizado más recientemente; la tabla, con el más antiguo. Las dos
 *     dejaban uno por coche —comprobado: cero repetidos en pantalla— pero
 *     señalaban anuncios diferentes, así que un cliente podía ver una ficha en
 *     el buscador y otra distinta al entrar.
 *
 * Ahora la agrupación se decide una vez, de madrugada, en
 * `moveadvisor_offer_duplicates`, y aquí solo se consulta.
 *
 * ── Por qué NO basta con "esconde lo que no sea canónico" ───────────────────
 *
 * Fue la primera versión de este cambio y la medí antes de darla por buena:
 *
 *     AND NOT EXISTS (SELECT 1 FROM moveadvisor_offer_duplicates d
 *                     WHERE d.offer_id = ... AND d.offer_id <> d.canonical_id)
 *
 * Parece inofensivo y no lo es. La huella agrupa por marca, versión, año, km,
 * precio y grupo de vendedor — **no por `model`**. Pero la consulta dirigida
 * filtra por `brand || model || version`. Cuando dos copias del mismo coche
 * escriben el modelo distinto ("Mégane" / "Megane", "Línea" / "Linea", o el
 * modelo directamente vacío), puede pasar que la canónica quede FUERA del
 * filtro y la copia dentro. Con el `NOT EXISTS`, la copia se esconde y nadie
 * la sustituye: el coche no se deduplica, desaparece.
 *
 * Medido sobre la base real: 478 copias activas tienen un `model` distinto al
 * de su canónica, de 166.315 copias. Es el 0,29 % — poco, y aun así es pérdida
 * silenciosa de comparables, que es justo lo que no queremos.
 *
 * Así que la tabla aporta **el grupo**, y el superviviente se elige entre las
 * filas que sí pasan el filtro. Prefiere la canónica cuando está visible —para
 * que buscador y tasación señalen el mismo anuncio— y sólo si no lo está se
 * queda con la más fresca. Ningún coche puede perderse: todo grupo con al menos
 * una fila visible aporta exactamente una.
 *
 * ── Qué pasa si la tabla se queda vieja o vacía ─────────────────────────────
 *
 * Degrada hacia el lado seguro. El `LEFT JOIN` deja `canonical_id` a NULL y el
 * `COALESCE` cae al propio `id`: cada oferta es su propio grupo y vuelven a
 * entrar duplicados —el comportamiento de antes—, pero no desaparece ninguna.
 *
 * Aun así, el silencio se vigila: el parte de silencios avisa si la agrupación
 * lleva 36 h sin rehacerse. Hizo falta porque el trabajo nocturno estuvo dos
 * días declarado y sin arrancar sin que nadie lo notara.
 *
 * @param {string} columnas  lista SELECT
 * @param {string} filtro    condiciones WHERE, ya montadas
 * @param {string} limite    marcador del LIMIT ("$1")
 */
function unaFilaPorCoche(columnas, filtro, limite, primeroCorta = false) {
  const grupo = "COALESCE(d.canonical_id, o.id)";

  /*
   * ── Cortar primero y agrupar después ───────────────────────────────────
   *
   * La consulta de siempre agrupa el conjunto ENTERO que pasa el filtro y
   * solo al final se queda con doce. Para la tasación está bien: necesita
   * todos los comparables y da igual que tarde.
   *
   * Para el consejero no. Ahí el filtro puede dejar medio millón de ofertas
   * -«gasolina» lo es- y ordenar medio millón de filas con su `LEFT JOIN`
   * para enseñar doce tardaba minutos. La pantalla corta a los 30 segundos,
   * así que el cliente veía el aviso de que se había tardado demasiado.
   *
   * Con `primeroCorta` se toman las más recientes que cumplen y se agrupan
   * esas. El índice de `updated_at` deja parar en cuanto hay suficientes.
   *
   * QUÉ SE PIERDE, dicho claro: se agrupa dentro de la ventana y no sobre
   * todo el pool. Dos copias del mismo coche que caigan las dos dentro se
   * juntan igual; si una queda fuera de la ventana, la de dentro se enseña
   * sola, que es justo lo que se quería. Lo que ya no se puede es preferir
   * la canónica cuando la canónica está fuera de la ventana. Para doce
   * ofertas de un consejo es un cambio que no se ve; para contar comparables
   * sí, y por eso la tasación no lo usa.
   */
  const ventana = primeroCorta
    ? `(SELECT * FROM moveadvisor_market_offers
         WHERE ${filtro}
         ORDER BY updated_at DESC NULLS LAST
         LIMIT ${limite} * 20) o`
    : `moveadvisor_market_offers o`;

  const coches = `
      SELECT DISTINCT ON (${grupo})
        ${columnas},
        d.ubicaciones AS _ubicaciones,
        o.scraped_at AS _scraped_at
      FROM ${ventana}
      LEFT JOIN moveadvisor_offer_duplicates d
        ON d.offer_id = o.id
      ${primeroCorta ? "" : `WHERE ${filtro}`}
      ORDER BY
        ${grupo},
        (o.id = d.canonical_id) DESC NULLS LAST,
        o.updated_at DESC NULLS LAST,
        o.scraped_at DESC NULLS LAST,
        o.id`;

  /*
   * ── Como mucho tres del mismo modelo ───────────────────────────────────
   *
   * Solo en el camino del consejero, y por una razon medida: a un perfil de
   * Madrid con siete criterios le salia UNA tarjeta teniendo la base 181
   * ofertas que cumplian. Se piden treinta filas, se toman las mas recientes,
   * y un concesionario que publica treinta Peugeot 308 se las lleva todas.
   * Al quedarse una por modelo para no ensenar tres veces el mismo coche,
   * quedaba una.
   *
   * La ventana ya trae veinte veces lo que hace falta, asi que el reparto no
   * cuesta ninguna lectura mas: se ordena dentro de cada modelo y se cortan
   * las sobrantes.
   *
   * La tasacion NO pasa por aqui: ahi hacen falta todos los comparables del
   * mismo modelo, que es justo lo contrario.
   */
  if (!primeroCorta) {
    return `
    SELECT * FROM (
${coches}
    ...
    ORDER BY updated_at DESC NULLS LAST, _scraped_at DESC NULLS LAST
    LIMIT ${limite}`;
  }
  ...
  return `
    SELECT * FROM (
      SELECT *, row_number() OVER (
        PARTITION BY lower(COALESCE(brand,'')), lower(COALESCE(model,''))
        ORDER BY updated_at DESC NULLS LAST
      ) AS _del_mismo_modelo
      FROM (
${coches}
      ) coches
    ) repartidas
    WHERE _del_mismo_modelo <= 3
    ORDER BY updated_at DESC NULLS LAST, _scraped_at DESC NULLS LAST
    LIMIT ${limite}`;
}
/**
 * Lee el universo de ofertas del que después se elige.
 *
 * `criterios` es lo que pide el consejero —combustible, carrocería,
 * etiqueta, precio, año— y va al `WHERE`. Antes no iba: se traían las 30.000
 * ofertas más recientemente actualizadas del pool entero y se filtraba en
 * JavaScript. Con 1.579.000 visibles, eso es buscar un compacto con etiqueta
 * CERO mirando las últimas treinta mil que tocó el scraper: entre 25 y 103
 * segundos, y cero resultados en los cinco perfiles que probé.
 */
async function readPostgresInventory(limit = 30000, modelTokens = [], criterios = null) {
  ...
  try {
    ...
    if (safeTokens.length > 0) {
      ...
      /*
       * La busqueda por modelo tambien pasa por lo presentable.
       *
       * Se salta las demas condiciones a proposito -si pides un Corolla,
       * quieres Corollas aunque no encajen con el resto- pero no las de
       * poder enseñarlo: siete de los doce Corollas que devolvia no tenian
       * foto.
       */
      const presentables = criterios && criterios.soloPresentables
        ? condicionesDelConsejero({ soloPresentables: true }, params.length).condiciones
        : [];

      /*
       * ── El modelo sugerido estrecha, no manda ──────────────────────────
       *
       * Esta consulta se saltaba TODOS los criterios a proposito: si pides un
       * Corolla, quieres Corollas aunque no encajen con el resto. Para quien
       * escribe un modelo a mano es lo correcto.
       *
       * Para el consejero no, porque ahi los modelos no los ha pedido nadie:
       * los sugiere el analisis. Medido con un perfil de Madrid con siete
       * criterios contestados, la base tenia 181 ofertas que cumplian y en
       * pantalla salia UNA: la consulta traia coches de los modelos sugeridos
       * de toda Espana, con cualquier cambio y cualquier precio, y los filtros
       * de despues los iban tirando uno a uno.
       *
       * Asi que primero se pregunta por los modelos CON los criterios. Si eso
       * no devuelve nada -el modelo sugerido no existe con esas condiciones-
       * se vuelve a preguntar solo por el modelo, que es como estaba: mejor
       * ensenar un Corolla que no encaja del todo que no ensenar ninguno.
       */
      const conCriterios = criterios
        ? condicionesDelConsejero(criterios, params.length, await losQueHay(pool))
        : { condiciones: [], valores: [] };

      const preguntaPorModelo = async (extra, valores) => {
        const query = unaFilaPorCoche(
          selectCols,
          [`(${conditions.join(" OR ")})`, "COALESCE(is_active, TRUE) = TRUE", ...extra].join(" AND "),
          "$1",
          Boolean(criterios && criterios.soloPresentables),
        );
        const res = await pool.query(query, [...params, ...valores]);
        return (res.rows || []).map(normalizeOffer).filter(Boolean);
      };

      try {
        if (conCriterios.condiciones.length) {
          const conTodo = await preguntaPorModelo(conCriterios.condiciones, conCriterios.valores);
          if (conTodo.length > 0) {
            return conTodo;
          }
        }
        ...
        /*
         * Y si con los criterios no hay nada, se sueltan LOS MODELOS, no los
         * criterios.
         *
         * Aqui se preguntaba por los modelos sin ninguna condicion. Para quien
         * escribe «Corolla» a mano es lo correcto. Para el consejero es al
         * reves, y se vio contestando el test entero: a una pareja que pidio
         * un COMPACTO generalista europeo, el analisis le propuso un C-HR, un
         * Niro, un Kona y un Qashqai -cuatro SUV japoneses y coreanos- y esta
         * consulta los traia. El colador los tiraba todos por carroceria y la
         * pantalla acababa con CERO OFERTAS.
         *
         * Los modelos del consejero son una sugerencia del modelo; la
         * carroceria, el presupuesto y los kilometros los ha dicho el cliente.
         * Si ninguno de los modelos sugeridos existe con lo que pidio, se
         * busca por lo que pidio, que es lo que hace la consulta general de
         * mas abajo.
         */
        if (!criterios) {
          const soloElModelo = await preguntaPorModelo(presentables, []);
          if (soloElModelo.length > 0) {
            return soloElModelo;
          }
        }
      } catch (err) {
    ...
    /*
     * Lo que pide el consejero, dentro del WHERE.
     *
     * Solo estrecha: lo que no se sabe traducir no se filtra, y el filtro
     * fino de después sigue siendo quien decide. Si aun asi no sale nada,
     * se vuelve a preguntar sin condiciones —estrechar de mas deja al
     * cliente sin ofertas, que es lo que veniamos a arreglar—.
     */
    /*
     * Los valores que de verdad hay en cada columna.
     *
     * Con ellos, el combustible y la carroceria se comparan con  en vez de
     * con , que no puede usar indice y obligaba a recorrer la
     * tabla entera: 118 segundos medidos con EXPLAIN. Son once combustibles y
     * cuarenta y ocho carrocerias; se leen de la vista de los desplegables y
     * se guardan cinco minutos.
     */
    const enLaBase = criterios ? await losQueHay(pool) : null;
    const delConsejero = criterios
      ? condicionesDelConsejero(criterios, 1, enLaBase)
      : { condiciones: [], valores: [] };

    const modernQuery = unaFilaPorCoche(
      selectCols,
      ["COALESCE(is_active, TRUE) = TRUE", ...delConsejero.condiciones].join(" AND "),
      "$1",
      Boolean(criterios && criterios.soloPresentables),
    );

    try {
      const modernResult = await pool.query(modernQuery, [boundedLimit, ...delConsejero.valores]);
      const modernList = (modernResult.rows || []).map(normalizeOffer).filter(Boolean);
      if (modernList.length > 0) {
        return modernList;
      }

      if (delConsejero.condiciones.length) {
        /*
         * Al ensanchar se sueltan los criterios, no lo presentable: mejor un
         * coche que no es exactamente lo pedido que una tarjeta sin foto.
         */
        const alEnsanchar = criterios && criterios.soloPresentables
          ? condicionesDelConsejero({ soloPresentables: true }, 1).condiciones
          : [];
        const sinCondiciones = await pool.query(
          unaFilaPorCoche(
            selectCols,
            ["COALESCE(is_active, TRUE) = TRUE", ...alEnsanchar].join(" AND "),
            "$1",
            Boolean(criterios && criterios.soloPresentables),
          ),
          [boundedLimit],
        );
        const anchaList = (sinCondiciones.rows || []).map(normalizeOffer).filter(Boolean);
        if (anchaList.length > 0) {
          return anchaList;
        }
      }
    } catch (err) {
...
/**
 * Los limites de Tukey (IQR x1,5) de una lista de precios ya ordenada.
 *
 * Se saca aparte porque la valla no solo hace falta para los precios: tambien
 * hay que poder preguntarle a una **oferta** si cae dentro. Con la valla suelta,
 * el precio recomendado se calculaba sobre los precios limpios y las tablas de
 * debajo seguian leyendo la lista entera.
 *
 * Sin datos suficientes o sin dispersion no hay valla que valga: se devuelve
 * abierta, que es lo mismo que no filtrar.
 */
function tukeyFence(sortedPrices) {
  const abierta = { lo: -Infinity, hi: Infinity };
  if (sortedPrices.length < 4) return abierta;
  const q1 = percentile(sortedPrices, 0.25);
  const q3 = percentile(sortedPrices, 0.75);
  const iqr = q3 - q1;
  if (!iqr) return abierta;
  return { lo: q1 - 1.5 * iqr, hi: q3 + 1.5 * iqr };
}

function removeOutliers(sortedPrices) {
  const { lo, hi } = tukeyFence(sortedPrices);
...
function computeUsageImpact(offers, userKm, userYear, medianPrice, priceField, brand) {
  ...
  /*
    `y` TAMBIÉN se centra, y sin esto lo demás no significa nada.

    `solveOLS2x2` no tiene término independiente: ajusta y = β₁x₁ + β₂x₂ forzando
    la recta por el origen. Eso es correcto solo si las TRES variables están
    centradas. Los predictores lo estaban —`km − mediana`, `año − mediana`, media
    cero— pero los precios entraban en bruto, con media ~11.000 €. La recta tenía
    que explicar esos 11.000 € con variables que valen cero de media, así que las
    pendientes se deformaban para absorber la media en vez de medir la relación.

    El efecto no era un sesgo constante, que se vería: era INESTABILIDAD. Dos
    pools casi idénticos daban pendientes muy distintas.

    Medido sobre el mismo SEAT León, con el pool de antes y después de deduplicar:

                        slopeKm      slopeYear   impacto
      en bruto (antes)  -8,55e-3        -183       667 €
      en bruto (antes)  -4,96e-3        +155       396 €   ← 72 % de diferencia
      centrado          -2,55e-2        +529     1.989 €
      centrado          -2,40e-2        +572     1.922 €   ← 3 % de diferencia

    Nótese el `slopeYear` NEGATIVO de la primera fila: "un coche más nuevo vale
    menos". El guardián de signo que hay debajo lo cazaba y caía al valor por
    defecto — lo que explica que a veces se usara la tabla por segmento sin
    motivo aparente. No era que el mercado fuera raro: era esta regresión.

    Se descubrió persiguiendo por qué la deduplicación había aplanado el
    `slopeKm` de un León hasta 3,37e-5 —100.000 km restaban 3,37 €—. No lo había
    aplanado: solo movió el pool lo justo para que una regresión ya inestable
    diera otro número.
  */
  const mediaY = ys.reduce((s, v) => s + v, 0) / ys.length;
  const [sk, sy] = solveOLS2x2(
    x1s.map((v) => v / s1),
    x2s.map((v) => v / s2),
    ys.map((v) => v - mediaY),
  );
...
async function readInventoryUniverse(options = {}) {
  ...
  /*
   * Con criterios, el universo es mucho mas pequeño y mucho mas util, asi
   * que con tres mil sobra: antes hacian falta treinta mil porque venian sin
   * filtrar y habia que confiar en que algo encajara.
   */
  /*
   * Los criterios, recorriendo la lista que declara quien escribe el SQL.
   *
   * Antes se enumeraban a mano aqui y lo que no estuviera escrito se perdia en
   * silencio: la provincia, los kilometros, el cambio y el vendedor, en cuatro
   * ocasiones distintas. Ver LOS_CRITERIOS en lo-que-busca-el-consejero.js.
   */
  const criterios = {};
  for (const clave of LOS_CRITERIOS) {
    const valor = options[clave];
    if (valor !== null && valor !== undefined && valor !== "") criterios[clave] = valor;
  }
  // La provincia tiene dos entradas y la del test manda; ver el mismo fichero.
  criterios.province = options.province || options.location || null;
  criterios.provinciaFormas = options.provinciaFormas || null;
  // Lo pide el consejero, no la tasacion: ver lo-que-busca-el-consejero.js.
  criterios.soloPresentables = Boolean(options.soloPresentables);
  const hayCriterios = Object.entries(criterios).some(([k, v]) => (k === 'soloPresentables' ? v === true : v !== null && v !== undefined && v !== ''));
  const fromPostgres = await readPostgresInventory(
    hayCriterios ? 3000 : 30000,
    pgModelTokens,
    hayCriterios ? criterios : null,
  );
...
async function listInventoryOffers(options = {}) {
  const {
    ...
    limit = 20,
    /*
     * Lo que pide el consejero y la tasación no.
     *
     * `soloPresentables` es «solo lo que se le puede enseñar a un cliente»
     * —con foto y con precio con sentido—; la provincia viene del test con
     * todas sus escrituras. Los comparables de una tasación no llevan ni lo
     * uno ni lo otro: un coche sin foto sigue siendo un precio válido.
     */
    soloPresentables = false,
    province = "",
    provinciaFormas = null,
    // De donde viene: lo pide quien no quiere papeleo de matriculacion.
    country = "",
    /*
     * Varios combustibles, porque la motorizacion se marca con casillas.
     * Ver formasDe en lo-que-busca-el-consejero.js.
     */
    fuels = null,
    // La familia de marcas que eligio en el test.
    brands = null,
  } = options;
  ...
  const { offers, source } = await readInventoryUniverse({
    ...
    color,
    /*
     * Estos cuatro se quedaban fuera y nadie se enteraba.
     *
     * Esta llamada enumera los campos uno a uno en vez de pasar 
     * entero, asi que todo lo que se anada arriba y no se escriba aqui se
     * pierde en silencio. Se veia en que buscar en Malaga devolvia ofertas de
     * Madrid: el criterio no llegaba a la consulta y la busqueda salia igual
     * de bien... pero de otra provincia.
     */
    brand,
    soloPresentables,
    province,
  ...
  const maxYearNumber = toNumber(maxYear);
  const minDoorsNumber = toNumber(minDoors);
  const maxDoorsNumber = toNumber(maxDoors);
  const minSeatsNumber = toNumber(minSeats);
  const maxSeatsNumber = toNumber(maxSeats);
  const minPowerCvNumber = toNumber(minPowerCv);
  const maxPowerCvNumber = toNumber(maxPowerCv);
  const minPriceNumber = toNumber(minPrice);
  const maxPriceNumber = toNumber(maxPrice);
  ...
  const filtered = offers
    ...
    .filter((offer) => {
      ...
      // §1g_combustible + §1g_combustible.2: "sin dato = pasa" base; para sujetos gasolina/diésel
      // se infiere combustible desde versión. Híbrido/enchufable/eléctrico excluidos de inferencia:
      // portales etiquetan 48V y PHEVs inconsistentemente; el modo de fallo sería exclusión silenciosa.
      const _offerFuelToken =
        normalizeToken(offer.fuel) ||
        ((normalizedFuel === 'gasolina' || normalizedFuel === 'diesel')
          ? inferFuelFromVersion(offer.version)
          : null);
      if (normalizedFuel && _offerFuelToken && !isFuelCompatible(_offerFuelToken, normalizedFuel)) {
        return false;
      }
      if (normalizedTransmission && normalizeToken(offer.transmission) && !normalizeToken(offer.transmission).includes(normalizedTransmission)) {
        return false;
      }
      if (normalizedVersion && !normalizeToken(offer.version).includes(normalizedVersion)) {
        return false;
      }
      if (normalizedCity && !ubicacionesDe(offer).includes(normalizedCity)) {
        return false;
      }
      if (normalizedLocation) {
        const locationHaystack = ubicacionesDe(offer);
      ...
      if (normalizedSellerType && normalizeToken(offer.sellerType) && !normalizeToken(offer.sellerType).includes(normalizedSellerType)) {
        return false;
      }
      ...
      if (Number.isFinite(minDisplacementNumber) && Number.isFinite(offerDisplacement) && offerDisplacement < minDisplacementNumber) {
        return false;
      }
      if (Number.isFinite(maxDisplacementNumber) && Number.isFinite(offerDisplacement) && offerDisplacement > maxDisplacementNumber) {
        return false;
      }
      const offerCo2 = toNumber(offer.co2);
      if (Number.isFinite(minCo2Number) && Number.isFinite(offerCo2) && offerCo2 < minCo2Number) {
        return false;
      }
      if (Number.isFinite(maxCo2Number) && Number.isFinite(offerCo2) && offerCo2 > maxCo2Number) {
        return false;
      }
      const offerPowerKw = toNumber(offer.powerKw);
      if (Number.isFinite(minPowerKwNumber) && Number.isFinite(offerPowerKw) && offerPowerKw < minPowerKwNumber) {
        return false;
      }
      if (Number.isFinite(maxPowerKwNumber) && Number.isFinite(offerPowerKw) && offerPowerKw > maxPowerKwNumber) {
        return false;
      }
      const offerConsumption = toNumber(offer.consumption);
      if (Number.isFinite(minConsumptionNumber) && Number.isFinite(offerConsumption) && offerConsumption < minConsumptionNumber) {
        return false;
      }
      if (Number.isFinite(maxConsumptionNumber) && Number.isFinite(offerConsumption) && offerConsumption > maxConsumptionNumber) {
        return false;
      }
      ...
      if (Number.isFinite(maxDoorsNumber) && Number.isFinite(offer.doors) && offer.doors > maxDoorsNumber) {
        return false;
      }
      if (Number.isFinite(minSeatsNumber) && Number.isFinite(offer.seats) && offer.seats < minSeatsNumber) {
        return false;
      }
      if (Number.isFinite(maxSeatsNumber) && Number.isFinite(offer.seats) && offer.seats > maxSeatsNumber) {
        return false;
      }
    ...
    .map((offer) => {
      ...
      if (normalizedFuel && isFuelCompatible(normalizeToken(offer.fuel), normalizedFuel)) {
        score += 2;
      }
      if (normalizedLocation) {
        if (ubicacionesDe(offer).includes(normalizedLocation)) {
          score += 1;
        }
      }
...
async function getMarketPriceSnapshot(options = {}) {
  ...
  /**
   * Las mismas ofertas, pero sin las de precio absurdo.
   *
   * El filtro estaba puesto solo sobre los precios, asi que protegia la cifra
   * grande del informe —la mediana— y dejaba crudo todo lo que va debajo: la
   * tabla de precio medio por portal, los anuncios parecidos que se le ensenan
   * al cliente, la tendencia y el ajuste por uso. En la base hay un Hyundai i20
   * de 2022 a 453.545 euros y un SEAT Leon a 247.990: uno solo de esos no mueve
   * una mediana, pero se lleva por delante una media y una recta de regresion.
   *
   * Y el informe se contradecia a si mismo: decia «187 comparables validos, 3
   * atipicos excluidos» y a la vuelta de la pagina promediaba los 190.
   *
   * La lista de sanos es la hermana de la de precios: los mismos coches, con
   * sus datos al lado en vez de sueltos. De aqui para abajo se usa esta.
   */
  const valla = tukeyFence(rawPrices);
  const sanos = computeOffers.filter((o) => {
    const p = o[priceField];
    return Number.isFinite(p) && p > 0 && p >= valla.lo && p <= valla.hi;
  });
  ...
  const { usageImpact, rawUsageImpact, medianKm: usageMedianKm, medianYear: usageMedianYr, usedDefault: usageUsedDefault, slopeKm, slopeYear } =
    computeUsageImpact(sanos, mileage, year, marketMedian, priceField, brand);

  // Price trend: compares entry prices of recent vs older listings
  const priceTrend = computePriceTrend(sanos, priceField);

  const daysValues = sanos
  ...
  const freshestOffer = sanos[0] || null;

  // % del pool con MÁS km que el sujeto. >60 → argumento de venta de km bajo.
  const kmAdvantagePct = mileage != null && sanos.length > 0
    ? Math.round(sanos.filter((o) => o.mileage > mileage).length / sanos.length * 100)
    : null;
  ...
  return {
    ...
    byPortal: aggregateByPortal(sanos, desiredType),
    priceHistogram: buildPriceHistogram(prices),
    samples: mileage != null
      ? [...sanos].sort((a, b) => {
          // Prioritise offers with matching fuel and transmission, then rank by km proximity.
          // fuel mismatch: +30k km penalty; transmission mismatch: +15k km penalty.
          const pen = (o) =>
            (fuel && o.fuel && o.fuel.toLowerCase() !== fuel.toLowerCase() ? 30000 : 0) +
            (transmission && o.transmission && o.transmission.toLowerCase() !== transmission.toLowerCase() ? 15000 : 0);
          return (Math.abs(a.mileage - mileage) + pen(a)) - (Math.abs(b.mileage - mileage) + pen(b));
        }).slice(0, 8)
      : sanos.slice(0, 8),
...
async function listMarketplaceVoOffers(options = {}) {
  ...
  try {
    ...
    if (color) {
      // "sin dato = pasa": ofertas sin color no se excluyen.
      // LIKE 'azul%' para que "Azul Marino" aparezca al filtrar "Azul".
      values.push(String(color).toLowerCase());
      conditions.push(`(o.color IS NULL OR o.color = '' OR lower(o.color) LIKE $${values.length} || '%')`);
    }
    if (fuel) {
      conditions.push(pgFuelCondition(fuel, values, `lower(COALESCE(o.fuel,''))`));
    }
    if (transmission) {
      values.push(String(transmission).toLowerCase());
      conditions.push(`(o.transmission IS NULL OR lower(o.transmission) = $${values.length})`);
    }
    ...
    const offers = (rowsResult.rows || []).map((row) => ({
      ...
      // El vendedor de un coche de garaje es un particular, y ahí dentro está su
      // correo: es de donde lo lee el aviso de visita. Fuera no sale — la API es
      // abierta y un dato personal en el JSON está expuesto aunque no se pinte.
      seller: normalizeText(row.seller).includes("@")
        ? `Particular ${MARCA.nombre}`
        : normalizeText(row.seller) || "Portal",
    ...
    // El motivo NO se tira.
    //
    // Este catch devolvía lista vacía en silencio, y aguas abajo eso acababa
    // siendo un `ok: true` con cero ofertas: para un visitante, "la base de
    // datos no responde" y "no hay coches" se veían exactamente igual. Ocurrió
    // de verdad — horas enteras tras rotar la contraseña de Neon, sin una sola
    // línea en los logs que lo dijera.
    //
    // Se registra con el motivo, y el motivo viaja en la respuesta para que
    // quien la consuma pueda distinguir "no hay" de "no lo sé".
    console.error("[inventoryStore] listMarketplaceVoOffers failed:", err?.message);
    return {
      offers: [],
      source: "postgres-marketplace-table-error",
      totalUniverse: 0,
      error: err?.message || "consulta fallida",
    };
  }
}
...
const MARKETPLACE_VO_OFFER_SELECT = `
  SELECT o.id, o.title, o.brand, o.model, o.price, o.year, o.mileage, o.location, o.color,
         ...
         o.image_urls, o.has_stock_management, o.sale_price,
         o.transmission, o.version, o.internal_location, o.seller_type,
         -- Las fotos del coche tal y como están ahora, si es un coche de garaje.
         -- Por qué, en lasFotosDelAnuncio.
         CASE WHEN o.id LIKE 'idcar-%' THEN (
           SELECT json_agg(f.file_url ORDER BY f.created_at, f.id)
             FROM moveadvisor_user_vehicle_files f
            WHERE f.vehicle_id = substring(o.id from 7)
 ...
 *
 * Al publicar se copiaba la lista de fotos al anuncio y ahí se quedaba. Todo lo
 * que pasara después —subir más, borrar una— no llegaba: el coche tenía nueve
 * fotos en el ERP y su ficha pública enseñaba tres, las del día de publicarlo.
 * Es el mismo fallo que el precio, que también se congelaba.
 *
 * Ahora, para un coche de garaje (`idcar-…`), las fotos se leen de donde viven.
 * Se ordenan por fecha de subida, que es el mismo orden que usa publicar y el
 * que enseña el ERP. Y si alguien eligió una principal a mano («Hacer
 * principal»), va delante mientras siga existiendo: es la única decisión sobre
 * las fotos que vive en el anuncio.
 *
 * Para las ofertas que no son de garaje —concesionarios, importación— no hay
 * fotos vivas y se usa la lista guardada, como siempre.
 */
function lasFotosDelAnuncio(row) {
  let vivas = row?.fotos_vivas;
  if (typeof vivas === "string") { try { vivas = JSON.parse(vivas); } catch { vivas = null; } }
  vivas = Array.isArray(vivas) ? vivas.map(normalizeText).filter(Boolean) : [];

  if (vivas.length) {
    const principal = normalizeText(row?.image_url);
    return principal && vivas.includes(principal)
      ? [principal, ...vivas.filter((u) => u !== principal)]
      : vivas;
  }

  try {
    const guardadas = JSON.parse(row?.image_urls || "[]");
    if (Array.isArray(guardadas) && guardadas.length) return guardadas.map(normalizeText).filter(Boolean);
  } catch { /* lista estropeada: se usa la principal suelta */ }
  return row?.image_url ? [normalizeText(row.image_url)] : [];
}

function mapMarketplaceVoRow(row) {
  if (!row) return null;
  const fotos = lasFotosDelAnuncio(row);
  return {
    id: normalizeText(row.id),
    title: normalizeText(row.title),
    brand: normalizeText(row.brand),
    ...
    renting48m: toNumber(row.renting_48m) || null,
    renting60m: toNumber(row.renting_60m) || null,
    // La rejilla de plazos por tramos de km. Sin ella el listado no puede
    // saber cuál es el precio más bajo que se puede contratar de verdad.
    rentingPricesJson: row.renting_prices_json || null,
    images: fotos,
    hasStockManagement: Boolean(row.has_stock_management),
    salePrice: row.sale_price != null ? Number(row.sale_price) : null,
...
async function getMarketplaceVoOfferById(offerId) {
  ...
  /**
   * Los coches publicados por un usuario llevan el prefijo `user_` y no viven
   * en la tabla de ofertas, sino en la del garaje.
   *
   * Sin esto, un enlace directo a la ficha de un coche de particular no
   * encontraba nada y acababa redirigiendo al listado general — compartir la
   * dirección de tu coche llevaba al comprador a «todos los coches», que es lo
   * contrario de lo que uno quiere al pasar un enlace.
   */
  const id = String(offerId);
  if (id.startsWith("user_")) {
    /*
     * Si el coche ya tiene su anuncio `idcar-`, es ése.
     *
     * Un mismo coche tenía dos fichas: la `user_`, montada al vuelo desde el
     * garaje, y la `idcar-`, la que publica el ERP. Lo que se corregía desde el
     * ERP —la cilindrada, el cambio— llegaba a la segunda y el enlace viejo
     * seguía enseñando la primera. Y las visitas se pedían contra un anuncio
     * distinto del que tiene las franjas del encargo.
     */
    const vehiculo = id.slice("user_".length);
    try {
      const publicado = await pool.query(
        `${MARKETPLACE_VO_OFFER_SELECT} WHERE o.id = $1 AND o.is_active = TRUE LIMIT 1`,
        [`idcar-${vehiculo}`]
      );
      if (publicado.rows?.length) return mapMarketplaceVoRow(publicado.rows[0]);
    } catch {
      // Sin la tabla de ofertas, sigue la ficha del garaje.
    }
    const propios = await listUserPublishedVehiclesForMarketplace({}).catch(() => []);
    return propios.find((o) => o.id === id) || null;
  }
...
async function getMarketplaceVoOfferByUrl(sourceUrl) {
  ...
  try {
    const result = await pool.query(
      `${MARKETPLACE_VO_OFFER_SELECT} WHERE o.source_url = $1 AND o.is_active = TRUE LIMIT 1`,
      [sourceUrl]
    );
    return mapMarketplaceVoRow(result.rows?.[0]);
  } catch {
...
/**
 * Un número entero escrito por una persona, en español.
 *
 * El garaje guarda estos campos como texto y tal cual los teclea el usuario, o
 * sea con el punto de los miles: «30.000». `Number("30.000")` devuelve **30**,
 * porque para JavaScript ese punto es el decimal — y así es como un T-Roc con
 * treinta mil kilómetros aparecía en el marketplace con treinta.
 *
 * Se queda solo con las cifras, que para kilómetros, año o potencia es lo
 * correcto: ninguno lleva decimales.
 */
function enteroEspanol(valor) {
  const digitos = String(valor ?? "").replace(/[^\d]/g, "");
  return digitos ? Number(digitos) : 0;
}

/**
 * Las fotos de un vehículo del garaje, limpias y sin repetir.
 *
 * `array_agg` devuelve `null` cuando no hay ninguna, y en la práctica se cuelan
 * duplicados —la misma imagen resubida— que en una galería se notan mucho.
 */
function fotosDeFila(row) {
  const brutas = Array.isArray(row?.photo_urls) ? row.photo_urls : [];
  const vistas = new Set();
  const salida = [];
  for (const url of brutas) {
    const limpia = normalizeText(url);
...
/*
 * Como se leen en la ficha lo que el garaje guarda en minúsculas y sin tildes.
 * Son los mismos valores del formulario del cliente y del ERP; uno que no esté
 * en la lista se enseña tal cual.
 */
const COMBUSTIBLES_LEIDOS = { gasolina: "Gasolina", diesel: "Diésel", hibrido: "Híbrido", electrico: "Eléctrico", gas: "Gas" };
const CAMBIOS_LEIDOS = { manual: "Manual", automatico: "Automático" };
function comoSeLeeElCombustible(v) {
  const t = normalizeText(v);
  return COMBUSTIBLES_LEIDOS[t.toLowerCase()] || t;
}
function comoSeLeeElCambio(v) {
  const t = normalizeText(v);
  return CAMBIOS_LEIDOS[t.toLowerCase()] || t;
}
...
async function listUserPublishedVehiclesForMarketplace(filters = {}) {
  ...
  try {
    const conditions = [
      ...
      /*
       * Fuera los que tienen anuncio `idcar-`, esté activo o no.
       *
       * Si está despublicado, porque se quitó a propósito. Si está activo,
       * porque ya sale en el listado por la tabla de ofertas: contarlo también
       * aquí era enseñar el mismo coche dos veces, con dos fichas que no decían
       * lo mismo.
       */
      `NOT EXISTS (
    ...
    if (filters.maxYear) {
      values.push(Number(filters.maxYear));
      conditions.push(`v.year <= $${values.length}`);
    }
    if (filters.minMileage) {
      values.push(Number(filters.minMileage));
    ...
    if (filters.transmission) {
      values.push(filters.transmission.toLowerCase());
      conditions.push(`lower(COALESCE(v.transmission_type,'')) = $${values.length}`);
    }
    if (filters.fuel) {
      conditions.push(pgFuelCondition(filters.fuel, values, `lower(v.fuel)`));
    }
    if (filters.color) {
      values.push(filters.color.toLowerCase());
      conditions.push(`(v.color IS NULL OR v.color = '' OR lower(v.color) LIKE $${values.length} || '%')`);
    }
    ...
    const sql = `
      SELECT
        ...
        -- Con to_jsonb: la columna la crea el ERP la primera vez que se corrige
        -- un coche, y nombrarla antes rompería el listado entero.
        to_jsonb(v)->>'displacement' AS displacement,
        v.vehicle_location, v.price, v.notes,
        v.plate, v.user_email, v.updated_at,
        vs.listing_url,
        -- Todas las fotos, no solo la primera: la ficha de un particular
        -- enseñaba una imagen suelta mientras las de concesionario tenían
        -- galería, y un anuncio con una foto parece un anuncio a medias.
        (SELECT array_agg(f.file_url ORDER BY f.created_at ASC, f.id ASC)
           FROM moveadvisor_user_vehicle_files f
          WHERE f.vehicle_id = v.id AND f.file_type = 'photo'
            AND f.file_url IS NOT NULL AND f.file_url != '') AS photo_urls
    ...
    return (result.rows || []).map((row) => ({
      ...
      model: normalizeText(row.model),
      version: normalizeText(row.version) || null,
      price: enteroEspanol(row.price),
      salePrice: null,
      year: enteroEspanol(row.year),
      mileage: enteroEspanol(row.mileage),
      location: normalizeText(row.vehicle_location),
      color: normalizeText(row.color),
      displacement: enteroEspanol(row.displacement) || 0,
      fuel: comoSeLeeElCombustible(row.fuel) || "Sin especificar",
      power: enteroEspanol(row.cv) ? `${enteroEspanol(row.cv)} CV` : "",
      seller: `Particular ${MARCA.nombre}`,
      hasGuaranteeSeal: false,
      portalScore: 60,
      warrantyMonths: 0,
      description: normalizeText(row.notes),
      image: fotosDeFila(row)[0] || "",
      images: fotosDeFila(row),
      url: normalizeText(row.listing_url),
      portal: `${MARCA.nombre} Particulares`,
      availableForPurchase: true,
      rentingAvailable: false,
      hasStockManagement: false,
      unitsAvailable: 1,
      availableColors: [],
      transmission: comoSeLeeElCambio(row.transmission_type) || null,
...
// Showcase "una de cada sección" para la ficha de detalle: 1 oferta de muestra
// por cada sección del ecosistema (VO CarsWise + importación + portales), con
// la metadata de navegación a su sección. Sólo devuelve las secciones con datos.
async function getMarketplaceShowcase() {
  const pool = getPostgresPool();
  if (!pool) return [];

  const firstImage = (imgUrl, imgsRaw) => {
    if (imgUrl) return imgUrl;
    try { const a = JSON.parse(imgsRaw || "[]"); return Array.isArray(a) && a.length ? a[0] : ""; }
    catch { return ""; }
  };
  const baseOffer = (r, image, price) => ({
    id: r.id,
    title: r.title || [r.brand, r.model].filter(Boolean).join(" ") || "Vehículo",
    image,
    year: r.year != null ? Number(r.year) : null,
    mileage: r.mileage != null ? Number(r.mileage) : null,
    price: price != null ? Number(price) : null,
  });
  const q = (sql) => pool.query(sql).then((r) => r.rows[0] || null).catch(() => null);
  const voCols = "id, title, brand, model, year, mileage, price, sale_price, image_url, image_urls";
  const mkCols = "id, title, brand, model, year, mileage, price, image_url, images";

  const [conce, part, rent, imp, flex, auto] = await Promise.all([
    q(`SELECT ${voCols} FROM moveadvisor_marketplace_vo_offers WHERE seller_type IN ('concesionario','importador') AND COALESCE(is_active,TRUE)=TRUE AND available_for_purchase IS NOT FALSE AND COALESCE(price,0)>0 AND COALESCE(image_url,'')<>'' ORDER BY portal_score DESC NULLS LAST LIMIT 1`),
    q(`SELECT ${voCols} FROM moveadvisor_marketplace_vo_offers WHERE seller_type='particular' AND COALESCE(is_active,TRUE)=TRUE AND COALESCE(price,0)>0 AND COALESCE(image_url,'')<>'' ORDER BY updated_at DESC LIMIT 1`),
    q(`SELECT ${voCols}, renting_12m, renting_24m, renting_36m, renting_48m, renting_60m FROM moveadvisor_marketplace_vo_offers WHERE renting_available=TRUE AND COALESCE(is_active,TRUE)=TRUE AND COALESCE(renting_36m, renting_48m, renting_60m, renting_24m, renting_12m) IS NOT NULL AND COALESCE(image_url,'')<>'' ORDER BY portal_score DESC NULLS LAST LIMIT 1`),
    q(`SELECT ${mkCols}, import_cost FROM moveadvisor_market_offers WHERE country='DE' AND import_published=TRUE AND price IS NOT NULL AND COALESCE(image_url,'')<>'' ORDER BY import_score DESC NULLS LAST LIMIT 1`),
    q(`SELECT ${mkCols} FROM moveadvisor_market_offers WHERE portal='flexicar' AND COALESCE(is_active,TRUE)=TRUE AND price IS NOT NULL AND COALESCE(image_url,'')<>'' LIMIT 1`),
    q(`SELECT ${mkCols} FROM moveadvisor_market_offers WHERE portal='autohero' AND COALESCE(is_active,TRUE)=TRUE AND price IS NOT NULL AND COALESCE(image_url,'')<>'' LIMIT 1`),
  ]);

  const sections = [
    conce && { key: "cw-concesionario", label: `Marketplace ${MARCA.nombre} · Concesionario`, nav: { type: "marketplace-vo", tab: "concesionarios" }, offer: baseOffer(conce, firstImage(conce.image_url, conce.image_urls), conce.sale_price ?? conce.price) },
    part && { key: "cw-particulares", label: `Marketplace ${MARCA.nombre} · Particulares`, nav: { type: "marketplace-vo", tab: "particulares" }, offer: baseOffer(part, firstImage(part.image_url, part.image_urls), part.sale_price ?? part.price) },
    rent && { key: "cw-renting", label: `Marketplace ${MARCA.nombre} · Renting`, nav: { type: "marketplace-vo", tab: "renting_empresa" }, offer: { ...baseOffer(rent, firstImage(rent.image_url, rent.image_urls), rent.renting_36m ?? rent.renting_48m ?? rent.renting_60m ?? rent.renting_24m ?? rent.renting_12m), monthly: true } },
    imp && { key: "cw-importacion", label: `Marketplace ${MARCA.nombre} · Importación`, nav: { type: "marketplace-vo", tab: "importacion" }, offer: baseOffer(imp, firstImage(imp.image_url, imp.images), Number(imp.price) + Number(imp.import_cost || 0)) },
    flex && { key: "portal-flexicar", label: "Ofertas Portales · Flexicar", nav: { type: "buy-known" }, offer: baseOffer(flex, firstImage(flex.image_url, flex.images), flex.price) },
    auto && { key: "portal-autohero", label: "Ofertas Portales · Autohero", nav: { type: "buy-known" }, offer: baseOffer(auto, firstImage(auto.image_url, auto.images), auto.price) },
  ].filter(Boolean);
...
module.exports = {
  lasFotosDelAnuncio,
  // Se exportan para poder probarlas: la valla de atipicos y la tabla por
  // portal son las dos piezas del descuadre que se arreglo en septiembre.
  tukeyFence,
  aggregateByPortal,
  listInventoryOffers,
  getMarketplaceShowcase,
