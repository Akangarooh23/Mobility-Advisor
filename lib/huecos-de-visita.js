...
/** Si dos ratos se solapan. Se tocan de canto no cuenta: 10-11 y 11-12 no. */
function seSolapan(a, b) {
  const ai = new Date(a?.starts_at).getTime();
  const af = new Date(a?.ends_at || ai + DURACION_MS).getTime();
  const bi = new Date(b?.starts_at).getTime();
  const bf = new Date(b?.ends_at || bi + DURACION_MS).getTime();
...
function sePisa(inicioIso, reservas) {
  const hora = {
    starts_at: inicioIso,
    ends_at: new Date(new Date(inicioIso).getTime() + DURACION_MS).toISOString(),
  };
  return (reservas || []).some((r) => {
    if (!["pending", "confirmed"].includes(String(r.status || ""))) return false;
    return seSolapan(hora, r);
  });
}

/**
 * Lo que queda de una franja al quitarle una hora.
 *
 * El vendedor ve su franja de 10:00 a 14:00 abierta en cuatro horas y quita
 * una: lo que queda son los trozos de antes y de despues, y solo valen los que
 * dan para una visita entera —media hora suelta no se le puede ofrecer a
 * nadie—. Si la hora no cae dentro, la franja se queda como estaba; si no
 * queda nada, el array vacio dice que la franja ya no tiene razon de ser.
 */
function loQueQuedaAlQuitar(franja, horaIso) {
  const inicio = new Date(franja?.starts_at).getTime();
  const fin = new Date(franja?.ends_at).getTime();
  const quitaDe = new Date(horaIso).getTime();
  if (![inicio, fin, quitaDe].every(Number.isFinite)) return [];
  const quitaHasta = quitaDe + DURACION_MS;
  const rato = (a, b) => ({ starts_at: new Date(a).toISOString(), ends_at: new Date(b).toISOString() });

  if (quitaHasta <= inicio || quitaDe >= fin) return [rato(inicio, fin)];

  const trozos = [];
  if (quitaDe - inicio >= DURACION_MS) trozos.push(rato(inicio, quitaDe));
  if (fin - quitaHasta >= DURACION_MS) trozos.push(rato(quitaHasta, fin));
  return trozos;
}
