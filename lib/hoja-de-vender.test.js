...
describe("que coche es lo dice quien lo sabe", () => {
   ...
   */
  test("si ha entrado, elige de una lista", () => {
    assert.match(FORM, /haySesion && misCoches && misCoches\.length > 0 \?/);
  ...
  test("si ha entrado y no tiene ninguno, se le manda a darlo de alta", () => {
    /*
     * No se le puede pedir despues: el coche lo sube el, con sus fotos y sus
     * papeles. Enseñarle un formulario que no puede rellenar y averiguarlo en
     * la llamada es aplazar el problema.
     */
    assert.match(FORM, /Primero da de alta tu coche/);
    assert.match(FORM, /href="\/panel\/vehiculos"/);
  });

  test("y si NO ha entrado, texto libre y sin registrarse", () => {
    /*
     * Obligar a registrarse para pedir presupuesto es el mismo error que el
     * candado de las visitas: se pierde a la mayoria en la puerta. De esos,
     * cual es el coche se averigua en la llamada.
     */
    const sinSesion = FORM.slice(FORM.indexOf("haySesion && misCoches && misCoches.length > 0"));
    assert.match(sinSesion, /<input\s*\n?\s*id="fev-coche"/);
    assert.ok(
