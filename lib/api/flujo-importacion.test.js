...
/**
 * Lo que deposita: el coche y nuestro fee.
 *
 * No el impuesto de matriculación, que es de Hacienda y se liquida al
 * matricular, y no un porcentaje: el coche entero, porque el coche se lo va a
 * comprar él al vendedor alemán y ese dinero tiene que estar.
 */
const { FEE_POPCAR } = require("../coste-importacion.js");
const DEPOSITO = OFERTA.price + FEE_POPCAR;
...
describe("pedir un coche de importación", () => {
  ...
  test("el impuesto de matriculación no se deposita", async () => {
    // Es de Hacienda, se liquida al matricular y su importe exacto no se sabe
    // hasta entonces. Cobrarlo ahora sería cobrar una estimación y tener que
    // devolver la diferencia.
    const r = await pide();
    const conImpuesto = precioPuestoAqui(OFERTA.price, OFERTA.market_price_es);
    assert.ok(r.cuerpo.deposit < conImpuesto, "el depósito lleva el impuesto dentro");
  });
