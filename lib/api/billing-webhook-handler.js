...
const { uploadBufferToSupabase, BUCKET_PRIVADO } = require("../supabaseStorage");
const { plantilla, parrafo, datos, esc, textoPlano } = require("../correo");
...
function getWebhookPgPool() {
  ...
  _webhookPgPool = new Pool({ connectionString: connString, ssl: SSL_POSTGRES });
...
// Las series y el contador viven en un sitio: lib/series-de-factura.js.
const { SERIES, siguienteNumeroDeFactura } = require("../series-de-factura.js");
...
async function depositoRecibido({ leadId, email, importe, fee, sessionId, cobroRef }) {
    ...
    const r = await pool.query(
      `UPDATE moveadvisor_market_leads
          SET deposit_paid_at = NOW(),
              ...
              -- El estado del dinero, que es lo que mira el ERP para dejar
              -- soltarlo. Faltaba: se quedaba en «pendiente» con el dinero ya
              -- dentro, y «Liberar el pago al vendedor» contestaba que el
              -- cliente todavia no habia pagado.
              --
              -- Va con la misma condicion que el resto —solo si no estaba
              -- marcada— para no pisar un «devuelto» o un «liberado», que son
              -- estados finales: el dinero ya se movio.

              escrow_estado = 'retenido',
  ...
  /**
   * Los suplidos: dinero que pagamos en nombre del cliente.
   *
   * El coche es del concesionario alemán, el impuesto es de Hacienda y la
   * garantía es de la aseguradora. Los tres pasan por nuestra cuenta y
   * **ninguno es ingreso nuestro**, así que van fuera de la base de IVA y
   * aparte en la factura.
   *
   * Se calculan **antes** de emitirla y no después: el PDF que se le manda por
   * correo se genera con este objeto, así que unos suplidos escritos luego solo
   * salían en el PDF del panel. El cliente recibía una factura de 3.000 € al
   * lado de una transferencia de 21.500 sin nada que explicara la diferencia,
   * que es justo lo que este bloque existe para evitar.
   *
   * Y la garantía va con ellos. Faltaba, así que la cuenta se quedaba 190 €
   * corta: 16.890 + 1.420 + 3.000 son 21.310, no 21.500.
   */
  const suplidos = [];
  try {
    const r = await pool.query(
      `SELECT escrow_coche, escrow_impuesto, escrow_garantia
         FROM moveadvisor_market_leads WHERE id = $1`,
      [leadId]
    );
    const f = r.rows[0] || {};
    const cocheEur = Math.round(Number(f.escrow_coche) || 0);
    const imp = Math.round(Number(f.escrow_impuesto) || 0);
    const gar = Math.round(Number(f.escrow_garantia) || 0);
    if (cocheEur) suplidos.push({ concepto: "Precio del coche (vendedor en Alemania)", importe: cocheEur });
    if (imp) suplidos.push({ concepto: "Impuesto de matriculación (a cuenta)", importe: imp });
    if (gar) suplidos.push({ concepto: "Garantía mecánica (aseguradora)", importe: gar });
  } catch (e) {
    console.error("[deposito] no se han podido leer los suplidos:", e.message);
  }

  try {
    const numero = await siguienteNumeroDeFactura(pool, SERIES.importacion);
    const factura = {
      id: `srv-${leadId}`,
      number: numero,
    ...
    // El identificador de la fila lo pone la base: `factura.id` es la
    // referencia de fuera —la sesión de Stripe— y ya no es la clave.
    const idDeLaFila = await upsertInvoiceToPostgres(email, factura);
    await pool.query(
      `UPDATE moveadvisor_user_invoices SET cw_invoice_number = $2 WHERE id = $1`,
      [idDeLaFila, numero]
    ).catch(() => {});

    // Se guardan también en su fila: el PDF que se genera al vuelo desde el
    // panel los lee de aquí, no del objeto de arriba.
    await pool.query(`
      ALTER TABLE moveadvisor_user_invoices
        ADD COLUMN IF NOT EXISTS suplidos JSONB
    `).catch(() => {});
    if (suplidos.length) {
      await pool.query(
        `UPDATE moveadvisor_user_invoices SET suplidos = $2::jsonb WHERE id = $1`,
        [idDeLaFila, JSON.stringify(suplidos)]
      ).catch((e) => console.error("[deposito] suplidos:", e.message));
    }
    ...
    try {
      ...
      await markInvoiceGenerated(idDeLaFila);
      await uploadInvoicePdfAndSaveUrl(idDeLaFila, pdf, factura.number);
      /**
       * Y el justificante del depósito, con lo que ha pagado de verdad.
       *
       * La factura son 3.000 € y él ha transferido 21.500. Aunque el bloque de
       * suplidos lo explique, ese papel dice «TOTAL 3.000 €» arriba del todo, y
       * no es un recibo de lo que ha pagado.
       *
       * Este segundo documento **no es una factura y lo dice**: es el resguardo
       * de lo recibido, con a quién va cada parte y qué falta por justificar.
       * Los suplidos se justifican con los papeles del tercero —la factura del
       * concesionario alemán, el impuesto pagado, la póliza—, y hasta que
       * lleguen esto es lo único que tiene el cliente en la mano.
       */
      let justificante = null;
      try {
        justificante = await generateDepositoPdf({
          numeroFactura: factura.number,
          fecha: factura.date,
          coche,
          email,
          fee: importeFacturado,
          suplidos,
          cobroRef,
        }, perfil);
      } catch (e) {
        // Sin justificante se manda la factura igual: es peor no mandar nada.
        console.error("[deposito] justificante no generado:", e.message);
      }
      await sendInvoiceEmail({ to: email, pdfBuffer: pdf, invoiceRecord: factura, depositoBuffer: justificante });
      await markInvoiceSent(idDeLaFila);
    } catch (e) {
...
/**
 * Guarda la factura, y devuelve **nuestro** identificador de la fila.
 *
 * Antes la fila se llamaba como la llamaba Stripe: `id` era la sesión de pago
 * (`cs_test_a1rey…`). Con eso, nuestra factura llevaba el nombre que le puso
 * otro, y el día que se cambie de pasarela —o que Stripe cambie de formato— las
 * facturas viejas quedan nombradas por un proveedor que ya no se usa.
 *
 * Ahora la clave la pone la base y lo de Stripe va donde le toca:
 * `referencia_externa`, que sigue siendo única porque es lo que evita emitir
 * dos veces la misma factura cuando un webhook llega repetido —y llega—.
 *
 * Devuelve el identificador de la fila para que quien llama pueda seguir
 * tocándola. Antes usaba `invoice.id` para eso, que ya no es la clave.
 */
  ...
  try {
    const { rows } = await pool.query(
      `INSERT INTO moveadvisor_user_invoices
         (referencia_externa, email, number, date, amount, status, pdf_url, description, cw_paid_at)
...
async function uploadInvoicePdfAndSaveUrl(invoiceId, pdfBuffer, invoiceNumber) {
  try {
    ...
    /*
     * Al cajón privado, no al de las fotos.
     *
     * Una factura lleva nombre, NIF, dirección y a veces matrícula. El trozo al
     * azar del nombre evitaba que se pudieran pedir todas seguidas, pero el
     * fichero seguía siendo público para quien tuviera la dirección — y la
     * dirección se guarda en la base y viaja en las respuestas.
     *
     * Nadie la lee a pelo: el PDF se sirve por `invoice-pdf`, que comprueba la
     * sesión y lo trae con la clave de servicio. Así que lo que se guarda es la
     * dirección privada.
     */
    const publicUrl = await uploadBufferToSupabase(
      pdfBuffer, "application/pdf", path, BUCKET_PRIVADO
    );
    const pool = getWebhookPgPool();
    if (pool && publicUrl) {
      await pool.query(
...
async function generateInvoicePdf(invoiceRecord, userProfile = {}) {
  ...
  // Alineado por la derecha. Una columna de importes con distinto número de
  // dígitos no cuadra de otra forma, y pegado a una etiqueta se le monta encima.
  function dtr(str, right, y, size, font, color) {
    const t = String(str || "");
    dt(t, right - font.widthOfTextAtSize(t, size), y, size, font, color);
  }
  ...
  /**
   * Los totales a la derecha, y los suplidos **debajo del cuadro**.
   *
   * Estaban los dos en la misma banda: el bloque de suplidos empezaba a la
   * misma altura que el recuadro amarillo del total, así que «21500.00 EUR» se
   * imprimía encima de «16890.00 EUR» y no se leía ninguno de los dos.
   *
   * Y las cifras se alinean por la derecha, no por la izquierda: una columna de
   * importes con distinto número de dígitos no cuadra de otra forma.
   */
  const totY = tY + 64;
  dt("Base imponible:", 370, totY, 9, ri, c(COLOR.textoSuave));
  dtr(fmtEur(base), 545, totY, 9, ri, c(COLOR.textoSuave));
  dt("IVA (21%):", 370, totY + 15, 9, ri, c(COLOR.textoSuave));
  dtr(fmtEur(iva), 545, totY + 15, 9, ri, c(COLOR.textoSuave));

  /**
   * Los suplidos, si los hay.
   *
   * Dinero pagado en nombre del cliente: el coche, que es del concesionario
   * alemán; el impuesto, que es de Hacienda; y la garantía, que es de la
   * aseguradora. Ninguno es ingreso nuestro, así que **van fuera de la base de
   * IVA** y aparte.
   *
   * Esto ya estaba en el PDF que se genera al vuelo desde el panel, pero no en
   * el que se manda por correo, que es el que el cliente abre de verdad: le
   * llegaba una factura de 3.000 € al lado de una transferencia de 21.500 sin
   * nada que explicara la diferencia. Dos generadores de PDF distintos es lo
   * que hay hoy; que digan cosas distintas, no puede ser.
   */
  const suplidos = Array.isArray(invoiceRecord.suplidos) ? invoiceRecord.suplidos : [];
  const sumaSuplidos = suplidos.reduce((t, x) => t + (Number(x && x.importe) || 0), 0);

  fr(330, totY + 30, 215, 22, c(COLOR.amarillo));
  dt(sumaSuplidos ? "TOTAL FACTURA:" : "TOTAL:", 340, totY + 37, 11, rb, c(COLOR.negro));
  dtr(fmtEur(amount), 535, totY + 37, 11, rb, c(COLOR.negro));

  let sy = totY + 74;
  if (sumaSuplidos) {
    dt("SUPLIDOS", 50, sy, 9, rb, c(COLOR.textoTenue));
    dt("Pagados en tu nombre. No son ingreso de PopCar y van fuera de la base imponible.",
       50, sy + 12, 8, ri, c(COLOR.textoSuave));
    sy += 30;
    for (const su of suplidos) {
      const importe = Number(su && su.importe) || 0;
      dt(String((su && su.concepto) || ""), 50, sy, 9, ri, c("#111111"));
      dtr(fmtEur(importe), 545, sy, 9, ri, c("#111111"));
      sy += 15;
    }
    ln(50, sy + 2, 545, sy + 2);
    sy += 12;
    // Y lo que ha transferido de verdad. Sin esta línea, la factura y el
    // ingreso son dos cifras distintas y ningún papel dice por qué.
    dt("Total pagado (factura + suplidos):", 50, sy, 9, rb, c("#111111"));
    dtr(fmtEur(amount + sumaSuplidos), 545, sy, 9, rb, c("#111111"));
    sy += 22;
  }

  // Pagada stamp
  dt("PAGADA", 50, Math.max(totY + 78, sy), 11, rb, c(COLOR.negro));
...
/**
 * Adónde va cada euro de lo que ha depositado.
 *
 * Nuestro servicio va primero y con su número de factura: es lo único de aquí
 * que es ingreso nuestro, y lo único que se factura. Lo demás son suplidos —el
 * coche, el impuesto y la garantía— y cada uno lleva de quién es.
 *
 * Está aparte del PDF a propósito. Lo que hay que poder comprobar es que la
 * suma cuadra con lo que transfirió y que ningún euro se queda sin dueño; que
 * eso se dibuje bien en una página es otra cosa.
 */
function repartoDelDeposito(datosDep = {}) {
  const fee = Math.round(Number(datosDep.fee) || 0);
  const suplidos = Array.isArray(datosDep.suplidos) ? datosDep.suplidos : [];
  const lineas = [];
  if (fee) {
    lineas.push({
      concepto: `Servicio ${MARCA.nombre}` + (datosDep.numeroFactura ? ` (factura ${datosDep.numeroFactura})` : ""),
      aQuien: MARCA.nombre,
      importe: fee,
      facturado: true,
    });
  }
  for (const x of suplidos) {
    const concepto = String((x && x.concepto) || "").trim();
    const importe = Math.round(Number(x && x.importe) || 0);
    if (!concepto || !importe) continue;
    lineas.push({ concepto, aQuien: aQuienVa(concepto), importe, facturado: false });
  }
  return { lineas, total: lineas.reduce((t, l) => t + l.importe, 0) };
}
/**
 * El justificante del depósito. **No es una factura, y lo dice.**
 *
 * La factura que se le manda son 3.000 € y él ha transferido 21.500. Aunque el
 * bloque de suplidos lo explique, ese papel lleva «TOTAL 3.000 €» arriba: no
 * sirve como resguardo de lo que ha pagado.
 *
 * Esto sí. Dice lo recibido, a quién va cada parte y qué falta por justificar,
 * porque un suplido se justifica con el papel del tercero —la factura del
 * concesionario alemán, el impuesto pagado, la póliza— y esos llegan después.
 * Hasta entonces, esto es lo único que el cliente tiene en la mano por 18.500 €.
 *
 * Y no dice «retenido en una cuenta de depósito». Ese dinero está en la cuenta
 * de PopCar mientras no haya escrow de verdad, así que lo que se promete es lo
 * que se puede cumplir: **que no se le paga al vendedor hasta verlo**.
 */
async function generateDepositoPdf(datosDep, userProfile = {}) {
  const { PDFDocument, rgb, StandardFonts } = require("pdf-lib");
  const doc = await PDFDocument.create();
  const rb = await doc.embedFont(StandardFonts.HelveticaBold);
  const ri = await doc.embedFont(StandardFonts.Helvetica);
  ...
  const reparto = repartoDelDeposito(datosDep);
  const total = reparto.total;
  const fmtEur = (n) => Number(n).toFixed(2) + " EUR";
  const fecha = datosDep.fecha ? new Date(datosDep.fecha).toLocaleDateString("es-ES") : "";
  const cliente = [userProfile.name, userProfile.apellidos].filter(Boolean).join(" ") || datosDep.email || "";

  const rightEdge = W - 50;
  const rtx = (str, size, font) => rightEdge - font.widthOfTextAtSize(String(str || ""), size);

  // Cabecera
  fr(50, 50, 6, 26, c(COLOR.amarillo));
  dt(MARCA.nombre, 66, 50, 22, rb, c(COLOR.negro));
  dt([MARCA.sitio, correoSoporte()].filter(Boolean).join(" \xb7 "), 66, 76, 8.5, ri, c(COLOR.textoSuave));
  const tit = "JUSTIFICANTE DE PAGO";
  dt(tit, rtx(tit, 15, rb), 52, 15, rb, c("#111111"));
  const noEs = "Este documento no es una factura";
  dt(noEs, rtx(noEs, 8.5, ri), 74, 8.5, ri, c(COLOR.textoSuave));
  const fLabel = `Fecha: ${fecha}`;
  dt(fLabel, rtx(fLabel, 9.5, ri), 90, 9.5, ri, c(COLOR.textoSuave));
  ln(50, 115, 545, 115);

  // Quién y qué coche
  dt("RECIBIDO DE", 50, 130, 8.5, rb, c("#96968F"));
  dt(cliente, 50, 145, 11, rb, c("#111111"));
  let ly = 162;
  [
    datosDep.email,
    userProfile.tax_id ? `NIF/CIF: ${userProfile.tax_id}` : "",
    datosDep.coche ? `Vehículo: ${datosDep.coche}` : "",
    datosDep.cobroRef ? `Referencia del cobro: ${datosDep.cobroRef}` : "",
  ].filter(Boolean).forEach((l) => { dt(l, 50, ly, 9, ri, c(COLOR.textoSuave)); ly += 13; });

  // El reparto: a quién va cada parte
  const tY = 245;
  fr(50, tY, 495, 24, c("#F7F7F3"));
  dt("CONCEPTO", 60, tY + 7, 8.5, rb, c(COLOR.textoSuave));
  dt("A QUIEN", 300, tY + 7, 8.5, rb, c(COLOR.textoSuave));
  dtr("IMPORTE", 535, tY + 7, 8.5, rb, c(COLOR.textoSuave));
  ln(50, tY + 24, 545, tY + 24);

  let y = tY + 34;
  for (const l of reparto.lineas) {
    dt(l.concepto.slice(0, 46), 60, y, 9.5, ri, c("#111111"));
    dt(l.aQuien, 300, y, 9, ri, c(COLOR.textoSuave));
    dtr(fmtEur(l.importe), 535, y, 9.5, ri, c("#111111"));
    y += 16;
  }
  ln(50, y + 4, 545, y + 4);

  // El cuadro empieza en 330 y no en 370: «TOTAL RECIBIDO:» a 11 puntos en
  // negrita no cabía con la cifra al lado y se le montaba encima.
  fr(330, y + 12, 215, 22, c(COLOR.amarillo));
  dt("TOTAL RECIBIDO:", 340, y + 19, 11, rb, c(COLOR.negro));
  dtr(fmtEur(total), 535, y + 19, 11, rb, c(COLOR.negro));

  // Lo que hace que 18.500 € en manos de otro sean aceptables: cuando se
  // sueltan y que vuelven si el coche no es el que se anunció.
  let ny = y + 56;
  dt("QUÉ PASA CON CADA PARTE", 50, ny, 8.5, rb, c("#96968F"));
  ny += 16;
  const notas = [
    "El precio del coche no se le paga al vendedor hasta que uno de los nuestros lo ve",
    "en Alemania y confirma que es el que se anunció. Si no lo es, se te devuelve entero.",
    "",
    "El impuesto de matriculación va a cuenta: se liquida al matricular. Si sale menos,",
    "se te devuelve la diferencia; si sale más, se te cobra.",
    "",
    "El coche, el impuesto y la garantía son suplidos: dinero pagado en tu nombre, que no",
    "es ingreso de " + MARCA.nombre + " y va fuera de la base de IVA. Te entregamos el justificante",
    "de cada uno —factura del vendedor, impuesto pagado y póliza— según se vayan teniendo.",
  ];
  for (const n of notas) {
    if (n) dt(n, 50, ny, 9, ri, c(COLOR.textoSuave));
    ny += 13;
  }

  ln(50, 750, 545, 750);
  dt(`${MARCA.razonSocial} \xb7 Justificante de pago. No sustituye a la factura del servicio.`,
     50, 758, 8, ri, c(COLOR.textoTenue));
...
/**
 * A quien va cada suplido.
 *
 * Sale del concepto y no de un campo aparte porque el concepto lo escribe el
 * mismo sitio que arma los suplidos, tres lineas mas arriba. Si un dia son mas
 * de tres, esto se convierte en un campo.
 */
function aQuienVa(concepto) {
  if (/coche|vendedor/i.test(concepto)) return "Vendedor en Alemania";
  if (/impuesto|matricul/i.test(concepto)) return "Hacienda (a cuenta)";
  if (/garant|asegurad/i.test(concepto)) return "Aseguradora";
  
  return "-";
}
async function sendInvoiceEmail({ to, pdfBuffer, invoiceRecord, depositoBuffer = null }) {
  ...
  // Con justificante, el correo va de lo que ha pagado —que es lo que le
  // importa— y la factura es una de las dos cosas que lleva dentro.
  const html = plantilla({
    titulo: depositoBuffer ? "Tu factura y el justificante del pago" : "Tu factura está lista",
    cuerpo:
      parrafo(depositoBuffer
        ? "Van dos documentos adjuntos: la factura de nuestro servicio y el justificante de todo lo que has pagado, con a quién va cada parte."
        : "Va adjunta en PDF.") +
      datos([
  ...
  const payload = {
    ...
    attachments: [
      ...
      ...(depositoBuffer ? [{
        filename: `Justificante_deposito_${invoiceNum}.pdf`,
        content: depositoBuffer.toString("base64"),
      }] : []),
    ],
  };
...
// Los dos correos de la tasacion viven en lib/tasacion-correo.js: los usa
// tambien el camino gratuito, que no pasa por aqui.
const { sendValuationEmail, sendFleetEmail } = require("../tasacion-correo");
const { registrarEntregada } = require("../tasacion");
...
module.exports = async function billingWebhookHandler(req, res) {
  ...
  if (TERMINO_LA_PANTALLA || LLEGO_EL_DINERO) {
    ...
    if (planId === "valuation" && email) {
      ...
      // Deja constancia, que es lo que cuenta las que lleva este cliente. Sin
      // esto, quien paga su segunda tasacion seguiria teniendo la gratuita
      // disponible para siempre: el contador solo veria las gratuitas.
      await registrarEntregada({ email, vehicle, reportData, pagada: true });
      try {
        const sessionId = normalizeText(eventData?.id);
        const amountEur = (eventData?.amount_total || 1000) / 100;
        /*
         * Su serie, y correlativa.
         *
         * Antes el número era `CW-2026-` más los seis últimos caracteres del
         * identificador de la sesión de Stripe. Eso parece un número de factura
         * y no lo es: no es correlativo, tiene huecos por definición y dos
         * sesiones pueden acabar igual. Una serie de facturación tiene que ir
         * de uno en uno dentro de su año, y eso lo da el contador.
         */
        const pool = getWebhookPgPool();
        const invoiceNum = pool
          ? await siguienteNumeroDeFactura(pool, SERIES.tasacion)
          : `${SERIES.tasacion}-${new Date().getFullYear()}-SIN-BASE`;
        const invoiceRecord = {
        ...
        const idDeLaFila = await upsertInvoiceToPostgres(email, invoiceRecord);
        appendOrUpdateInvoice(email, invoiceRecord);
        try {
          const userProfile = await getUserProfile(email);
          const invoicePdf = await generateInvoicePdf(invoiceRecord, userProfile);
          await markInvoiceGenerated(idDeLaFila);
          await uploadInvoicePdfAndSaveUrl(idDeLaFila, invoicePdf, invoiceRecord.number);
    ...
    if (planId === "valuation_fleet" && email) {
      ...
      // Una fila por coche tasado: el contador cuenta tasaciones, no compras.
      // Quien tase cinco de golpe gasta su gratuita ahi, no le queda otra luego.
      for (let i = 0; i < fullVehicles.length; i++) {
        await registrarEntregada({
          email,
          vehicle: fullVehicles[i],
          reportData: reports[i] ? reports[i].reportData : {},
          pagada: true,
        });
      }
      try {
        ...
        // La misma serie que un informe suelto: es el mismo servicio, vendido
        // de varios en varios.
        const pool = getWebhookPgPool();
        const invoiceNum = pool
          ? await siguienteNumeroDeFactura(pool, SERIES.tasacion)
  ...
  if (eventType === "customer.subscription.updated" || eventType === "customer.subscription.created" || eventType === "customer.subscription.deleted") {
    ...
    try {
      email =
        (await getEmailByStripeCustomerId(customerId)) ||
        (await getEmailByStripeSubscriptionId(subscriptionId));
    } catch (e) {
      /*
       * No poder mirar no es lo mismo que no ser nuestro. Con un 200 Stripe da
       * el evento por entregado y no vuelve: una baja se perderia para
       * siempre. Con el 500 lo reintenta durante tres dias.
       */
      console.error("[webhook] no se ha podido resolver la cuenta:", e.message);
      return res.status(500).json({ error: "No se ha podido resolver la cuenta" });
    }

    if (!email) {
      console.log(`[webhook] ${eventType} de un cliente que no es nuestro: ${customerId || subscriptionId}`);
    }
  ...
  if (eventType === "invoice.paid" || eventType === "invoice.payment_failed") {
    ...
    let email = normalizeText(eventData?.customer_email).toLowerCase();
    if (!email) {
      try {
        email = await getEmailByStripeCustomerId(customerId);
      } catch (e) {
        // Una factura que no se apunta es dinero cobrado sin constancia.
        console.error("[webhook] no se ha podido resolver la cuenta de la factura:", e.message);
        return res.status(500).json({ error: "No se ha podido resolver la cuenta" });
      }
    }
...
// Se exportan para poder probarlas: son las dos cosas del justificante que, mal
// hechas, dejan euros del cliente sin dueño en el papel.
module.exports.generateDepositoPdf = generateDepositoPdf;
module.exports.repartoDelDeposito = repartoDelDeposito;
module.exports.aQuienVa = aQuienVa;
