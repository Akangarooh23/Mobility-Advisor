...
const { lasMarcasQueQuiere } = require("./las-marcas-que-quiere");
const {
...
/**
 * Cuántos años como máximo, en años.
 *
 * ## El fallo que esto arregla, que llevaba ahí desde siempre
 *
 * La búsqueda hacía esto con la respuesta:
 *
 *     targetYear: answers?.antiguedad_vehiculo_buscada?.[0]
 *
 * Y la respuesta es una **cadena**, «5_anos», no una lista. Así que `[0]` es
 * el carácter `"5"`, que se metía en la consulta **como si fuera un año**: la
 * búsqueda pedía coches matriculados alrededor del año 5. Medido con un perfil
 * real: con ese carácter, 0 ofertas; sin él, 3.
 *
 * Y no fallaba. Devolvía cero, se disparaban las búsquedas de emergencia y
 * acababan saliendo SUV donde se había pedido un compacto.
 *
 * Le pasaba a cualquiera que contestara la antigüedad, que es casi todo el
 * mundo.
 */
const LOS_ANOS = {
  cero_anos: 0,
...
function elEncargoDeBusqueda(answers = {}) {
  ...
  /*
   * La carrocería y los combustibles, para quien quiera los mismos criterios.
   *
   * Los usa el análisis para preguntarle a la base qué modelos hay de verdad
   * para este cliente, en vez de sacarlos de una tabla escrita a mano. Los
   * valores del test —«compacto», «hibrido_no_enchufable»— son ya las claves
   * de las tablas de formas, así que no hay que traducirlos.
   */
  const carroceria = texto(answers.carroceria_preferida);
  if (carroceria && carroceria !== "indiferente_carroceria") encargo.bodyType = carroceria;

  const marcas = lasMarcasQueQuiere(answers);
  if (marcas) encargo.brands = marcas;

  /*
   * La antigüedad se convierte en un año mínimo, que es lo que entiende la
   * consulta. «5 años» hoy son los matriculados de 2021 en adelante.
   */
  const anos = LOS_ANOS[texto(answers.antiguedad_vehiculo_buscada)];
  if (Number.isFinite(anos)) encargo.minYear = new Date().getFullYear() - anos;

  const motores = (Array.isArray(answers.propulsion_preferida)
    ? answers.propulsion_preferida
    : answers.propulsion_preferida ? [answers.propulsion_preferida] : [])
    .map(texto)
    .filter((v) => v && v !== "indiferente_motor");
  if (motores.length) encargo.fuels = motores;
