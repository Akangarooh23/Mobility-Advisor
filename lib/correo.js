...
/**
 * Varias opciones, una debajo de otra, cuando ninguna es «la buena».
 *
 * `botones` pinta la principal en amarillo y la otra apagada, y eso vale para
 * confirmar o rechazar. Aqui no: si se le pregunta al cliente que tal fue, las
 * tres respuestas valen lo mismo y pintar una de amarillo es empujarle a ella.
 *
 * En columna y no en fila porque en un movil tres botones al lado se parten
 * mal, y estos se leen: la diferencia entre «no pude ir» y «me lo quede» esta
 * en el texto, no en el color.
 */
function opciones(lista) {
  const filas = lista.map(({ texto, url }) => `
     <tr><td style="border:1px solid ${COLOR.linea};border-radius:8px;padding:0">
       <a href="${urlSegura(url)}" style="display:block;padding:13px 20px;font-family:${TIPO};font-size:15px;font-weight:600;color:${COLOR.negro};text-decoration:none">${texto}</a>
     </td></tr>
     <tr><td style="height:8px"></td></tr>`).join("");
  return `<table role="presentation" cellpadding="0" cellspacing="0" border="0" style="margin:0 0 12px 0;width:100%">${filas}</table>`;
}
...
/**
 * La versión en texto del correo, sacada de su propio HTML.
 *
 * Los envíos de la aplicación mandaban **solo HTML**. Un correo sin parte de
 * texto es una de las señales que los filtros miran para separar lo que escribe
 * una persona de lo que escribe una máquina. No es la que más pesa —eso es la
 * autenticación del dominio, que se arregla en el DNS y no aquí— pero es gratis
 * y va en la dirección correcta.
 *
 * Y sirve para algo más terrenal: hay quien lee el correo con el HTML
 * desactivado, y en un reloj o en un lector de pantalla esto es lo único que se
 * oye.
 *
 * Se saca del HTML y no se escribe aparte a propósito. Dos textos que hay que
 * mantener a mano acaban diciendo cosas distintas, y el que se queda viejo es
 * justo éste, el que nadie mira.
 */
function textoPlano(html) {
  const conEnlaces = String(html ?? '').replace(
    // Un enlace se lee «texto: dirección», que es como se puede seguir sin pulsar.
    /<a\b[^>]*href="([^"]*)"[^>]*>([\s\S]*?)<\/a>/gi,
    (_m, url, dentro) => {
  ...
  return conEnlaces
    ...
    // La cabecera del documento no es el correo: su `<title>` repetía el titular.
    .replace(/<head[\s\S]*?<\/head>/gi, '')
    /*
     * Las etiquetas de dentro de una frase se quitan **sin dejar hueco**.
     *
     * Cambiándolas por un espacio, «<strong>T-Roc</strong>.» se leía «T-Roc .»
     * y el logotipo —que son dos `span`, uno por color— salía como «Pop Car».
     */
    .replace(/<\/?(span|strong|b|i|em|u|small|font)\b[^>]*>/gi, '')
...
module.exports = { plantilla, parrafo, datos, aviso, boton, botones, opciones, enlace, codigo, esc, urlSegura, textoPlano };
