 ...
 * el cliente la elige.
 */
function precioPuestoAqui(precioAleman, precioEspanol, garantia = 0) {
  const referencia = Number(precioEspanol) || 0;
  return (Number(precioAleman) || 0) + FEE_POPCAR
    + IMPUESTO_MATRICULACION * referencia + (Number(garantia) || 0);
}
 ...
 */
function desgloseParaElCliente(precioAleman, precioEspanol, garantia = null) {
...
function sqlPrecioPuestoAqui(alias = "", garantia = 0) {
  ...
  return `(COALESCE(${c}price,0) + ${FEE_POPCAR} + ${g} + ${IMPUESTO_MATRICULACION}*COALESCE(${c}market_price_es,0))`;
}
 ...
 */
function ahorroDelCliente(precioAleman, precioEspanol, garantia = 0) {
  const referencia = Number(precioEspanol) || 0;
  const puesto = precioPuestoAqui(precioAleman, referencia, garantia);
 ...
 */
function sePublica({ precioAleman, precioEspanol, comparables, viva = true, garantia = 0 }) {
  ...
  const { pct } = ahorroDelCliente(precioAleman, precioEspanol, garantia);
