...
beforeEach(() => {
  ...
  Pool.prototype.query = async (sql, params) => {
    ...
    if (/INSERT INTO moveadvisor_user_invoices/i.test(t)) {
      facturasEmitidas.push({ id: params[0], importe: params[4] });
...
const { fianzaCobrada } = require("./billing-webhook-handler.js");

const COBRO = { leadId: LEAD, email: CLIENTE, importe: 1019, sessionId: "cs_1", cobroRef: "pi_1" };

describe("la fianza se anota una sola vez", { concurrency: 1 }, () => {
  test("el primer aviso la marca y emite su factura", async () => {
    await fianzaCobrada(COBRO);
  ...
  test("el segundo no emite otra factura ni manda otro correo", async () => {
    ...
    assert.equal(correosMandados.length, trasElPrimero.correos,
      "el cliente recibiría dos veces la factura de la misma fianza");
  });
  ...
  test("sin saber de qué solicitud es, no se toca nada", async () => {
    await fianzaCobrada({ ...COBRO, leadId: "" });
