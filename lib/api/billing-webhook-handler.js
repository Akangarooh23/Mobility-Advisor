 ...
 *   · el expediente pasa a «Fianza pagada», que es lo que ve el cliente,
 ...
 */
async function fianzaCobrada({ leadId, email, importe, sessionId, cobroRef }) {
  const pool = getWebhookPgPool();
    ...
    const r = await pool.query(
      `UPDATE moveadvisor_market_leads
          SET deposit_paid_at = NOW(),
              ...
              status = CASE WHEN status IN ('Pendiente','Contactado') THEN 'Fianza pagada' ELSE status END
  ...
  if (!primeraVez) {
    console.log(`[fianza] ${leadId} ya estaba cobrada: no se emite otra factura`);
    return;
  }

  // La factura, por el mismo camino que la del informe de tasación: ese está
  // probado de punta a punta —se cobra, llega el correo con el PDF y sale en su
  // panel— y no hay motivo para que una fianza tenga otro. Lo unico distinto es
  // la serie: FIA, porque una fianza no es una venta ni una suscripcion.
  try {
    const numero = await siguienteNumeroDeFactura(pool, "FIA");
    const factura = {
      id: `fia-${leadId}`,
      number: numero,
      date: new Date().toISOString(),
      amount: Number(importe) || 0,
      status: "Pagada",
      pdfUrl: "",
      description: `Fianza de importación${coche ? ` · ${coche}` : ""}`,
    };
...
module.exports.fianzaCobrada = fianzaCobrada;