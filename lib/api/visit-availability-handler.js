...
async function sendBookingEmails(booking, opts = {}) {
   ...
   */
  const isParticular = !pendiente && booking.source === "marketplace" && booking.seller_email;
  ...
  const opsHtml = plantilla({
    ...
    cuerpo:
      (pendiente
        ...
        ? aviso('Hay que confirmarla', 'El cliente ya sabe que está pendiente. Confírmala o proponle otra desde la Agenda del ERP. Al vendedor se le avisa al confirmarla, no antes.')
        : '') +
      datos([
  ...
  if (isParticular) {
    sends.push(sendEmail({
      ...
      html: sellerHtml,
      movil: { titulo: "Alguien va a ver tu coche", cuerpo: `${dateStr} a las ${timeStr}`, pantalla: "visitas" },
...
async function sendCancelEmails(booking) {
   ...
   */
  const elVendedorLoSabia = Boolean(booking.seller_email) && booking.estado_anterior !== "pending";
...
async function cancelBooking(bookingId, token) {
  ...
  if (!r.rows.length) throw new Error("not_found");
  const booking = { ...r.rows[0], estado_anterior: antes.rows[0]?.status || "" };
  // Free the slot
  await pool.query(
