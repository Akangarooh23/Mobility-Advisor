...
async function facetaMarcas(pool, q) {
  ...
  const conOfertas = await pool.query(
    `SELECT (array_agg(brand ORDER BY cnt DESC))[1] AS nombre, SUM(cnt)::int AS n
      ...
      ORDER BY lower((array_agg(brand ORDER BY cnt DESC))[1]) ASC`,
    valores
  );

  const conocidas = new Set(conOfertas.rows.map((r) => r.nombre.toLowerCase()));
  const catalogo = await pool.query(
    `SELECT name FROM moveadvisor_vehicle_brands WHERE is_active ORDER BY lower(name) ASC`
  );
  const sinOfertas = catalogo.rows
    .map((r) => r.name)
    .filter((n) => n && !conocidas.has(n.toLowerCase()))
    .map((n) => ({ nombre: n, n: 0 }));
...
async function facetaModelos(pool, q) {
  ...
  const conOfertas = await pool.query(
    `SELECT (array_agg(model ORDER BY cnt DESC))[1] AS nombre, SUM(cnt)::int AS n
      ...
      ORDER BY lower((array_agg(model ORDER BY cnt DESC))[1]) ASC`,
    valores
  );
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (facets === "models") {
      return res.status(200).json({ ok: true, ...(await facetaModelos(pool, q)) });
    }
    if (facets === "extra") {
