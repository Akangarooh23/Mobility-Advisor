...
const { MARCA, correoInterno, correoSoporte } = require("../marca");
...
async function notifyOps(subject, lines) {
  ...
  if (!key) return;
  const ops = process.env.OPS_EMAIL || correoInterno();
  await fetch('https://api.resend.com/emails', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${key}` },
    body: JSON.stringify({
      // El remitente sale de la configuración, como el resto: escrito a fuego
      // aquí, este aviso salía de un dominio de la marca anterior.
      from: remitente(),
      to: [ops],
      subject,
      html: `<pre style="font-family:sans-serif;font-size:14px">${lines.join('\n')}</pre>`,
      // Éste ya es texto: no hace falta sacarlo del HTML, que es lo contrario.
      text: lines.join('\n')
    })
  }).catch(() => {});
}
...
/** Nuestro sitio como una fuente más, escrito igual que las de arriba: sin www. */
const DOMINIO_PROPIO = MARCA.sitio.replace(/^www\./, '');
...
// Admite los dos nombres para poder renombrar la variable en Vercel cuando
// convenga, sin que el bot se quede sin telefono entre una cosa y la otra.
const PHONE = process.env.POPCAR_PHONE || process.env.CARSWISE_PHONE || '';

function actionReply(tipo, offerTitle, ofertaId) {
  const t   = offerTitle || 'el vehículo';
  const url = ofertaId ? `${MARCA.sitioUrl}/marketplace-vo/${ofertaId}` : MARCA.sitioUrl;
  return {
...
async function processStep(pool, session, phone, body, transcript) {
  ...
  if (step === 'pre_reg_email') {
    ...
    return {
      ...
      ctx,
      reply:
        `Antes de continuar, ${MARCA.nombre} tratará tus datos (nombre, email, teléfono) para gestionar tu consulta según el RGPD.\n\n` +
        `¿Aceptas? Responde *SÍ* o *NO*.\n\n${MARCA.sitioUrl}/politica-privacidad`
    };
  }
  ...
  if (step === 'pre_reg_consent') {
    if (!/^s[ií]?$|^yes$|^acepto$/i.test(lo)) {
      return {
        step: 'rejected',
        ctx,
        reply: `Sin tu consentimiento no puedo continuar.${correoSoporte() ? ` Para consultas escríbenos a ${correoSoporte()}.` : ""}\n¡Hasta pronto! 👋`
      };
    }
  ...
  if (step === 'menu') {
    ...
    if (lo === '3') return { step: 'sell',              ctx, reply: `¿Ya tienes cuenta en ${MARCA.nombre}?\n\n1️⃣ Sí, tengo cuenta\n2️⃣ No, soy nuevo` };
  ...
  if (step === 'buy_search_budget') {
    ...
    // `subFuente` responde a "¿en qué portal viste el anuncio?": convive con
    // coches.net, wallapop y compañía. Este es el valor para "en ninguno, en
    // nuestra propia web", así que va sin www, como los demás.
    await saveLead(pool, { phone, preClienteId: ctx.preClienteId, userEmail: ctx.userEmail, tipo: 'BUSQUEDA', subFuente: DOMINIO_PROPIO, criterios: ctx.criterios, transcript });
  ...
  if (step === 'sell') {
    ...
    return { step, ctx, reply: `1️⃣ Tengo cuenta en ${MARCA.nombre}   2️⃣ Soy nuevo` };
  }
  ...
  if (step === 'tasacion_km') {
    ...
    return { step: 'done', ctx, reply: 'Perfecto. Nuestro equipo te contactará en menos de 24h con una valoración 💰\n\n¿Algo más? *SÍ* / *NO*' };
  }
  ...
  try {
    ...
    if (!session) {
      ...
      if (userInfo.type === 'unknown') {
        step     = 'pre_reg_name';
        greeting = flow === 'vo'
          ? `Hola 👋 Soy el asistente de ${MARCA.nombre}. Veo que estás interesado en uno de nuestros vehículos.\n\nPara poder ayudarte, ¿cómo te llamas?`
          : `Hola 👋 Soy el asistente de ${MARCA.nombre}.\n\nPara poder ayudarte, ¿cómo te llamas?`;
      } else {
    ...
    await sendWhatsApp(from, `Ha ocurrido un error. Por favor inténtalo de nuevo${correoSoporte() ? ` o contáctanos en ${correoSoporte()}` : ""}`, sendFromId).catch(() => {});
  }
};
