 ...
 *
 * Esa frase es la propiedad, no el comentario de un arreglo. Antes de esto, el
 * pool contaba anuncios; a partir de aquí cuenta coches. Quien toque estas
 * consultas tiene que poder seguir diciéndola en voz alta — si un cambio futuro
 * la rompe, lo que se rompe no es una optimización: es lo que hace que una
 * mediana signifique algo.
 *
 * ── El daño, medido ─────────────────────────────────────────────────────────
 *
 * Un concesionario con delegaciones publica el MISMO coche una vez por ciudad:
 * el mismo Dacia Dokker de 138.931 km y 7.590 € aparece 36 veces en
 * autoscout24, con seis nombres de sucursal distintos. Eso entra 36 veces en la
 * mediana y la tasación sale mal sin que nada dé error.
 *
 * En 917 cohortes marca+modelo, el 24,7 % de las ofertas son copias, la mediana
 * se desvía un 3,51 % de media y 197 cohortes pasan del 5 %, con un máximo del
 * 83,4 %. Es el triple de daño que las ofertas muertas y afecta a ocho veces
 * más cohortes.
 *
 * ── La huella, y por qué cada pieza ─────────────────────────────────────────
 *
 * `marca` + `version` + `año` + `km` + `precio`, del mismo grupo de vendedor.
 *
 * · `version` y NO `model`: cada portal escribe el modelo a su manera —"Serie
 *   2" contra "216"— y la versión llega casi idéntica. Agrupar por modelo daba
 *   un 57 % de duplicados, casi todo falso.
 * · Kilometraje exacto y NO redondo: el 18,8 % marca múltiplos de mil, y
 *   "100.000 km" junta coches distintos de verdad.
 * · Grupo de vendedor = primer token del nombre: sin eso "CLICARS MADRID" y
 *   "CLICARS MÁLAGA" son dos vendedores y sus coches dos coches.
 *
 * ── Y la coherencia: ante la duda, NO se fusiona ────────────────────────────
 *
 * Si dos anuncios del grupo se contradicen en algo físico, el grupo se deja
 * entero sin tocar. Los umbrales salen de mirar los 54.161 grupos reales, no
 * del sentido común: de 807 conflictos de potencia, 744 difieren en 5 CV o
 * menos —un portal dice 100 y otro 102 del mismo motor— y de 285 de color, 147
 * son "Gris" contra "Plata". Descartar cualquier diferencia habría tirado 1.146
 * grupos buenos para quitar unos 130 malos.
 *
 * Se implementa con `min() over` y `max() over` porque `count(distinct)` no
 * existe como función de ventana: si el mínimo y el máximo coinciden, todos los
 * valores no nulos son iguales.
 *
 * ── Lo que NUNCA se fusiona ─────────────────────────────────────────────────
 *
 * Una oferta sin vendedor conocido, con kilometraje redondo o sin precio no
 * entra en ninguna huella: se parte por su propio `id`, así que es su propio
 * grupo y sobrevive siempre. Sin ese cuidado, todas las ofertas sin
 * `dealer_name` caerían en la misma partición y la tasación perdería el 95 %
 * de sus comparables de golpe.
 */
const HUELLA_APLICABLE = `
...
/** Diferencia de potencia tolerada dentro de un grupo, en CV. Ver arriba: 744 de 807 están por debajo de 5. */
const BRECHA_CV_TOLERADA = 20;

/**
 * Envuelve una consulta de ofertas y se queda con un representante por coche.
 *
 * `columnas` es la lista que devuelve la consulta de dentro; se repite fuera
 * para no arrastrar las auxiliares de la deduplicación hasta quien consume.
 */
function deduplicando(consulta, limite) {
  return `
    WITH pool AS (${consulta}),
    marcado AS (
      SELECT p.*,
        /*
          DOS ventanas, y la diferencia no es cosmética.

          "w_orden" lleva ORDER BY para que ROW_NUMBER sea determinista: gana el
          anuncio actualizado más recientemente, que es el que mejor refleja el
          precio de hoy.

          "w_todo" NO lo lleva a propósito. Una ventana con ORDER BY cambia el
          marco por defecto a "desde el principio hasta la fila actual", así que
          MIN y MAX serían acumulados en vez de del grupo entero — y la
          comprobación de coherencia diría que no hay conflicto en las primeras
          filas de cada grupo.
        */
        ROW_NUMBER() OVER w_orden                                             AS _copia,
        MIN(_color_norm) OVER w_todo IS DISTINCT FROM MAX(_color_norm) OVER w_todo AS _choca_color,
        COALESCE(MAX(_cv) OVER w_todo - MIN(_cv) OVER w_todo, 0) > ${BRECHA_CV_TOLERADA} AS _choca_cv,
        MIN(_puertas) OVER w_todo IS DISTINCT FROM MAX(_puertas) OVER w_todo  AS _choca_puertas,
        MIN(_plazas)  OVER w_todo IS DISTINCT FROM MAX(_plazas)  OVER w_todo  AS _choca_plazas
      FROM pool p
      WINDOW
        w_orden AS (PARTITION BY _huella ORDER BY updated_at DESC NULLS LAST, _scraped_at DESC NULLS LAST),
        w_todo  AS (PARTITION BY _huella)
    )
    SELECT * FROM marcado
    -- Se queda el primero de cada coche; y si el grupo se contradice, se quedan
    -- TODOS: no se fusiona lo que no se entiende.
    WHERE _copia = 1 OR _choca_color OR _choca_cv OR _choca_puertas OR _choca_plazas
    /*
      El ORDER BY y el LIMIT viven FUERA, y ese orden importa.

      Dentro, el límite cortaría a 1.500 ANTES de quitar copias, y el pool
      acabaría con muchas menos ofertas útiles justo en los modelos con más
      duplicados — que son los que más falta les hace tener comparables.
    */
...
/**
 * Las columnas auxiliares que la deduplicación necesita. Van dentro del pool y
 * no salen fuera.
 *
 * `_huella` cae al `id` cuando la huella no es aplicable, y eso es lo que hace
 * que una oferta sin vendedor sea su propio grupo en vez de caer en el saco
 * común de todas las que no lo tienen.
 */
const COLUMNAS_DE_HUELLA = `
        CASE WHEN ${HUELLA_APLICABLE}
          THEN LOWER(REGEXP_REPLACE(brand, '[^a-zA-Z0-9]', '', 'g'))
        ...
        CASE
          ...
          ELSE LOWER(BTRIM(color))
        END AS _color_norm,
        NULLIF(power_cv, 0) AS _cv,
        NULLIF(doors, 0)    AS _puertas,
        NULLIF(seats, 0)    AS _plazas,
...
async function readPostgresInventory(limit = 30000, modelTokens = []) {
  ...
  try {
    ...
    if (safeTokens.length > 0) {
      ...
      const targetedQuery = deduplicando(
        `SELECT ${selectCols}, ${COLUMNAS_DE_HUELLA}
    ...
    const modernQuery = deduplicando(
      `SELECT ${selectCols}, ${COLUMNAS_DE_HUELLA}
       FROM moveadvisor_market_offers
