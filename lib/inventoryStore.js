...
async function getMarketplaceShowcase() {
  ...
  const sections = [
    conce && { key: "cw-concesionario", label: "Marketplace CarsWise · Concesionario", nav: { type: "marketplace-vo", tab: "concesionarios" }, offer: baseOffer(conce, firstImage(conce.image_url, conce.image_urls), conce.sale_price ?? conce.price) },
    part && { key: "cw-particulares", label: "Marketplace CarsWise · Particulares", nav: { type: "marketplace-vo", tab: "particulares" }, offer: baseOffer(part, firstImage(part.image_url, part.image_urls), part.sale_price ?? part.price) },
    rent && { key: "cw-renting", label: "Marketplace CarsWise · Renting", nav: { type: "marketplace-vo", tab: "renting_empresa" }, offer: { ...baseOffer(rent, firstImage(rent.image_url, rent.image_urls), rent.renting_36m ?? rent.renting_48m ?? rent.renting_60m ?? rent.renting_24m ?? rent.renting_12m), monthly: true } },
    imp && { key: "cw-importacion", label: "Marketplace CarsWise · Importación", nav: { type: "marketplace-vo", tab: "importacion" }, offer: baseOffer(imp, firstImage(imp.image_url, imp.images), Number(imp.price) + Number(imp.import_cost || 0)) },
