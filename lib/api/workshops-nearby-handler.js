...
module.exports = async function workshopsNearbyHandler(req, res) {
  ...
  // If precise coords provided, skip CP validation (geolocation arrives before reverse-geocode fills CP)
  if (!hasPrecise && !/^\d{5}$/.test(postalCode)) {
    return res.status(400).json({ error: "postalCode must be a 5-digit code" });
  }

  // If frontend sent precise geocoded coordinates, use them directly
  let userLocation;
  if (hasPrecise) {
  ...
  // Merge all providers and sort by distance so the closest workshop
  // (partner or independent) always appears first regardless of brand
