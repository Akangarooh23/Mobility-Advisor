...
// Las series, el contador y lo que una rectificativa tiene que decir.
const {
  SERIES, siguienteNumeroDeFactura, ENSURE_RECTIFICATIVA, referenciaALaOriginal,
...
module.exports = async function fianzaDevolucionHandler(req, res) {
  ...
  try {
    await pool.query(ENSURE_RECTIFICATIVA).catch(() => {});

    /*
     * A cuál rectifica, y desde cuándo.
     *
     * Una rectificativa que no dice a quién corrige no corrige nada: quien la
     * recibe no puede casarla con la que tiene. Se busca la original de este
     * mismo expediente; si no aparece —una devolución de algo que nunca llegó a
     * facturarse—, la rectificativa se emite igual y el hueco queda escrito, que
     * es mejor que no emitirla.
     */
    const original = await pool.query(
      // Por la referencia, no por la clave: la factura del servicio se guarda
      // con `srv-<lead>` en `referencia_externa` desde que la clave la pone la
      // base. Buscarla por `id` no encontraba nada y la rectificativa salía sin
      // decir a qué factura rectifica, que es justo lo que no puede faltar.
      `SELECT number, date FROM moveadvisor_user_invoices
        WHERE referencia_externa = $1 AND number IS NOT NULL`,
      [`srv-${leadId}`]
    ).catch(() => ({ rows: [] }));
    const previa = original.rows[0] || {};

    numero = await siguienteNumeroDeFactura(pool, SERIES.rectificativa);
    const porQue = motivo || "Devolución de la fianza: no se sigue adelante con la importación";
    const referencia = referenciaALaOriginal({ numero: previa.number, fecha: previa.date });

    await pool.query(
      /*
       * `rect-<lead>` va en `referencia_externa`, no en la clave.
       *
       * Es lo que hace que esta rectificativa se emita **una sola vez** aunque
       * se pulse dos veces: el conflicto se mira contra esa referencia. La clave
       * de la fila la pone la base, como en el resto de facturas, desde que se
       * dejó de llamarlas como las llamaba Stripe.
       */
      `INSERT INTO moveadvisor_user_invoices
         (referencia_externa, email, number, date, amount, status, description, cw_invoice_number, cw_paid_at,
          rectifica_numero, rectifica_fecha, rectifica_motivo)
       VALUES ($1, lower($2), $3, NOW(), $4, 'Devuelta', $5, $3, NOW(), $6, $7, $8)
