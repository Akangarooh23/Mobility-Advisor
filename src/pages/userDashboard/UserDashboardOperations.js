...
export default function UserDashboardOperations({
  ...
  const [showCalendarReminders, setShowCalendarReminders] = useState(false);
    ...
    <section id="user-dashboard-operations" style={{ ...panelStyle, ...sectionFrame, marginBottom: 16 }}>
      ...
      {calendarReminders.length > 0 && (
        <div style={{ marginTop: 20 }}>
          ...
          >
            <span>📅</span>
            <span>Recordatorios de citas ({calendarReminders.length})</span>
            <span style={{ marginLeft: "auto", fontSize: 10 }}>{showCalendarReminders ? "▲" : "▼"}</span>
          </button>
