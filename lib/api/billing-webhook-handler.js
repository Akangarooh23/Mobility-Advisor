...
async function depositoRecibido({ leadId, email, importe, fee, sessionId, cobroRef }) {
  ...
  try {
    ...
    if (imp) suplidos.push({ concepto: "Impuesto de matriculacion (a cuenta)", importe: imp });
    if (gar) suplidos.push({ concepto: "Garantia mecanica (aseguradora)", importe: gar });
  } catch (e) {
...
async function generateDepositoPdf(datosDep, userProfile = {}) {
  ...
  [
    datosDep.email,
    userProfile.tax_id ? `NIF/CIF: ${userProfile.tax_id}` : "",
    datosDep.coche ? `Vehiculo: ${datosDep.coche}` : "",
  ...
  // sueltan y que vuelven si el coche no es el que se anuncio.
  let ny = y + 56;
  dt("QUE PASA CON CADA PARTE", 50, ny, 8.5, rb, c("#96968F"));
  ny += 16;
  const notas = [
    "El precio del coche no se le paga al vendedor hasta que uno de los nuestros lo ve",
    "en Alemania y confirma que es el que se anuncio. Si no lo es, se te devuelve entero.",
    "",
    "El impuesto de matriculacion va a cuenta: se liquida al matricular. Si sale menos,",
    "se te devuelve la diferencia; si sale mas, se te cobra.",
    "",
    "El coche, el impuesto y la garantia son suplidos: dinero pagado en tu nombre, que no",
    "es ingreso de " + MARCA.nombre + " y va fuera de la base de IVA. Te entregamos el justificante de",
    "cada uno —factura del vendedor, impuesto pagado y poliza— segun se vayan teniendo.",
  ];
