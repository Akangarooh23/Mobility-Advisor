/**
 * Le dice al orquestador de Flexicar a quién tiene que llamar.
 *
 *   npm run enlaza-segmento-flexicar
 *
 * Busca «Flexicar – Segmento (páginas)» en la base de n8n, coge su id, lo escribe en
...
const GENERADOR = path.join(__dirname, "genera-scraper-flexicar.js");
const NOMBRE = "Flexicar – Segmento (páginas)";
...
if (!filas.length) {
  ...
  console.error("Impórtalo primero (n8n-workflows/flexicar-segmento.json) y vuelve a lanzar esto.");
  process.exit(1);
}
if (filas.length > 1) {
...
console.log("\n  Ahora reimporta n8n-workflows/flexicar-scraper-offers.json en n8n.");
