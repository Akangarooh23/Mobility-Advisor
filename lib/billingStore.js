...
async function listGarageVehiclesByEmailPostgres(email = "") {
  ...
  return vehicles.map((row) => {
    ...
    return sanitizeGarageVehicle({
      id: row.id,
      // El numero del IDCar (IDC-0003): lo que el cliente dice cuando llama.
      numero: row.numero || "",
...
function sanitizeGarageVehicle(input = {}) {
  ...
  return {
    ...
    /*
     * El número del IDCar: IDC-0003.
     *
     * Lo pone la base al dar de alta el coche. Se manda al panel y a la app
     * porque es lo que el cliente puede leer en voz alta cuando llama, y lo que
     * quien coge el teléfono escribe en el buscador del ERP. El identificador
     * de verdad —el que va en las direcciones— sigue sin salir a pantalla.
     */
 ...
 * ## Lo que pasaba
 *
 * Esto lo buscaba en el fichero de `readStore()`, y `readStore()` **en
 * producción no lee el fichero nunca**: devuelve vacío a propósito, para no
 * arrastrar datos de prueba. O sea que la respuesta era siempre «no lo
 * conozco».
 *
 * Y quien pregunta es el webhook de Stripe. Sin correo, la rama entera se
 * saltaba en silencio: una baja o un cambio de plan que manda Stripe no
 * llegaba nunca a nuestra base. Las facturas se salvaban de casualidad, porque
 * Stripe manda `customer_email` aparte.
 *
 * ## Dónde estaba el dato
 *
 * En Postgres, desde el principio: `syncPlanToPostgres` escribe
 * `moveadvisor_users.stripe_customer_id` y `stripe_subscription_id` en cada
 * activación. La búsqueda al revés miraba en el sitio equivocado.
 *
 ...
 * entregado y la baja se pierde para siempre. Quien llama decide, y lo que
 * hace es pedirle a Stripe que lo reintente.
 */
const COLUMNAS_DE_STRIPE = {
  cliente: "stripe_customer_id",
  suscripcion: "stripe_subscription_id",
};

async function buscaCuentaPorStripe(cual, valor) {
  const columna = COLUMNAS_DE_STRIPE[cual];
  if (!columna) throw new Error("columna de Stripe desconocida: " + cual);

  const normalized = normalizeText(valor);
  if (!normalized) {
    return "";
  }
  ...
  if (hasPostgresConnection()) {
    const { rows } = await getPgPool().query(
      `SELECT email FROM moveadvisor_users WHERE ${columna} = $1 LIMIT 1`,
      [normalized]
    );
    if (rows.length) {
  ...
  // En local, sin base, sigue valiendo el fichero. En producción esto devuelve
  // vacío, que ahí ya es la respuesta correcta: no es nuestro.
  const store = readStore();
  const mapa = cual === "cliente" ? store?.stripeCustomerToEmail : store?.stripeSubscriptionToEmail;
  return normalizeEmail(mapa?.[normalized]);
}

async function getEmailByStripeCustomerId(customerId = "") {
  return buscaCuentaPorStripe("cliente", customerId);
}

async function getEmailByStripeSubscriptionId(subscriptionId = "") {
  return buscaCuentaPorStripe("suscripcion", subscriptionId);
}
