...
async function sendBookingEmails(booking, opts = {}) {
  ...
  const buyerHtml = plantilla({
    titulo: isReschedule ? 'Tu cita se ha movido' : 'Tu cita está confirmada',
    cuerpo:
      ...
      boton('Gestionar mi cita', manageUrl) +
  ...
  const opsHtml = plantilla({
    ...
    cuerpo:
      ...
      boton('Ver en el ERP', `${SITE_URL.replace('www.', 'erp.')}/marketplace`),
