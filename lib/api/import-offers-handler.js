...
const { opcionesParaElCoche, catalogoDeGarantias, sqlGarantiaPorDefecto } = require("../garantias");
const { serviciosParaElCliente } = require("../servicios");
const { loQuePagaAhora } = require("../escrow");
const { FEE_POPCAR_CON_IVA } = require("../coste-importacion");
...
function getImportPool() {
  ...
  _importPool = new Pool({ connectionString: connString, ssl: SSL_POSTGRES });
  return _importPool;
}
 ...
 *
 * Se arma con el catálogo de garantías porque el precio publicado lleva una
 * puesta, y no cuesta lo mismo en todos los coches: a uno de quince años no se
 * le puede dar ninguna y su precio no sube nada. Por eso no puede ser una
 * constante de módulo.
 */
const precioPuestoSql = (garantias) => sqlPrecioPuestoAqui("", sqlGarantiaPorDefecto(garantias));
 ...
 */
const ahorroPctSql = (garantias) => sqlAhorroPct("", sqlGarantiaPorDefecto(garantias));
 ...
 */
function ordenSql(sort, PRECIO_PUESTO, AHORRO_PCT) {
...
function mapOffer(r, garantias) {
   ...
   */
  /**
   * El precio que se publica lleva la garantía de por defecto dentro.
   *
   * Así el precio de la lista, el de la ficha y el que se le pide son el mismo,
   * y quitar la garantía **baja** el total en vez de que añadirla lo suba al
   * final. Es el mismo dinero y se lee mucho mejor.
   */
  const conGarantia = opcionesParaElCoche(garantias, r);
  const desglose  = desgloseParaElCliente(r.price, marketEs, conGarantia.porDefecto, r);
  ...
  /**
   * Lo que deposita: el coche entero y nuestro fee.
   *
   * Ya no es una fianza del 30 %. PopCar no le vende el coche —se lo vende el
   * concesionario alemán— así que el dinero del coche tiene que estar, y se
   * queda retenido hasta que uno de los nuestros lo ve allí.
   *
   * El impuesto de matriculación no va en el depósito: es de Hacienda y se
   * liquida al matricular, cuando ya se sabe cuánto es de verdad.
   */
  const impuesto = desglose.lineas.find((l) => /impuesto/i.test(l.concepto))?.importe || 0;
  const deposito = loQuePagaAhora({
    precioCoche: r.price, fee: FEE_POPCAR_CON_IVA, impuesto,
    garantia: conGarantia.porDefecto ? conGarantia.porDefecto.precio : 0,
  });
  ...
  return {
    ...
    origin: "Alemania",
    importPrice: landed,
    // Lo que deposita ahora, partido por destino, para que la ficha pueda
    // decir qué parte es del vendedor y qué parte nuestra.
    importDeposito: deposito,
...
module.exports = async function importOffersHandler(req, res) {
    ...
    try {
      const r = await pool.query(
        `SELECT ${SELECT_COLS} FROM moveadvisor_market_offers
         WHERE id = $1 AND country = 'DE' AND import_published = TRUE AND COALESCE(is_active, TRUE) = TRUE
           AND COALESCE(import_locked, FALSE) = FALSE LIMIT 1`,
        [offerId]
      );
   ...
   */
  //  es un coche que ya es de un cliente nuestro: se le ha
  // pagado al vendedor aleman. Sigue vivo en AutoScout24 y sigue saliendo a
  // cuenta, asi que las otras dos banderas no lo apartan.
  const conditions = ["country = 'DE'", "import_published = TRUE", "price IS NOT NULL", "COALESCE(is_active, TRUE) = TRUE", "COALESCE(import_locked, FALSE) = FALSE"];
  const values = [];
  ...
  // La horquilla de precio se añade más abajo, cuando ya se sabe cuánto vale la
  // garantía: el precio por el que se corta es el que está viendo, con ella.
  ...
  try {
    /**
     * El catálogo va **antes** y no en la misma tanda.
     *
     * Es lo que costaba: una consulta de tres filas de más en serie. Lo que se
     * gana es que el precio por el que se ordena y por el que se corta la
     * horquilla sea el mismo que el de la tarjeta, garantía incluida. Pedirlos
     * a la vez obligaría a ordenar por un precio sin garantía y a pintar otro
     * con ella, y eso se lee como que el filtro está roto.
     */
    const garantias = await catalogoDeGarantias(pool);
    const PRECIO_PUESTO = precioPuestoSql(garantias);
    const AHORRO_PCT = ahorroPctSql(garantias);
    ...
    const [rows, totalRes, marcasRes] = await Promise.all([
      pool.query(
        `SELECT ${SELECT_COLS} FROM moveadvisor_market_offers ${where}
         ORDER BY ${ordenSql(sort, PRECIO_PUESTO, AHORRO_PCT)}
