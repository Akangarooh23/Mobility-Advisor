 ...
 *
 * Ya no es una fianza del 30 %. PopCar no compra el coche: lo compra el cliente
 * al concesionario alemán. Así que deposita **el coche entero y nuestro fee**,
 * y ese dinero no se mueve hasta que alguien nuestro está delante del coche.
 */
...
/**
 * Lo que deposita: el coche, nuestro fee y el impuesto.
 *
 * No un porcentaje: el coche entero, porque se lo va a comprar él al vendedor
 * alemán y ese dinero tiene que estar.
 *
 * Y el impuesto **como provisión**. Si fuera un precio cerrado y el real saliera
 * por encima —pasa en los coches de más de 160 g/km— la diferencia saldría de
 * nuestro margen. Así la paga siempre el cliente, que es de quien es.
 */
const { FEE_POPCAR_CON_IVA, desgloseParaElCliente } = require("../coste-importacion.js");
const IMPUESTO = desgloseParaElCliente(OFERTA.price, OFERTA.market_price_es)
  .lineas.find((l) => /impuesto/i.test(l.concepto)).importe;
const DEPOSITO = OFERTA.price + FEE_POPCAR_CON_IVA + IMPUESTO;
  ...
  Pool.prototype.query = function (sql, params, cb) {
    ...
    if (/INSERT INTO moveadvisor_market_leads/i.test(t)) {
      guardado = {
        ...
        servicios: p[15],
        escrowCoche: p[16], escrowFee: p[17], escrowGarantia: p[18], escrowImpuesto: p[19],
...
describe("pedir un coche de importación", () => {
  ...
  test("el enlace al anuncio se guarda entero", async () => {
    ...
    // ninguna parte.
    //
    // El dominio sale de la marca y no escrito a mano. Estaba dentro de una
    // expresión regular, con los puntos escapados, y por eso no lo encontró
    // ninguna búsqueda del dominio viejo al cambiarlo: lo cazó esta prueba
    // fallando, que es tarde pero es el único sitio donde iba a saltar.
    assert.ok(
      guardado.url.startsWith(`${MARCA.sitioUrl}/marketplace-vo/`),
      `esperaba una ficha de ${MARCA.sitioUrl} y llegó ${guardado.url}`,
    );
    assert.ok(guardado.url.endsWith(OFERTA.id), "y es el de ese coche");
  });

  test("lo que deposita queda guardado", async () => {
    const r = await pide();
    assert.equal(r.cuerpo.deposit, DEPOSITO, "el coche entero más nuestro fee");
    assert.equal(Number(guardado.fianza), DEPOSITO,
      "si no se guarda, cambia el precio de la oferta y ya nadie sabe qué se le prometió");
  });

  test("y se guarda partido, porque cada parte va a un sitio", async () => {
    // Lo del coche se libera al vendedor alemán; lo del fee es nuestro y no
    // viaja. Guardarlo junto obligaría a recalcular la parte de cada uno el día
    // que haya que soltar el dinero, y para entonces el fee puede haber cambiado.
    await pide();
    assert.equal(Number(guardado.escrowCoche), OFERTA.price);
    assert.equal(Number(guardado.escrowFee), FEE_POPCAR_CON_IVA);
  });

  test("el impuesto entra, y es el mismo que se le enseñó en la ficha", async () => {
    ...
    // principio: ese cobro no se puede caer.
    const r = await pide();
    const enLaFicha = Math.round(precioPuestoAqui(OFERTA.price, OFERTA.market_price_es));
    assert.equal(Number(r.cuerpo.deposit), enLaFicha,
      "lo que se le pide tiene que ser lo que se le enseñó");
  });
  ...
  test("y es la misma que va en su correo", async () => {
    ...
    assert.ok(suyo.html.includes(DEPOSITO.toLocaleString("es-ES")) || suyo.html.includes(String(DEPOSITO)),
...
describe("los servicios que se contratan aparte", () => {
  test("se guardan los que ha marcado", async () => {
    await pide({ servicios: ["seguro", "reacondicionado"] });
    assert.deepEqual(JSON.parse(guardado.servicios), ["seguro", "reacondicionado"]);
  });
  ...
  test("lo que llegue y no sea un servicio, fuera", async () => {
    ...
    await pide({ servicios: ["seguro", "un-descuento-del-90", ""] });
    assert.deepEqual(JSON.parse(guardado.servicios), ["seguro"]);
  });
  ...
  test("la fianza no sube por marcarlos", async () => {
    ...
    const con = await pide({ servicios: ["seguro", "reacondicionado"] });
    assert.equal(Number(con.cuerpo.deposit), Number(sin.cuerpo.deposit));
    assert.equal(Number(con.cuerpo.deposit), DEPOSITO);
  });
...
/**
 * El impuesto guardado, o el reparto no cuadra.
 *
 * El depósito lo lleva dentro. Si no se guarda en su columna, el ERP enseñaría
 * el coche y el fee —19.890— contra un depósito de 21.310, y la diferencia no
 * tendría nombre.
 */
describe("el impuesto guardado con el depósito", () => {
  test("se guarda su parte", async () => {
    await pide();
    assert.equal(Number(guardado.escrowImpuesto), IMPUESTO);
  });

  test("y las partes suman lo que se le pide", async () => {
    const r = await pide();
    const suma = Number(guardado.escrowCoche) + Number(guardado.escrowFee)
      + Number(guardado.escrowGarantia) + Number(guardado.escrowImpuesto);
    assert.equal(suma, Number(r.cuerpo.deposit),
      "el reparto del ERP no cuadraría con la cifra grande");
  });
});
