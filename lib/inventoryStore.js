...
async function listMarketplaceVoOffers(options = {}) {
  ...
  try {
    ...
    // Hide dealer/importer records when a 'particular' user has published the same plate
    conditions.push(`NOT EXISTS (
      SELECT 1 FROM moveadvisor_marketplace_vo_offers p
      WHERE p.seller_type = 'particular'
        AND p.is_active = TRUE
        AND p.matricula IS NOT NULL AND p.matricula != ''
        AND upper(p.matricula) = upper(o.matricula)
        AND p.id != o.id
    )`);
