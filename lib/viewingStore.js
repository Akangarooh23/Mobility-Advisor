...
const FROM = process.env.ALERT_EMAIL_FROM || process.env.RESEND_FROM_EMAIL || MARCA.remitentePorDefecto;
...
async function sendConfirmationEmails({ appointment }) {
  ...
  const buyerHtml = `
    <div style="font-family:Arial,sans-serif;line-height:1.6;color:#0f172a;max-width:620px;margin:0 auto;">
      ...
      <p>El vendedor se pondrá en contacto contigo para confirmar el punto de encuentro. Si necesitas gestionar la cita, contacta con ${MARCA.nombre}.</p>
    </div>`;
  const sellerHtml = `
    <div style="font-family:Arial,sans-serif;line-height:1.6;color:#0f172a;max-width:620px;margin:0 auto;">
      ...
      <p>Prepárate para la visita. ${MARCA.nombre} puede ayudarte con la gestión de la venta si lo necesitas.</p>
    </div>`;
  await Promise.all([
