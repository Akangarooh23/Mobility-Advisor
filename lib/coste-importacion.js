...
function cilindrada(displacement) {
  /*
   * La columna guarda las dos unidades: «1498» y «1.5» son el mismo motor.
   *
   * Quitando todo lo que no sea número, «1.5» se convierte en 15 cc. Y 15 cc
   * no es un error inofensivo: los caballos por litro salen por las nubes,
   * así que el coche NUNCA cuenta como «motor grande y perezoso» y se queda
   * sin el 20 % de CO₂ que le toca. Medido el 21-sep sobre las ofertas
   * alemanas: 3.891 coches cambiaban de banda de impuesto al arreglarlo, y el
   * impuesto subía 643 € de media.
   *
   * Sube, que es el lado malo: hoy les calculamos de menos, y el precio que
   * se le enseña al cliente sería más barato del real. Un SEAT Leon 1.5 de
   * 116 CV pasa de 144 a 172 g/km estimados y de 4,75 % a 9,75 % de impuesto.
   *
   * Los datos se convirtieron a cc con scripts/arregla-cilindrada.js, pero
   * esto se queda aquí de todas formas: el dato entra de doce portales y no
   * hay garantía de que no vuelva a llegar en litros.
   */
  const texto = String(displacement || "").trim();
  // Un decimal entre 0,6 y 8,0 es un motor de calle expresado en litros. El
  // punto va entre corchetes y no escapado: una barra no sobrevive a viajar
  // dentro de una cadena, y un punto suelto casa con cualquier carácter.
  if (/^[0-9]+[.][0-9]+$/.test(texto)) {
    const litros = Number(texto);
    if (litros >= 0.6 && litros <= 8) return Math.round(litros * 1000);
  }
  const cc = Number(texto.replace(/[^0-9]/g, ""));
...
/**
 * Y cuánto tiene que ahorrar EN EUROS, que es lo que de verdad decide.
 *
 * El porcentaje solo no sirve para elegir pocos coches y buenos. Un 15 % sobre
 * un coche de 20.000 € son 3.000, que es justo lo que cuesta el fee: el cliente
 * se mete en una importación para no ahorrar nada. Con el listón solo en
 * porcentaje, el peor coche del escaparate le ahorraba 2.935 €.
 *
 * Y hay una razón más, que es la que decide entre subir el porcentaje o subir
 * los euros. Medido el 21-sep sobre las ofertas alemanas reales:
 *
 *     LISTÓN              COCHES   AHORRO MEDIO   EL PEOR    12-25k  25-45k  45k+
 *      15% y      0 €       942        8.132 €    2.935 €      634     223     85
 *      15% y  8.000 €       339       13.204 €    8.020 €       95     159     85
 *      20% y  8.000 €       212       14.280 €    8.020 €       95      77     40
 *      25% y  8.000 €       122       15.074 €    8.062 €       86      18     18
 *
 * Mirad la última columna. SUBIR EL PORCENTAJE VACÍA LA GAMA ALTA: de 85 coches
 * de más de 45.000 € a 40, y luego a 18. No es que esos coches sean malos, es
 * que un 25 % sobre un coche de 60.000 € casi no existe —aunque ahorrar 12.000 €
 * en uno, sí—. El porcentaje castiga justo donde más dinero se le ahorra al
 * cliente.
 *
 * El suelo en euros hace lo contrario: corta por abajo —de 634 coches baratos a
 * 95— y deja la gama alta intacta. Eso es «pocos y que merezcan la pena».
 *
 * Así que el 15 % se queda como red de seguridad, no como filtro: impide que un
 * coche carísimo entre solo porque el ahorro en euros es grande. Quien elige es
 * este número.
 */
const AHORRO_MINIMO_EUROS = 8000;
...
function sePublica({
  precioAleman, precioEspanol, comparables, viva = true,
  danado = false, danosComprobados = true, garantia = 0, coche = null,
}) {
  ...
  /*
   * Y un coche dañado tampoco, ni uno del que no lo sepamos.
   *
   * Esto vivía solo en el flujo que comprueba los daños, que despublica lo que
   * encuentra roto. Pero la decisión de publicar es esta función, y no miraba
   * los daños: cada vez que se recalculaba, los volvía a publicar. Así
   * acabaron 29 dañados en la tienda el 10 de septiembre.
   *
   * «Sin comprobar» cuenta como dañado a propósito. El comprobador se bloquea
   * a veces -el portal alemán nos corta- y deja coches sin mirar; publicarlos
   * mientras tanto es exactamente lo que no se puede hacer.
   */
  if (danado === true) return false;
  if (danosComprobados === false) return false;
  ...
  const { pct, euros } = ahorroDelCliente(precioAleman, precioEspanol, garantia, coche);
  // Los euros primero, que son los que eligen; el porcentaje es la red de
  // seguridad para que un coche carísimo no entre solo por el tamaño del
  // ahorro. Y el techo, para que no entre por una errata de precio.
  if (!(euros >= AHORRO_MINIMO_EUROS)) return false;
