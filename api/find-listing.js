...
async function findListing({ result, answers, filters }) {
  ...
  try {
    ...
    const inventory = await listInventoryOffers({
      ...
      displacementMax: Number(filters?.displacementMax) || null,
      co2: normalizeText(filters?.co2 || ""),
      co2Min: Number(filters?.co2Min) || null,
      co2Max: Number(filters?.co2Max) || null,
      powerKw: normalizeText(filters?.powerKw || ""),
      powerKwMin: Number(filters?.powerKwMin) || null,
      powerKwMax: Number(filters?.powerKwMax) || null,
