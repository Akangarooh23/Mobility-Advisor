...
async function ensureMobilitySchemaPostgres() {
  ...
  // Fast-path: single probe query — if the last-added column exists, schema is current
  try {
    await pool.query("SELECT next_itv_date FROM moveadvisor_user_vehicles LIMIT 0");
...
async function listGarageVehiclesByEmailPostgres(email = "") {
  ...
  // Fetch all vehicle-related data in parallel (states, files, docs, insurances, maintenances)
  const [statesResult, filesResult, documentsResult, insurancesResult, maintenancesResult] = await Promise.all([
    pool.query(
