...
async function getUserMobilityDataByEmailPostgres(email = "") {
  // Ensure schema ONCE before parallel queries so all concurrent calls
  // skip the DDL block (which would deadlock if run simultaneously).
  await ensureMobilitySchemaPostgres();

  const [
    vehicles, appointments, maintenances, insurances,
    valuations, vehicleStates, savedOffers, solicitudes, garageVehicleCount,
  ] = await Promise.all([
    listGarageVehiclesByEmailPostgres(email).catch(() => []),
    listAppointmentsByEmailPostgres(email).catch(() => []),
    listMaintenancesByEmailPostgres(email).catch(() => []),
    listInsurancesByEmailPostgres(email).catch(() => []),
    listValuationsByEmailPostgres(email).catch(() => []),
    listVehicleStatesByEmailPostgres(email).catch(() => []),
    listSavedOffersByEmailPostgres(email).catch(() => []),
    listSolicitudesByEmailPostgres(email).catch(() => []),
    countGarageVehiclesByEmailPostgres(email).catch(() => 0),
  ]);
  return { vehicles, appointments, maintenances, insurances, valuations, vehicleStates, savedOffers, solicitudes, garageVehicleCount };
}
