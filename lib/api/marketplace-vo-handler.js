const { listMarketplaceVoOffers, getMarketplaceVoOfferByUrl } = require("../inventoryStore");
  ...
  if (req?.query?.stats === "1" && vehicleId) {
    ...
    try {
      const [, contactResult, viewResult] = await Promise.all([
        pool.query(
          `UPDATE moveadvisor_marketplace_vo_offers SET view_count = COALESCE(view_count, 0) + 1 WHERE id = $1`,
          [vehicleId]
        ).catch(() => null),
        pool.query(
        ...
        pool.query(
          `SELECT COALESCE(view_count, 0)::int AS cnt FROM moveadvisor_marketplace_vo_offers WHERE id = $1 LIMIT 1`,
          [vehicleId]
        ),
      ]);
