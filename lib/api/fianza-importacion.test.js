...
process.env.BILLING_STORE_PATH = path.join(os.tmpdir(), `popcar-fianza-importacion-${process.pid}.json`);
process.env.DATABASE_URL = "postgres://nadie@localhost:5432/ninguna";
process.env.STRIPE_SECRET_KEY = "sk_de_mentira";
// Sin esto el correo de la factura no se intenta mandar y la prueba de que
// llega no comprobaria nada.
...
// El depósito es el coche entero más nuestro fee. Se factura solo el fee.
const FIANZA = 20999;
const FEE = 2999;
...
function reinicia() {
  ...
  sesionCreada = null;
  guardado = { marcada: null, factura: null, serie: null, correo: null };
}
...
before(() => {
  Pool.prototype.query = function (sql, params, cb) {
    ...
    if (/UPDATE moveadvisor_market_leads/i.test(t)) {
      guardado.marcada = { id: p[0], paga: /deposit_paid_at/.test(t), avanza: /Depósito retenido/.test(t) };
    ...
    if (/INSERT INTO moveadvisor_user_invoices/i.test(t)) {
      // id, email, numero, fecha, importe, estado, pdf, concepto, pagada.
      guardado.factura = { id: p[0], email: p[1], numero: p[2], importe: p[4], concepto: p[7] };
      return responde([]);
    }
    return responde([]);
  };
  global.fetch = async (url, opciones) => {
    ...
    if (u.includes("resend.com")) {
      ...
      guardado.correo = { to: String(c.to), subject: String(c.subject), adjuntos: (c.attachments || []).length };
...
describe("abrir el pago de la fianza", () => {
  ...
  test("la sesión lleva de qué solicitud es, que es lo que la ata al expediente", async () => {
    await abrePago();
    assert.equal(sesionCreada.get("metadata[plan_id]"), "deposito");
...
describe("cuando Stripe dice que está pagada", () => {
  /**
   * El aviso de que la transferencia ha llegado.
   *
   * Con transferencia es `async_payment_succeeded`, no `completed`: el
   * `completed` llega cuando el cliente ve el IBAN, y entonces todavía no hay
   * dinero. Hay una prueba abajo que fija justo eso.
   */
  async function avisaStripe(meta = {}, tipo = "checkout.session.async_payment_succeeded") {
    const evento = {
      type: tipo,
      data: { object: {
        id: "cs_1", customer_email: CLIENTE, amount_total: FIANZA * 100,
        payment_status: tipo === "checkout.session.completed" ? "unpaid" : "paid",
        metadata: {
          plan_id: "deposito", lead_id: LEAD, customer_email: CLIENTE,
          importe: String(FIANZA), fee: String(FEE), ...meta,
        },
      } },
    };
  ...
  test("la solicitud queda pagada y el expediente avanza", async () => {
    ...
    assert.ok(guardado.marcada.avanza, "de «Contactado» pasa a «Depósito retenido» solo");
  });

  test("se emite su factura, con serie propia y por lo nuestro", async () => {
    await avisaStripe();
    assert.equal(guardado.serie, "SRV", "lo que facturamos es un servicio, no una venta");
    assert.ok(guardado.factura, "sin factura, ha pagado y no tiene justificante");
    assert.equal(guardado.factura.numero, "SRV-" + new Date().getFullYear() + "-0001");
    assert.equal(Number(guardado.factura.importe), FEE,
      "se estaría facturando el coche del vendedor alemán, que no es ingreso nuestro");
    assert.equal(guardado.factura.email, CLIENTE);
    assert.match(guardado.factura.concepto, /Servicio de importación/);
  });

  test("ver el IBAN no es haber transferido", async () => {
    // Con tarjeta, terminar la pantalla era haber pagado. Con transferencia es
    // haber visto un número de cuenta, y el dinero llega días después. Si se
    // marcara aquí, el expediente diría que hay dinero dentro y con ese estado
    // alguien coge un avión para ir a ver un coche.
    await avisaStripe({}, "checkout.session.completed");
    assert.equal(guardado.marcada, null, "se ha marcado el depósito sin que llegue el dinero");
    assert.equal(guardado.factura, null, "y se ha emitido una factura por dinero que no está");
  });

  test("y le llega por correo, con el PDF adjunto", async () => {
    await avisaStripe();
    assert.ok(guardado.correo, "una factura que no sale de la base no la ve nadie");
    assert.equal(guardado.correo.to, CLIENTE);
    assert.ok(guardado.correo.adjuntos > 0, "con el PDF: es lo que guarda la gente");
  });
