...
async function sendViewingLeadEmails({ name, email, message, vehicle_title }) {
  ...
  const results = await Promise.allSettled([
    fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({ from, reply_to: respuestaA(), to: email, subject: `Solicitud recibida — ${vehicle_title || MARCA.nombre}`, html: clientHtml, text: textoPlano(clientHtml) }),
    }),
    fetch("https://api.resend.com/emails", {
      method: "POST",
      headers: { Authorization: `Bearer ${apiKey}`, "Content-Type": "application/json" },
      body: JSON.stringify({ from, to: internalEmail, subject: `Nueva visita — ${vehicle_title || "sin vehículo"}`, html: internalHtml, text: textoPlano(internalHtml) }),
    }),
  ]);
  results.forEach((r, i) => {
