  ...
  return (
    <div style={{ ...styles.center, maxWidth: 980, textAlign: "left" }}>
      ...
      <article className="ma-card-soft" style={{ border: "1px solid rgba(148,163,184,0.22)", borderRadius: 16, background: "rgba(15,23,42,0.55)", padding: "16px 14px" }}>
        ...
        <h1 style={{ margin: "0 0 10px", color: "var(--gris-50)", fontSize: "clamp(26px, 4vw, 36px)", lineHeight: 1.25 }}>
          {article.title}
        </h1>
        <p style={{ margin: "0 0 14px", color: "var(--gris-300)", fontSize: 14, lineHeight: 1.75 }}>
          {article.intro}
        </p>
              ...
              {Array.isArray(section.paragraphs) && section.paragraphs.length > 0 && (
                <div style={{ display: "grid", gap: 8 }}>
                  {section.paragraphs.map((paragraph) => (
                    <p key={paragraph} style={{ margin: 0, color: "var(--gris-300)", fontSize: 13, lineHeight: 1.75 }}>
                      {paragraph}
                    </p>
                  ))}
                </div>
              ...
              {Array.isArray(section.bullets) && section.bullets.length > 0 && (
                <ul style={{ margin: "0", paddingLeft: 18, display: "grid", gap: 6 }}>
                  {section.bullets.map((item) => (
                    <li key={item} style={{ color: "var(--gris-300)", fontSize: 13, lineHeight: 1.65 }}>
                      {item}
                    </li>
                  ))}
                </ul>
        ...
        <p style={{ margin: "14px 0 0", color: "var(--gris-200)", fontSize: 13, lineHeight: 1.75, fontWeight: 700 }}>
          {article.conclusion}
        </p>
      </article>
    </div>
