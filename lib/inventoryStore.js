...
// Activado 2026-07-23: { alpha:0, balance:true } — cuota por año, sin kernel.
// El sweep confirmó que alpha no añade nada sobre la cuota; mkt-full yrPct ≤ 0.12
// en todos los fixtures → mercado genuinamente sesgado, no artefacto del LIMIT.
const POOL_CONFIG = { alpha: 0, balance: true };
...
async function getMarketPriceSnapshot(options = {}) {
  ...
  const baseQuery = { desiredType, modelCandidates, maxMileage: mileage, location, limit: POOL_STORE_LIMIT };
  ...
  // Separación almacenamiento/cómputo: el cálculo usa los primeros POOL_COMPUTE_LIMIT
  // (= updated_at DESC, mismos que Ola 1). _pool usa el array completo (POOL_STORE_LIMIT).
  const computeOffers = offers.slice(0, POOL_COMPUTE_LIMIT);
