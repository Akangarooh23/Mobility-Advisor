...
function buildIcs(booking) {
  const lines = [
    ...
    `ORGANIZER;CN=${MARCA.nombre}:mailto:citas@carswiseai.com`,
    "STATUS:CONFIRMED",
    "END:VEVENT",
    "END:VCALENDAR",
  ];
...
async function sendBookingEmails(booking, opts = {}) {
  ...
  const buyerHtml = plantilla({
    titulo: isReschedule ? 'Tu cita se ha movido' : pendiente ? 'Hemos recibido tu solicitud' : 'Tu cita está confirmada',
    cuerpo:
...
async function bookSlot({ slotId, offerId, vehicleTitle, buyerEmail, buyerName, buyerPhone, notes, source }) {
  ...
  try {
    ...
    // Un hueco que nadie publicó no puede dar una cita confirmada.
    //
    // Cuando una oferta no tiene horarios, el sistema se los inventa —L a V de
    // 9 a 18— para que se pueda pedir visita igualmente. Pero eso es una
    // propuesta, no un acuerdo: el concesionario puede cerrar ese día. Guardarla
    // como «confirmed» y mandarle al cliente el .ics es prometerle algo que
    // nadie ha dicho. Queda «pending» hasta que alguien la confirme desde el
    // ERP; si el hueco lo publicó una persona, sigue naciendo confirmada.
    const estado = slot.source === "auto" ? "pending" : "confirmed";
...
module.exports = async function visitAvailabilityHandler(req, res) {
  ...
  if (method === "POST" && route === "book") {
    ...
    const { slotId, offerId, vehicleTitle, buyerEmail, buyerName, buyerPhone, notes, source } = body;
