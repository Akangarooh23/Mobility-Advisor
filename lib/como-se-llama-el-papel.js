 ...
 *
 * Se lleva por delante el caso raro de quien llama a su fichero «Seguro de mi
 * coche.pdf»: pierde ese título, pero lo que dice ya lo dice el nombre nuevo.
 */
function yaSeLoPusimos(nombre) {
  const base = sinExtension(nombre).trim();
  return Object.values(COMO_SE_LLAMAN).some((etiqueta) => base.startsWith(etiqueta));
}
