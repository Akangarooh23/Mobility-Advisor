...
const FORMAS_DEL_COMBUSTIBLE = {
  ...
  // El motor llama «microhibrido» al MHEV; sin esto se buscaba esa palabra
  // tal cual en la columna y no aparece en ningun anuncio.
  microhibrido: ["hibrid", "híbrid", "hybrid", "mhev"],
...
function formasDe(tabla, valor) {
  /*
   * Varias a la vez, porque la motorización se pregunta con casillas.
   *
   * Quien marca «gasolina» e «híbrido» estaba quedándose SIN FILTRO DE
   * COMBUSTIBLE en la base: había una regla que decía «con varias preferencias
   * no sobre-constriñas» y devolvía cadena vacía. La intención era buena y el
   * efecto el contrario: la consulta se traía de todo —eléctricos incluidos— y
   * un filtro de JavaScript los tiraba después, sobre una ventana pequeña. El
   * resultado, medido contestando el test entero, era CERO OFERTAS teniendo el
   * pool 75 que cumplían.
   *
   * Dos combustibles son dos combustibles: se buscan los dos.
   */
  if (Array.isArray(valor)) {
    const todas = valor.flatMap((v) => formasDe(tabla, v) || []);
    return todas.length ? [...new Set(todas)] : null;
  }
...
function condicionesDelConsejero(criterios = {}, desde = 0, losQueHayEnLaBase = null) {
  ...
  /*
   * O la familia de marcas que ha elegido.
   *
   * Esto solo se filtraba en JavaScript sobre las ofertas ya traidas, asi que
   * la consulta se traia marcas descartadas y se tiraban despues. Y la lista
   * de modelos recomendados no lo sabia: a quien pidio «generalista europea»
   * le proponia un Ford Focus y un Toyota Yaris, que el filtro tiraba justo
   * despues. Ver lib/las-marcas-que-quiere.js.
   */
  const marcas = Array.isArray(criterios.brands) ? criterios.brands.map((m) => plano(m)).filter(Boolean) : null;
  if (marcas && marcas.length) condiciones.push(`lower(COALESCE(brand,'')) = ANY(${parametro(marcas)}::text[])`);

  /*
   * Varios combustibles si ha marcado varios. Ver formasDe: con dos marcados
   * antes se quedaba sin filtro de combustible en la base.
   */
  porFormas("COALESCE(fuel,'')", FORMAS_DEL_COMBUSTIBLE, criterios.fuels || criterios.fuel, "fuel");
