...
function buildReportData(vehicle, national, damageFactor = null, referenceDate = null) {
  ...
  // SHADOW — cross-check market median vs depreciation (log only, no behavior change)
  // Activation: replace log with comparables=0 guard once shadow run confirms no false positives
  if (!usedFallback && median > 0) {
    const shadowEst = estimatePriceByDepreciation(vehicle, refDate);
    if (shadowEst && shadowEst.optimal > 0) {
      const ratio = median / shadowEst.optimal;
      if (ratio < 0.40 || ratio > 2.50) {
        console.warn(`[RATIO_CHECK] SHADOW ratio=${ratio.toFixed(2)} brand=${vehicle.brand} model=${vehicle.model} n=${comps} median=${median} est=${shadowEst.optimal}`);
      }
    }
  }
  ...
  // Combined factor = df × color × owner, with a floor of 0.72 to protect against
  ...
  return {
    vehicle,
    priceOptimal:    Math.max(0, Math.round((base     + totalAdj) * effectiveFactor)),
    priceLow:        Math.max(0, Math.round((baseLow  + Math.round(totalAdj * 0.70)) * effectiveFactor)),
    priceHigh:       Math.max(0, Math.round((baseHigh + Math.round(totalAdj * 1.25)) * effectiveFactor)),
    comparables:     comps,
...
async function buildPdf(rd) {
    ...
    : [
        {
          ...
          n: rd.usageUsedDefault
            ? `${vKm != null ? new Intl.NumberFormat("es-ES").format(vKm) + " km · " : ""}${vAge != null ? vAge + " anos · " : ""}estimado por segmento${rd.usageCapApplied ? " · tope de segmento" : ""}`
            : (rd.usageImpact > 50  ? `Km y antiguedad favorables vs. mercado${rd.usageCapApplied ? " · tope de segmento" : ""}`
               : rd.usageImpact < -50 ? `Km elevado o antiguedad alta vs. mercado${rd.usageCapApplied ? " · tope de segmento" : ""}`
  ...
  const methodBase = rd.usedFallback
    ...
    : "Datos en tiempo real de los principales portales de venta de VO en Espana. Precio optimo = mediana de comparables con fence IQR x1.5 (Tukey) para eliminar atipicos. Ajuste por uso derivado de regresion multiple (km + antiguedad) sobre los propios comparables; si n<15 se usan pendientes de referencia por segmento.";
...
async function generateSellReport(vehicle = {}) {
  ...
  console.log("[SELL_REPORT]", JSON.stringify({
    ...
    effectiveFactor: Math.max(0.72, reportData.damageFactor * (reportData.colorAdj?.factor ?? 1) * (reportData.ownerAdj?.factor ?? 1)),
  }));
...
module.exports = { generateSellReport, buildReportData, BRAND_TIERS };
