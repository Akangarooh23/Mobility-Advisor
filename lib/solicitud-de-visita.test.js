...
describe("el correo que se manda", () => {
  const correo = S.elCorreoDeConfirmacion({
    vehicleTitle: "Seat Ibiza 2019", dia: "jueves, 12 de septiembre de 2026", franja: "de 10:00 a 14:00",
    enlace: "https://popcar.es/confirmar-visita?t=abc",
  });

  test("con la maqueta de PopCar, no un párrafo suelto", () => {
    // Es el primer correo que recibe alguien que no nos conoce: pelado, parece
    // un engaño y acaba en spam.
    assert.match(correo.html, /<!doctype html>/i);
    assert.ok(correo.html.includes("Confirmar la visita</a>"), "no lleva el botón");
  });

  test("con la franja entera, no una hora suelta", () => {
    assert.match(correo.html, /de 10:00 a 14:00/);
    assert.doesNotMatch(correo.html, /a las 10:00/);
  });
