...
function estimatePriceByDepreciation(vehicle) {
  const age = vehicle.year ? new Date().getFullYear() - Number(vehicle.year) : null;
...
function buildReportData(vehicle, national, damageFactor = null) {
  ...
  if (comps >= 3 && median > 0) {
    ...
    usedFallback = true;
    const est = estimatePriceByDepreciation(vehicle);
  ...
  // Color liquidity factor (applied after all other adjustments)
  const colorAdj = getColorAdj(vehicle.color || "");

  return {
    vehicle,
    priceOptimal:    Math.max(0, Math.round((base     + totalAdj) * colorAdj.factor)),
    priceLow:        Math.max(0, Math.round((baseLow  + Math.round(totalAdj * 0.70)) * colorAdj.factor)),
    priceHigh:       Math.max(0, Math.round((baseHigh + Math.round(totalAdj * 1.25)) * colorAdj.factor)),
    comparables:     comps,
    ...
    priceTrend:      national.market?.priceTrend || null,
