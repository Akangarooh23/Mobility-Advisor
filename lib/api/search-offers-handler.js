...
async function facetaMarcas(pool, q) {
  ...
  const [alias, grafias, principales] = await Promise.all([
    aliasDeMarcas(pool), mapaGrafias(pool), clavesPrincipales(pool),
  ]);
  const agrupadas = agrupar(bruto.rows, alias, grafias);
  ...
  for (const c of agrupadas) {
    ...
    if (k) { conClave.add(k); porClave.set(k, c); }
  }

  /* Las principales salen arriba y ya no vuelven a salir abajo. Se recorre la
   * lista, no lo agrupado, para que una principal sin ninguna oferta siga
   * apareciendo con un cero: es principal por decision, no por tener stock. */
  ...
  const otras = agrupadas.filter((c) => {
    const k = grafias.porGrafia.get(c.nombre.toLowerCase());
    return !k || !principalesClaves.has(k);
  });
