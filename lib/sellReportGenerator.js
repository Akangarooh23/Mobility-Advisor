...
VEHÃCULO: ${v.brand || ""} ${v.model || ""} ${v.version || ""} | AÃ±o ${v.year || "-"} | ${v.mileage ? new Intl.NumberFormat("es-ES").format(v.mileage) + " km" : "km no indicados"} | ${v.fuel || "combustible no indicado"}${damageCtx}
...
async function buildPdf(rd) {
  ...
  if (validSamples.length > 0 && !rd.usedFallback) {
    ...
    validSamples.slice(0, 5).forEach(function(s, i) {
      ...
      dt(p3, (s.version || s.model || "-").slice(0, 22),                                      MX + 248, y + 6, 7.5, ib, GRAY, 130);
  ...
  if (validSamples4.length > 0 && !rd.usedFallback) {
    ...
    validSamples4.slice(0, 5).forEach(function(s, i) {
      ...
      dt(p4, s.mileage ? new Intl.NumberFormat("es-ES").format(s.mileage) + " km" : "-", MX + 100, y + 6, 8.5, ib, GRAY);
      dt(p4, (s.version || s.model || "-").slice(0, 22), MX + 196, y + 6, 7.5, ib, GRAY, 150);
      dt(p4, (s.portal || "-").slice(0, 18), MX + 360, y + 6, 7.5, ib, TEAL, 100);
      y += 22;
    });
    y += 6;
...
async function generateSellReport(vehicle = {}) {
  ...
  // Diagnostic tuple - logged on every production tasaciÃ³n for observability
  const mkt = national.market || {};
