 ...
 *
 * La distinción sale del prefijo de la clave y no de una variable aparte, para
 * que no se pueda quedar mal puesta: el día que se ponga una clave real, la
 * tarjeta desaparece sola.
 */
function formasDePago(stripeSecretKey) {
  const enPruebas = String(stripeSecretKey || "").startsWith("sk_test_");
  const transferencia = {
    ...
    "payment_method_options[customer_balance][bank_transfer][eu_bank_transfer][country]": "ES",
  };
  if (!enPruebas) {
