 ...
 */
function unaFilaPorCoche(columnas, filtro, limite) {
  const grupo = "COALESCE(d.canonical_id, moveadvisor_market_offers.id)";
  return `
    SELECT * FROM (
      SELECT DISTINCT ON (${grupo})
        ${columnas},
        d.ubicaciones AS _ubicaciones,
        scraped_at AS _scraped_at
      FROM moveadvisor_market_offers
      LEFT JOIN moveadvisor_offer_duplicates d
        ON d.offer_id = moveadvisor_market_offers.id
      WHERE ${filtro}
      ORDER BY
        ${grupo},
        (moveadvisor_market_offers.id = d.canonical_id) DESC NULLS LAST,
        updated_at DESC NULLS LAST,
        scraped_at DESC NULLS LAST,
        moveadvisor_market_offers.id
    ) coches
