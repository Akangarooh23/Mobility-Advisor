...
before(() => {
  Pool.prototype.query = async function (sql, p = []) {
    ...
    // Desde que añadir y borrar franjas pide sesión, el manejador pregunta
    // antes de nada si el coche es de quien lo pide. Aquí lo es.
    if (/FROM moveadvisor_user_vehicles/.test(t)) return r([{ ok: 1 }]);
...
async function quita(slotId) {
  // El correo va en la consulta porque fuera de producción la identidad se
  // admite así —`AUTH_BILLING_REQUIRE_SESSION=false`, arriba—, que es lo que
  // permite probar con curl. En producción manda la cookie de sesión.
  const req = {
    method: "DELETE", headers: {}, body: {},
    query: { route: "delete_slot", slotId, offerId: OFERTA, email: "ana@example.com" },
  };
