...
module.exports = async function billingAccountHandler(req, res) {
  ...
  if (req.method === "GET") {
    ...
    const pgPlan = await getPlanFromPostgres(email);
    if (pgPlan) {
    ...
    // If status is ambiguous (checkout_abierto or unknown), verify with Stripe and auto-heal
    const currentStatus = normalizeText(account?.billingState?.status).toLowerCase();
    const KNOWN_TERMINAL_STATUSES = new Set(["activa", "trialing", "pendiente", "inactivo", "cancelado", "active", "activo"]);
    const stripeKey = normalizeText(process.env.STRIPE_SECRET_KEY);
    if (!KNOWN_TERMINAL_STATUSES.has(currentStatus) && stripeKey) {
      try {
