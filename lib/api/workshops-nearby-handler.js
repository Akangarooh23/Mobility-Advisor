...
function workshopShape(w) {
  return {
    ...
    phone: w.phone || null,
    serviceTypes: Array.isArray(w.service_types) ? w.service_types : [],
...
async function queryNearbyWorkshops(userLat, userLon) {
  ...
  try {
    const [{ rows: partnerRows }, { rows: indRows }] = await Promise.all([
      client.query(
        `SELECT id, name, address, city, postcode, province, lat, lon, partner, phone, service_types, rating, rating_count
...
async function queryByBbox(latMin, latMax, lonMin, lonMax) {
  ...
  try {
    ...
    return addDistance(rows, centerLat, centerLon);
  } finally {
    client.release();
  }
}
...
module.exports = async function workshopsNearbyHandler(req, res) {
  ...
  // ── Bbox mode (map "Buscar en esta zona") ──────────────────────────────────
  const bLatMin = parseFloat(req.query?.bLatMin || "");
  const bLatMax = parseFloat(req.query?.bLatMax || "");
  const bLonMin = parseFloat(req.query?.bLonMin || "");
  const bLonMax = parseFloat(req.query?.bLonMax || "");
  const hasBbox = [bLatMin, bLatMax, bLonMin, bLonMax].every((v) => !isNaN(v));

  if (hasBbox) {
    try {
      const rows = await queryByBbox(bLatMin, bLatMax, bLonMin, bLonMax);
      const centerLat = (bLatMin + bLatMax) / 2;
      const centerLon = (bLonMin + bLonMax) / 2;

      const partnersByKey = {};
      const indRows = [];
      for (const w of rows) {
        if (w.partner !== "independent") {
      ...
      const partnerProviders = PARTNER_ORDER.filter((k) => partnersByKey[k]).map((k) => ({
        providerKey: k,
        providerName: PARTNER_LABELS[k] || k,
        available: true,
        workshop: workshopShape(partnersByKey[k]),
      }));

      const indProviders = indRows
        .filter((w) => w.name.trim().length > 2)
        .slice(0, 100)
        .map((w) => ({
      ...
      const allProviders = [...partnerProviders, ...indProviders].sort(
        (a, b) => (a.workshop.distanceKm ?? 9999) - (b.workshop.distanceKm ?? 9999)
      );

      return res.status(200).json({
        ok: true,
        search: { source: "bbox" },
        userLocation: { lat: centerLat, lon: centerLon, source: "bbox" },
        providers: allProviders,
      });
    } catch (err) {
      console.error("[workshops-nearby] bbox DB error:", err.message);
      return res.status(500).json({ error: "DB error" });
    }
  }

  // ── Normal mode (by CP / geolocation) ─────────────────────────────────────
  ...
  const lonParam   = parseFloat(req.query?.lon || "");
  const hasPrecise = !isNaN(latParam) && !isNaN(lonParam);

  if (!hasPrecise && !/^\d{5}$/.test(postalCode)) {
    return res.status(400).json({ error: "postalCode must be a 5-digit code" });
  }

  let userLocation;
  if (hasPrecise) {
    const fallback = resolveUserLocation(postalCode, province);
    userLocation = {
      lat: latParam,
      lon: lonParam,
      province: fallback?.province || province,
      source: "precise_geocode",
    };
  } else {
    userLocation = resolveUserLocation(postalCode, province);
  }
  ...
  return res.status(200).json({
    ...
    userLocation: { lat: userLocation.lat, lon: userLocation.lon, source: userLocation.source },
    providers: allProviders,
  });
};
