...
async function uploadInvoicePdfAndSaveUrl(invoiceId, pdfBuffer, invoiceNumber) {
  try {
    ...
    const path = `invoices/${safeNum}.pdf`;
...
async function generateInvoicePdf(invoiceRecord, userProfile = {}) {
  ...
  dt(`${MARCA.sitio} · ${MARCA.correoSoporte}`, 66, 76, 8.5, ri, c(COLOR.textoSuave));
...
async function sendInvoiceEmail({ to, pdfBuffer, invoiceRecord }) {
  ...
  const from   = normalizeText(process.env.RESEND_FROM_EMAIL) || MARCA.remitentePorDefecto;
  ...
  const html = `
    <div style="font-family:sans-serif;max-width:520px;margin:0 auto;padding:32px 24px;color:#111111">
      <div style="background:#111111;padding:20px 24px;border-radius:8px 8px 0 0;border-bottom:3px solid #FFCC00">
        <span style="font-size:18px;font-weight:800;letter-spacing:-0.5px"><span style="color:#FFCC00">Pop</span><span style="color:#fff">Car</span></span>
      </div>
      <div style="border:1px solid #E4E4DF;border-top:none;border-radius:0 0 8px 8px;padding:28px 24px">
        <h2 style="margin:0 0 8px;font-size:20px">Tu factura está lista</h2>
        <p style="color:#5E5E59;margin:0 0 24px">Adjunto encontrarás la factura en PDF correspondiente a tu compra.</p>
        <table style="width:100%;border-collapse:collapse;margin-bottom:24px">
          <tr style="background:#F7F7F3"><td style="padding:8px 12px;font-size:12px;color:#96968F;font-weight:700">N.º FACTURA</td><td style="padding:8px 12px;font-size:12px;color:#96968F;font-weight:700">CONCEPTO</td><td style="padding:8px 12px;font-size:12px;color:#96968F;font-weight:700">TOTAL</td></tr>
          <tr style="border-bottom:1px solid #E4E4DF"><td style="padding:10px 12px;font-weight:600">${invoiceNum}</td><td style="padding:10px 12px">${concept}</td><td style="padding:10px 12px;font-weight:700;color:#111111">${amount} EUR</td></tr>
        </table>
        <p style="font-size:12px;color:#96968F;margin:0">${MARCA.nombre} · <a href="${MARCA.sitioUrl}" style="color:#96968F">${MARCA.sitio}</a></p>
      </div>
    </div>`;
...
async function sendValuationEmail({ to, pdfBuffer, reportData, vehicle: vehFallback }) {
  ...
  const html = `
    <div style="font-family:Inter,Arial,sans-serif;max-width:600px;margin:0 auto;color:#111111;">
      <div style="background:#111111;padding:28px 32px 24px;border-bottom:4px solid #FFCC00;">
        <span style="font-size:20px;font-weight:800;letter-spacing:-0.5px;"><span style="color:#FFCC00">Pop</span><span style="color:#fff">Car</span></span>
      ...
      <div style="padding:28px 32px;">
        ...
        <div style="background:#FFF6D2;border:1.5px solid #FFCC00;border-radius:12px;padding:20px 24px;margin-bottom:24px;">
  ...
  const payload = {
    ...
    html,
    ...(pdfBuffer ? { attachments: [{ filename: `Informe_Tasacion_${MARCA.nombre}.pdf`, content: pdfBuffer.toString("base64") }] } : {}),
  };
...
async function sendFleetEmail({ to, reports, vehicles }) {
    ...
    <div style="font-family:Inter,Arial,sans-serif;max-width:600px;margin:0 auto;color:#111111;">
      ...
      <div style="padding:28px 32px;">
        ...
        <table style="width:100%;border-collapse:collapse;margin-bottom:24px;background:#fafafa;border-radius:10px;overflow:hidden;border:1px solid #e5e7eb;">
          <thead><tr style="background:#FFF6D2;">
  ...
  const payload = { from, to: [to], subject: `Tus ${count} informes de tasación — ${MARCA.nombre}`, html };
