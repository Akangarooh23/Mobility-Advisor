...
function normalizeOffer(raw = {}) {
  ...
  const province = normalizeText(firstDefined(raw, ["province", "provincia"]));
  const city = normalizeText(firstDefined(raw, ["city", "ciudad"]));
  ...
  const image = normalizeText(firstDefined(raw, ["image", "imagen_principal_url", "image_url"]));
...
async function readPostgresInventory(limit = 4000) {
  ...
  try {
    const query = `
      ...
      LIMIT $1
    `;
    const result = await pool.query(query, [Math.max(100, Math.min(limit, 10000))]);
    return (result.rows || []).map(normalizeOffer).filter(Boolean);
  } catch {
    return [];
  }
}

async function readSqlServerInventory(limit = 4000, filters = {}) {
  ...
  const boundedLimit = Math.max(100, Math.min(limit, 10000));
...
async function readInventoryUniverse(options = {}) {
  ...
  if (trySqlServerFirst) {
    const fromSqlServer = await readSqlServerInventory(4000, options);
