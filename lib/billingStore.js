...
async function addGarageVehicleByEmailPostgres(email = "", vehicle = {}) {
  ...
  for (const photo of photoRows) {
    ...
    const fileUrl   = b64 ? (await uploadBase64ToSupabase(b64, mimeType, storagePath) || '') : '';
    await pool.query(
...
async function listVehicleStatesByEmailPostgres(email = "") {
  ...
  const result = await pool.query(
    `
      ...
      WHERE (s.user_id = $1 OR (s.user_id IS NULL AND s.user_email = $2))
...
async function getUserMobilityDataByEmailPostgres(email = "") {
  ...
  try {
    ...
    const maintenances  = Array.isArray(row.maintenances) ? row.maintenances : [];
    const insurances    = Array.isArray(row.insurances)   ? row.insurances   : [];
    const valuations    = Array.isArray(row.valuations)   ? row.valuations   : [];
    const vehicleStates = Array.isArray(row.vehicle_states) ? row.vehicle_states : [];
    const savedOffers   = Array.isArray(row.saved_offers) ? row.saved_offers : [];
    const leads         = Array.isArray(row.leads)        ? row.leads        : [];
    const views         = Array.isArray(row.views)        ? row.views        : [];
    const solicitudes   = [...leads, ...views].sort((a, b) => new Date(b.created_at || 0) - new Date(a.created_at || 0)).slice(0, 20);
    ...
    return { vehicles, appointments, maintenances, insurances, valuations, vehicleStates, savedOffers, solicitudes, garageVehicleCount: vehicles.length, planId: "free" };
  }
}
