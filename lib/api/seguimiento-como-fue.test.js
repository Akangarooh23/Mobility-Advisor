...
// El aviso de verdad pide Firebase y la base; aqui solo interesa si sale y a quien.
let avisos = [];
...
let conTestigo = true;
/** Si el coche es de un particular: entonces la pregunta es «¿te lo quedas?». */
let deParticular = false;
  ...
  Pool.prototype.query = async (sql) => {
    ...
    if (/FROM vehicle_visit_bookings/i.test(t) && /followup_sent_at IS NULL/i.test(t)) {
      return {
        rows: [{
          ...RESERVA,
          token_buyer: conTestigo ? RESERVA.token_buyer : null,
          offer_id: deParticular ? "idcar-veh-1" : "erp-9",
          seller_email: deParticular ? "vendedor@example.com" : null,
        }],
        rowCount: 1,
      };
    }
...
describe("el correo de «¿qué tal fue?»", { concurrency: 1 }, () => {
  test("sale, y con las tres respuestas", async () => {
    ...
    for (const texto of ["No fui", "Lo vi y no me lo quedé", "Me lo quedé"]) {
  ...
  test("ninguna respuesta va destacada sobre las otras", async () => {
     ...
     */
    const html = (await corre(), elSeguimiento().html);
    const trozo = html.slice(html.indexOf("No fui") - 400, html.indexOf("Me lo quedé") + 200);
...
describe("y en el coche de un particular, «¿te lo quedas?»", { concurrency: 1 }, () => {
  const elDeQuedarselo = () => correos.find((c) => /te lo quedas/i.test(c.subject));

  test("la pregunta es otra, y una de las salidas es comprarlo", async () => {
    deParticular = true;
    await corre();
    const c = elDeQuedarselo();
    assert.ok(c, "no ha salido el correo de «¿te lo quedas?»");
    assert.ok(c.html.includes("Quiero comprarlo"), "falta la salida que empieza la compra");
    assert.match(c.html, /\/quiero-comprarlo\?id=b-1&amp;token=t-buena/);
  });

  test("y suena en el móvil, que de esa respuesta sale la compra", async () => {
    /*
     * El seguimiento normal no suena a propósito —«¿qué tal fue?» no es algo
     * que haya que saber con el móvil sonando—, pero aquí el coche se le
     * reserva a quien conteste y los demás se quedan sin él.
     */
    deParticular = true;
    await corre();
    const aviso = avisos.find((a) => /te lo quedas/i.test(a.titulo));
    assert.ok(aviso, "no ha sonado");
    assert.deepEqual(aviso.a, ["cliente@example.com"]);
    assert.equal(aviso.datos.pantalla, "visitas");
  });

  test("el de un concesionario sigue sin sonar", async () => {
    await corre();
    assert.equal(avisos.length, 0, "ha sonado un seguimiento que no lo necesita");
  });
});
