  ...
  try {
    const result = await pool.query(`
      WITH
        ...
        -- suya.
        citas AS (
          SELECT id, offer_id, vehicle_title, starts_at, ends_at, status,
                 token_buyer, notes, created_at
    ...
    const mappedCitas = (Array.isArray(row.citas) ? row.citas : []).map((r) => ({
      ...
      status: ESTADO_DE_CITA[s(r.status)] || s(r.status),
      createdAt: r.created_at ? new Date(r.created_at).toISOString() : "",
    }));

    const solicitudes = [...mappedLeads, ...mappedViews, ...mappedCitas]
