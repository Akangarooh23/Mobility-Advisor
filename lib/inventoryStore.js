  ...
  try {
    ...
    if (safeTokens.length > 0) {
      ...
      const presentables = criterios && criterios.soloPresentables
        ...
        : [];
      const targetedQuery = unaFilaPorCoche(
        selectCols,
        [`(${conditions.join(" OR ")})`, "COALESCE(is_active, TRUE) = TRUE", ...presentables].join(" AND "),
        "$1",
...
async function readInventoryUniverse(options = {}) {
  ...
  const criterios = {
    ...
    environmentalLabel: options.environmentalLabel,
    minPrice: options.minPrice,
    maxPrice: options.maxPrice,
    minYear: options.minYear,
    maxYear: options.maxYear,
    maxMileage: options.maxMileage,
    province: options.province || options.location,
    // Las escrituras de la provincia elegida en el test.
    provinciaFormas: options.provinciaFormas || null,
    // Lo pide el consejero, no la tasacion: ver lo-que-busca-el-consejero.js.
    soloPresentables: Boolean(options.soloPresentables),
  };
