...
async function getMarketPriceSnapshot(options = {}) {
  ...
  // 1st try: exact (brand + model + fuel + year)
  let { offers, source, totalUniverse } = await listInventoryOffers({ ...baseQuery, fuel, targetYear: year });

  // 2nd try: relax fuel filter
  if (!offers.length && fuel) {
    ({ offers, source, totalUniverse } = await listInventoryOffers({ ...baseQuery, fuel: "", targetYear: year }));
  }

  // 3rd try: relax fuel + year (brand + model only)
  if (!offers.length) {
