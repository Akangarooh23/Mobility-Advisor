...
const PorMatricula = require("../coche-por-matricula");
...
function getStatsPool() {
  ...
  _statsPgPool = new Pool({ connectionString: connString, ssl: SSL_POSTGRES });
  return _statsPgPool;
}
...
module.exports = async function marketplaceVoHandler(req, res) {
  ...
  if (req?.query?.stats === "1" && vehicleId) {
    ...
    // Sin base no hay cuenta posible, y decir "0 visitas" seria afirmar algo
    // que nadie ha contado.
    if (!pool) {
      return res.status(200).json({ ok: false, stats: null, warning: "Sin conexion a la base: no se puede contar." });
    }

    try {
      // Sin `.catch()` sustituyendo ceros.
      //
      // Habia dos, uno por consulta, y entre los dos se tragaban cualquier
      // fallo antes de que llegara al catch de abajo — que era el unico que
      // podia declararlo. El resultado: un vendedor veia "0 visitas, 0
      // contactos" con la misma cara tanto si nadie habia mirado su anuncio
      // como si la consulta habia reventado.
      //
      // Ahora un fallo sube. Se pierde el dato que si funciono, y es el
      // intercambio correcto: media verdad presentada como completa es
      // exactamente lo que se esta quitando.
      ...
      // "0 visitas" y "no he podido contarlas" no son lo mismo, y para un
      // vendedor mirando su anuncio la diferencia importa: lo primero le dice
      // que nadie lo ve.
      //
      // El consumidor ya hace `if (d.ok) setOfferStats(d.stats)`, así que con
      // ok:false simplemente no pinta nada — que es lo correcto cuando no se
      // sabe.
      console.error("[marketplace-vo] stats query failed:", err?.message);
      return res.status(200).json({
        ok: false,
        stats: null,
        warning: err?.message || "No se pudieron contar las visitas.",
      });
    }
  }
  ...
  /*
   * Por matrícula: es lo que resuelve la dirección corta de los anuncios.
   *
   * Los tres desenlaces son distintos y hay que poder contarlos, así que se
   * devuelven con códigos distintos: 200 con el coche, 404 cuando ya no está a
   * la venta —que le va a pasar a todo el que pulse el enlace después de que se
   * venda— y 400 cuando lo que llega ni siquiera es una matrícula.
   */
  const matricula = req?.query?.plate ? String(req.query.plate) : null;
  if (matricula) {
    if (!PorMatricula.pareceUnaMatricula(matricula)) {
      return res.status(400).json({ ok: false, offer: null, error: "no_parece_una_matricula" });
    }
    try {
      const pool = getStatsPool();
      if (!pool) return res.status(500).json({ ok: false, offer: null, error: "No DB" });
      const r = await pool.query(PorMatricula.SQL_POR_MATRICULA, [
        PorMatricula.comoSeCompara(matricula),
      ]);
      const oferta = PorMatricula.laDelAnuncio(r.rows);
      if (!oferta) return res.status(404).json({ ok: false, offer: null, error: "ya_no_esta_a_la_venta" });
      return res.status(200).json({ ok: true, offer: oferta });
    } catch (err) {
      return res.status(500).json({ ok: false, offer: null, error: err?.message });
    }
  }
