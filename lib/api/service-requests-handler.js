...
async function sendConfirmationEmail(userEmail, request) {
  ...
  const html = `
    <div style="font-family:Arial,sans-serif;max-width:520px;margin:0 auto;color:#1a1a2e">
      <div style="background:#0f172a;padding:24px 32px;border-radius:12px 12px 0 0">
        ...
        <p style="color:#94a3b8;margin:4px 0 0;font-size:14px">${MARCA.nombre} — Tarifa profesional</p>
      </div>
      <div style="background:#f8fafc;padding:24px 32px;border-radius:0 0 12px 12px;border:1px solid #e2e8f0">
        ...
        <div style="background:#eff6ff;border:1px solid #bfdbfe;border-radius:8px;padding:14px 16px;margin-top:8px">
          <p style="margin:0;font-size:13px;color:#1e40af">
            <strong>Tarifa profesional ${MARCA.nombre}:</strong> Como usuario de ${MARCA.nombre} accedes a precios negociados para flotas y profesionales, normalmente entre un 15% y 30% por debajo del precio de mostrador.
          </p>
        </div>
        <p style="color:#94a3b8;font-size:12px;margin-top:20px">Ref: ${request.id} · ${MARCA.nombre} Servicios Profesionales</p>
      </div>
    </div>
  `;
  ...
  await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      from: process.env.ALERT_EMAIL_FROM || process.env.RESEND_FROM_EMAIL || MARCA.remitentePorDefecto,
      to: [userEmail],
...
async function sendInternalAlert(userEmail, request) {
  ...
  await fetch("https://api.resend.com/emails", {
    ...
    headers: { Authorization: `Bearer ${apiKey}`, "Content-Type": "application/json" },
    body: JSON.stringify({
      from: process.env.ALERT_EMAIL_FROM || MARCA.remitentePorDefecto,
      to: [internalEmail],
