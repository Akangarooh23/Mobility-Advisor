...
// ── Factor de cierre por tramo de precio de publicación ───────────────────────
...
async function buildPdf(rd) {
  ...
  // Closing price strip — capa final, separada del precio de publicación.
  // Solo cuando el precio cae en economy o mainstream (tramos con muestra fiable).
  if (rd.priceClose) {
    const csY = y + verdictH + 6;
    fr(p1, MX, csY, CW, 28, hex("#F7F7F7"), BORDER, 0.5);
    dt(p1, "PRECIO ESTIMADO DE CIERRE", MX + 14, csY + 8, 6.5, rb, GRAY);
    dt(p1, eur(rd.priceClose),
       MX + 14 + rb.widthOfTextAtSize("PRECIO ESTIMADO DE CIERRE", 6.5) + 12,
       csY + 8, 10, rb, INK);
    dt(p1, "orientativo \xB7 anuncios de Flexicar que salieron del mercado",
       MX + 14, csY + 20, 6, ib, LIGHT);
  }

  y += hasCompData ? 200 : 166;
  if (rd.priceClose) y += 34;
