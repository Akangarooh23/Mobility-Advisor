...
describe("reservar sobre un horario publicado por una persona", () => {
  test("la reserva nace confirmada, como siempre", async () => {
    await reserva("erp");
    assert.equal(estadoGuardado, "confirmed");
  });

  test("y sí lleva el calendario", async () => {
    await reserva("erp");
    const suyo = correos.find((c) => String(c.to).includes("cliente@example.com"));
    assert.ok(/confirmada/i.test(suyo.subject));
    assert.ok(suyo.attachments && suyo.attachments.length === 1);
  });
});
