...
async function listGarageVehiclesByEmailPostgres(email = "") {
  ...
  const stateByVehicleId = (statesResult.rows || []).reduce((acc, row) => {
    if (row.vehicle_id) {
      // If is_listed=true, expose as active_sale so frontend publish toggle shows correctly
      acc[String(row.vehicle_id)] = row.is_listed === true ? 'active_sale' : String(row.state || 'owned');
    }
    return acc;
  }, {});
...
async function listVehicleStatesByEmailPostgres(email = "") {
  ...
  const result = await pool.query(
    `
      SELECT s.vehicle_id, s.state, COALESCE(s.is_listed, false) AS is_listed, s.listing_url, s.notes, s.updated_at,
  ...
  try {
    const result = await pool.query(`
      WITH
        ...
        vstates AS (
          SELECT s.vehicle_id, s.state, COALESCE(s.is_listed, false) AS is_listed,
