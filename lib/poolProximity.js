...
/* ------------------------------------------------------------------ *
 * Ponderación del pool por cercanía — Ola 2, paso 2
 *
 ...
 *
 *   1. alpha   — kernel gaussiano de proximidad. Elige, dentro de cada
 *                lado, los comparables más parecidos. alpha=0 → inerte.
 *   2. balance — cuota por lado (más antiguos / más nuevos que el sujeto).
 *                Es lo que CENTRA el pool. balance=false → inerte.
 *
 * Con { alpha: 0, balance: false } la salida es idéntica al comportamiento
 * actual: se puede mergear en oscuro y verificar 0 DRIFT antes de encender.
 *
 * Por qué el kernel solo no basta: la oferta del mercado es asimétrica
 * (muchos más 2023 que 2017). Una penalización simétrica sobre un
 * suministro asimétrico sigue dando un pool descentrado, solo que además
 * estrecho — peor que el actual. La cuota es lo que arregla el centrado.
 * ------------------------------------------------------------------ */

const DEFAULT_OPTS = {
  alpha: 0,             // 0 = sin kernel. Barrer {0, 0.5, 1, 2} en shadow.
...
/** Fracción del pool con valor <= v. 0.5 = el sujeto está en la mediana. */
...
/**
 * Peso gaussiano en (0, 1]. Simétrico y suave: no corta, atenúa.
 * alpha=0 → 1 para todos (inerte). alpha alto → pool estrecho (peligro).
 */
function proximityWeight(offer, user, o) {
  ...
  return Math.exp(-o.alpha * (dkm * dkm + dyr * dyr));
}
...
function selectBalancedPool(candidates, user, options = {}) {
  ...
  if (!o.alpha && !o.balance) {
    const pool = candidates.slice(0, o.targetSize);
    return {
      pool,
      diagnostics: poolDiagnostics(pool, user, o),
      applied: { alpha: 0, balance: false, relaxed: false, kmScale: o.kmScale },
    };
  }

  const scored = candidates.map((c) => {
    const base = o.relevanceKey ? (c[o.relevanceKey] ?? 1) : 1;
    return { offer: c, score: base * proximityWeight(c, user, o) };
  });
  ...
  if (!o.balance) {
    ...
    return {
      pool,
      diagnostics: poolDiagnostics(pool, user, o),
      applied: { alpha: o.alpha, balance: false, relaxed: false, kmScale: o.kmScale },
    };
  }
  ...
  return {
    ...
    diagnostics: poolDiagnostics(pool, user, o),
    applied: {
      alpha: o.alpha, balance: true, relaxed, kmScale: Math.round(o.kmScale),
...
function poolDiagnostics(pool, user, options = {}) {
  ...
  const n = pool.length;
  const r = pearson(
    kms.slice(0, Math.min(kms.length, yrs.length)),
    yrs.slice(0, Math.min(kms.length, yrs.length)),
  );
 ...
 * sin aplicar ninguna. Se loguea y se elige alpha con datos, no a ojo.
 */
function sweepDiagnostics(candidates, user, configs, options = {}) {
  const list = configs || [
    { alpha: 0,   balance: false },  // actual
    { alpha: 0,   balance: true  },  // solo cuota
    { alpha: 0.5, balance: true  },
    { alpha: 1,   balance: true  },
    { alpha: 2,   balance: true  },  // agresivo: vigilar caída de varianza
  ];
  return list.map((cfg) => {
