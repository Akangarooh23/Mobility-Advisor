...
function eur(value) {
  ...
  return new Intl.NumberFormat("es-ES").format(Math.round(value)) + " â‚¬";
}

function eurSigned(value) {
  if (!Number.isFinite(value)) return "â€“";
  const abs = new Intl.NumberFormat("es-ES").format(Math.abs(Math.round(value)));
  return value >= 0 ? `+${abs} â‚¬` : `-${abs} â‚¬`;
}

// â”€â”€ Factor de cierre por tramo de precio de publicaciÃ³n â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// El descuento real es mayor que el factor medido: sesgo de composiciÃ³n + desfase discovery
// apilan hacia arriba. Cifra es cota inferior, no punto central.
// Tramo decidido por precio de publicaciÃ³n final, con los mismos cortes que Query B.
...
// â”€â”€ ZBE (Zero Emission Zone) restriction â€” Spain â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
function getZbeFlag(year, fuel) {
  ...
  if (y > 0 && y <= 2014 && (f.includes("diesel") || f.includes("gasoil"))) {
    return {
      ...
      detail: "Diesel matriculado hasta 2014 â€” sin acceso libre a las ZBE de Madrid, Barcelona y otras ciudades.",
  ...
  if (y > 0 && y <= 2000 && !f.includes("diesel") && !f.includes("electri") && !f.includes("hibrid")) {
    return {
      ...
      detail: "Gasolina matriculada hasta 2000 â€” puede tener restricciones en ZBE avanzadas.",
...
// â”€â”€ DGT environmental label â€” derived from year + fuel â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
function getEnvLabel(year, fuel) {
  const y = Number(year) || 0;
  const f = (fuel || "").toLowerCase();
  if (f.includes("electri") || f.includes("hidrogen")) {
    return { code: "0",   hex: "#1A6B3C", bg: "#DCFCE7", detail: "Electrico/hidrogeno â€” acceso libre a todas las ZBE" };
  }
  if (f.includes("enchuf") || f.includes("plug") || f.includes("phev")) {
    return { code: "0",   hex: "#1A6B3C", bg: "#DCFCE7", detail: "Hibrido enchufable â€” acceso libre a ZBE" };
  }
  if (f.includes("hibrid") || f.includes("hybrid")) {
    return { code: "ECO", hex: "#0369A1", bg: "#E0F2FE", detail: "Hibrido â€” acceso preferente en ZBE, estacionamiento gratuito" };
  }
  const isDiesel = f.includes("diesel") || f.includes("gasoil");
  if (isDiesel) {
    if (y >= 2015) return { code: "C",   hex: "#1D4ED8", bg: "#DBEAFE", detail: "Etiqueta C (Euro 6) â€” circulacion normal salvo episodios de alta contaminacion" };
    if (y >= 2011) return { code: "B",   hex: "#92400E", bg: "#FEF3C7", detail: "Etiqueta B (Euro 5) â€” restringido en ZBE activas" };
    return             { code: "sin", hex: "#B91C1C", bg: "#FEE2E2", detail: "Sin etiqueta DGT â€” acceso muy limitado o nulo en ZBE" };
  }
  if (y >= 2006) return   { code: "C",   hex: "#1D4ED8", bg: "#DBEAFE", detail: "Etiqueta C â€” circulacion normal" };
  if (y >= 2000) return   { code: "B",   hex: "#92400E", bg: "#FEF3C7", detail: "Etiqueta B â€” posibles restricciones en ZBE" };
...
function getItvWarning(itvStatus) {
  ...
  if (s === "caducada") {
    return { level: "danger", label: "ITV CADUCADA", detail: "La ITV ha vencido â€” el comprador lo usara como argumento de descuento fuerte (coste ITV + posibles reparaciones)." };
  }
  if (s === "pronto") {
    return { level: "warn", label: "ITV proxima (<6 meses)", detail: "La ITV vence pronto â€” el comprador asumira ese gasto. Ajusta el precio aprox. 70-150 EUR a la baja o renuvala antes de vender." };
  }
  return null;
}
...
function buildReportData(vehicle, national, damageFactor = null, referenceDate = null) {
  ...
  // Single unconditional call â€” ratio market_median/depreciationEstimate persisted to telemetry
  ...
  // Usage impact from joint OLS (km + year) â€” 0 when on fallback path
...
async function callGeminiAnalysis(rd) {
  ...
  const damageCtx = hasDamage
    ? `\nDAÃ‘OS: nivel "${v.damageLevel}"${v.damageDescription ? `, descripcion: "${v.damageDescription}"` : ""}. Calcula el factor de descuento apropiado (campo damageFactor, rango 0.60-0.99).`
    : `\nESTADO: Sin daÃ±os declarados. damageFactor debe ser 1.00.`;

  const marketCtx = rd.usedFallback
    ? "Sin comparables activos â€” estimacion por depreciacion estandar."
    : `Precio optimo ${rd.priceOptimal}â‚¬ Â· Rango ${rd.priceLow}â‚¬â€“${rd.priceHigh}â‚¬ Â· ${rd.comparables} comparables Â· Confianza ${rd.confidence}%${rd.days ? ` Â· ${rd.days} dias medianos en cartera` : ""}`;

  const prompt = `Eres un analista experto en el mercado de vehÃ­culos de ocasiÃ³n en EspaÃ±a.

VEHÃCULO: ${v.brand || ""} ${v.model || ""} ${v.version || ""} | AÃ±o ${v.year || "â€“"} | ${v.mileage ? new Intl.NumberFormat("es-ES").format(v.mileage) + " km" : "km no indicados"} | ${v.fuel || "combustible no indicado"}${damageCtx}
DATOS DE MERCADO: ${marketCtx}

Responde ÃšNICAMENTE con un objeto JSON vÃ¡lido (sin markdown, sin texto fuera del JSON):
{
  "damageFactor": 1.00,
  "analisisMercado": "2-3 frases sobre la situaciÃ³n actual de demanda de este modelo en EspaÃ±a",
  "factoresClave": ["factor especÃ­fico 1", "factor especÃ­fico 2", "factor especÃ­fico 3"],
  "margenNegociacion": { "pct": 5, "estrategia": "frase corta de cÃ³mo negociar" },
  "consejosPersonalizados": ["consejo 1 especÃ­fico para este coche", "consejo 2", "consejo 3"]
}

Para damageFactor: 1.00 = sin daÃ±os; 0.97 = cosmÃ©tico mÃ­nimo; 0.91 = carrocerÃ­a moderada; 0.84 = mecÃ¡nico/estructural importante; 0.70 = daÃ±o grave mÃºltiple. SÃ© preciso segÃºn la descripciÃ³n real.`;
...
async function buildPdf(rd) {
  ...
  const specs5 = [
    ["ANO",         String(vehicleYear || "â€“")],
    ["KILOMETROS",  vKm != null ? new Intl.NumberFormat("es-ES").format(vKm) : "â€“"],
    ["COMBUSTIBLE", rd.vehicle.fuel         || "â€“"],
    ["TRANSMISION", rd.vehicle.transmission || "â€“"],
  ...
  // 3 KPI cards
  const kpis = [
    { n: percentile != null ? "P" + percentile : "â€“", l: "POSICION DE TU PRECIO",  s: percentile != null ? "Por encima del " + percentile + "% del mercado" : "Sin datos",          c: INK },
    { n: rd.usedFallback ? "â€“" : String(rd.comparables),        l: "UNIDADES EN PORTALES",   s: "Oferta activa del modelo",            c: TEAL },
  ...
  if (rd.usedFallback || rd.byPortal.length === 0) {
    ...
    if (rd.histogram.length) {
      ...
      rd.histogram.forEach(function(bucket) {
        const label = Math.round(bucket.from / 1000) + "kâ€“" + Math.round(bucket.to / 1000) + "k â‚¬";
    ...
    const mvItems = [
      { n: pvdDiff != null ? (pvdDiff > 0 ? "+" : "") + pvdDiff + "%" : "â€“", l: "PARTICULAR VS. CONCESIONARIO", s: pvdDiff != null ? (pvdDiff < 0 ? "Los particulares publican mas barato" : "Dealers publican mas barato") : "Sin datos de segmentacion", c: INK },
      { n: tr ? (tr.direction === "up" ? "Al alza" : tr.direction === "down" ? "A la baja" : "Estable") : "â€“", l: "TENDENCIA DE PRECIO", s: tr ? "\xB1" + Math.abs(tr.pctMonthly || 0).toFixed(1) + "% mensual en 8 semanas" : "Sin datos de tendencia", c: mv2c },
      { n: rd.comparables > 0 ? (rd.comparables >= 80 ? "Alta" : rd.comparables >= 30 ? "Media" : "Baja") : "â€“", l: "PRESION DE OFERTA", s: rd.comparables > 0 ? rd.comparables + " activos para demanda " + (rd.demand === "ALTO" ? "alta" : rd.demand === "MEDIO" ? "media" : "baja") : "Sin comparables", c: TEAL },
    ];
  ...
  // â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
  //  PAGE 3 â€” ESTRATEGIA + FACTORES + COMPARABLES
  // â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
  ...
  const scenarios = [
    { tag: "VENTA RAPIDA",  price: rd.priceLow,     t: rd.days ? Math.round(rd.days*0.5) + "â€“" + Math.round(rd.days*0.7) + " dias" : "15â€“25 dias",  bg: WHITE,          bc: BORDER, tc: TEAL,  desc: "Precio competitivo. Ideal si necesitas liquidez pronto." },
    { tag: "* RECOMENDADO", price: rd.priceOptimal, t: rd.days ? Math.round(rd.days*0.8) + "â€“" + Math.round(rd.days*1.2) + " dias" : "30â€“45 dias",  bg: hex("#F8FBF8"), bc: TEAL,   tc: TEAL,  desc: "Mejor relacion entre precio y tiempo de venta.", reco: true },
    { tag: "MAXIMO VALOR",  price: rd.priceHigh,    t: rd.days ? Math.round(rd.days*1.4) + "â€“" + Math.round(rd.days*1.9) + " dias" : "50â€“75 dias",  bg: WHITE,          bc: BORDER, tc: GRAY,  desc: "Por encima de la mediana. Requiere paciencia." },
  ];
  const sW = (CW - 16) / 3;
  scenarios.forEach(function(sc, i) {
    ...
    dt(p3, sc.t, sx + 10, y + 50, 8, ib, GRAY);
  ...
  const adjs = rd.usedFallback
    ? [
        { f: "Depreciacion",   imp: vAge != null ? "x" + standardDepreciationFactor(vAge).toFixed(2) + " (" + vAge + " anos)" : "â€“", c2: GRAY, n: "Curva estandar " + (rd.vehicle.brand || "") + " \xB7 ref. nuevo ~" + eur(rd.fallbackRefPrice) },
        { f: "Kilometraje",    imp: vKm && vAge ? (vKm > vAge*15000 ? eurSigned(Math.round(-(vKm - vAge*15000)*0.08)) : "neutro") : "â€“", c2: vKm && vAge && vKm > vAge*15000 ? RED : GRAY, n: vKm ? new Intl.NumberFormat("es-ES").format(vKm) + " km \xB7 media esperada ~" + new Intl.NumberFormat("es-ES").format((vAge||0)*15000) + " km" : "Km no declarado" },
    ...
    : [
        {
          ...
          n: rd.usageUsedDefault
            ? (vKm != null ? new Intl.NumberFormat("es-ES").format(vKm) + " km \xB7 " : "") + (vAge != null ? vAge + " anos \xB7 " : "") + "estimado por segmento" + (rd.usageCapApplied ? " \xB7 tope de segmento" : "")
  ...
  if (rd.vehicle.color) {
    ...
    adjs.push(ca.pct !== 0
      ? { f: "Color", imp: ca.pct + "%", c2: ca.pct < 0 ? RED : GRAY, n: colorName + " â€” menor liquidez vs. blanco/negro/plata" }
      : { f: "Color", imp: "neutro", c2: GRAY, n: colorName + " â€” color de alta liquidez en el mercado espanol" });
  }
  ...
  if (validSamples.length > 0 && !rd.usedFallback) {
    ...
    validSamples.slice(0, 5).forEach(function(s, i) {
      fr(p3, MX, y, CW, 22, i % 2 === 0 ? WHITE : hex("#FAFAFA"));
      dt(p3, String(s.year || "â€“"),                                                            MX + 8,   y + 6, 8.5, ib, GRAY);
      dt(p3, s.mileage ? new Intl.NumberFormat("es-ES").format(s.mileage) + " km" : "â€“",      MX + 52,  y + 6, 8.5, ib, GRAY);
      dt(p3, eur(s.price),                                                                          MX + 148, y + 6, 8.5, rb, INK);
      dt(p3, (s.version || s.model || "â€“").slice(0, 22),                                      MX + 248, y + 6, 7.5, ib, GRAY, 130);
      dt(p3, (s.portal || "â€“").slice(0, 18),                                                  MX + 390, y + 6, 7.5, ib, TEAL, 100);
      y += 22;
    });
  }
  ...
  // â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
  //  PAGE 4 â€” PROYECCION Y EVOLUCION
  // â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
  ...
  // Time-to-sell bar
  if (rd.days != null) {
    dt(p4, "Tiempo estimado de venta", MX, y, 13, rb, INK); y += 14;
    dt(p4, "Dias hasta la venta segun el precio de salida, para este modelo en el mercado actual.", MX, y, 8, ib, GRAY); y += 12;

    const tBarW = CW - 2, tBarX = MX;
    const maxDays = Math.max(75, Math.round(rd.days * 2.1));
    const dFast = Math.round(rd.days * 0.6);
    const dOpt  = rd.days;
    const dMax  = Math.round(rd.days * 1.7);
    const mkX   = function(d) { return tBarX + Math.round(Math.min(d / maxDays, 1) * tBarW); };

    fr(p4, MX, y, CW + 2, 42, hex("#FAFAFA"), BORDER, 0.5);

    const barH2 = 10, barY2 = y + 10;
    fr(p4, tBarX, barY2, Math.round(tBarW * 0.4),  barH2, hex("#C8E6C9"));
    fr(p4, tBarX + Math.round(tBarW * 0.4), barY2, Math.round(tBarW * 0.35), barH2, hex("#FFE082"));
    fr(p4, tBarX + Math.round(tBarW * 0.75), barY2, Math.round(tBarW * 0.25), barH2, hex("#FFCDD2"));

    [[dFast, eur(rd.priceLow), GREEN], [dOpt, eur(rd.priceOptimal), AMBER], [dMax, eur(rd.priceHigh), RED]].forEach(function(item) {
      const d = item[0], label = item[1], c = item[2];
      const mx3 = mkX(d);
      fr(p4, mx3 - 1, barY2 - 2, 2, barH2 + 4, c);
      dt(p4, d + " d.", mx3 - 8, barY2 + barH2 + 6, 7, ib, GRAY);
      dt(p4, label,     mx3 - 14, barY2 - 10, 6.5, rb, c);
    });
    dt(p4, "Tu precio: " + dOpt + " dias aprox.", tBarX + Math.round(tBarW * 0.35), barY2 - 18, 7.5, rb, INK);
    y += 56;
  }
  y += 10;

  // Comparable listings with note
  const validSamples4 = (rd.samples || []).filter(function(s) { return s.price > 0; });
  if (validSamples4.length > 0 && !rd.usedFallback) {
    dt(p4, "Comparables: cuanto llevan publicados", MX, y, 13, rb, INK); y += 14;
    dt(p4, "Un anuncio que lleva mucho tiempo activo suele estar sobrevalorado. Usalo como argumento de venta.", MX, y, 8, ib, GRAY); y += 12;
    fr(p4, MX, y, CW, 20, BG, BORDER, 0.5);
    dt(p4, "PRECIO",  MX + 8,   y + 5, 6.5, rb, LIGHT);
    dt(p4, "KM",      MX + 100, y + 5, 6.5, rb, LIGHT);
    dt(p4, "VERSION", MX + 196, y + 5, 6.5, rb, LIGHT);
    dt(p4, "PORTAL",  MX + 360, y + 5, 6.5, rb, LIGHT);
    y += 20;
    validSamples4.slice(0, 5).forEach(function(s, i) {
      fr(p4, MX, y, CW, 22, i % 2 === 0 ? WHITE : hex("#FAFAFA"));
      dt(p4, eur(s.price), MX + 8, y + 6, 8.5, rb, INK);
      dt(p4, s.mileage ? new Intl.NumberFormat("es-ES").format(s.mileage) + " km" : "â€“", MX + 100, y + 6, 8.5, ib, GRAY);
      dt(p4, (s.version || s.model || "â€“").slice(0, 22), MX + 196, y + 6, 7.5, ib, GRAY, 150);
      dt(p4, (s.portal || "â€“").slice(0, 18), MX + 360, y + 6, 7.5, ib, TEAL, 100);
      y += 22;
    });
    y += 6;
    dt(p4, "(*) Dias publicado: pendiente de instrumentacion del scraper (listed_at actualmente NULL).", MX, y, 6.5, ib, LIGHT);
  }

  footer(p4, null, ib);

  // â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
  //  PAGE 5 â€” COMO VENDER MEJOR + METODOLOGIA
  // â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
...
async function generateSellReport(vehicle = {}) {
  ...
  // Diagnostic tuple â€” logged on every production tasaciÃ³n for observability
  const mkt = national.market || {};
  ...
  // Telemetry INSERT â€” awaited with 250 ms timeout so it executes in serverless
  ...
  try {
    ...
    if (pgPool) {
      // Both sides of the race must never reject â€” otherwise the loser becomes an
...
module.exports = { generateSellReport, buildReportData, BRAND_TIERS, getClosingFactor, CLOSE_FACTORS };
