...
function buildShape(w) {
  ...
  return {
    ...
    businessHours: w.business_hours || null,
  };
}
...
module.exports = async function workshopsEnrichHandler(req, res) {
  ...
  // Google Places API desactivado temporalmente (cuenta sin crédito)
  // Devolver los datos que ya tengamos en BD sin hacer ninguna petición externa
  const GOOGLE_API_DISABLED = true;
  ...
  try {
    const { rows } = await client.query(
      `SELECT id, source, external_id, phone, rating, rating_count, website, opening_hours, photos, enriched_at, business_hours
    ...
    // API desactivada: devolver datos actuales sin llamar a Google
    if (GOOGLE_API_DISABLED) {
      return res.status(200).json({ ok: true, enriched: buildShape(w) });
    }
    ...
    try {
      ...
      // Se devolvían los datos SIN enriquecer bajo la etiqueta `enriched`, y
      // con ok:true. El log ya existía —este era el único de los cinco que
      // registraba algo— pero la respuesta seguía afirmando lo contrario de lo
      // que acababa de escribir en el log.
      return res.status(200).json({
        ok: false,
        enriquecido: false,
        taller: buildShape(w),
        warning: `No se pudo consultar Places: ${err.message}`,
      });
    }
