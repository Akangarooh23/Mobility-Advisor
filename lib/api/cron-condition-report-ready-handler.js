 ...
 *
 * Va aquí y no en PopCar Check por una razón sencilla: **el correo del cliente
...
const { textoPlano } = require("../correo");
...
async function enviar(destinatario, correo) {
  ...
  const r = await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({ from, reply_to: respuestaA(), to: destinatario, ...correo, text: textoPlano(correo.html) }),
  });
...
function autorizado(req) {
  ...
  // Aquí además vale la llave interna, que es la que usa un servicio nuestro
  // para desatascar un aviso a mano.
  return cronAutorizado(req, { conLlaveInterna: true });
}
...
module.exports = async function cronConditionReportReadyHandler(req, res) {
  ...
  try {
    ...
    for (const fila of rows) {
      ...
      if (!LISTO.has(estado) && popcarCheck.estaConfigurado()) {
        const consulta = await popcarCheck.leerEstadoDeSesion(fila.capture_session_id);
        if (consulta.ok) {
      ...
      // Y al móvil, si tiene la app. Va detrás del correo y no en su lugar: el
      // correo lleva el PDF y queda guardado, el aviso solo dice que ya está.
      // Que no salga no cambia nada de lo de arriba, y por eso ni se mira.
      const coche = [normalizeText(fila.brand), normalizeText(fila.model)]
        .filter(Boolean)
        .join(" ");
      await avisosPush
        .enviaAviso([destinatario], {
          titulo: "Tu informe de estado ya está",
          cuerpo: coche ? `${coche} · ya puedes verlo en la app.` : "Ya puedes verlo en la app.",
          datos: { pantalla: "informe", vehicleId: String(fila.vehicle_id || "") },
        })
        .catch(() => {});
