...
async function ensureMarketplaceVoTablePostgres() {
  ...
  await pool.query("ALTER TABLE moveadvisor_marketplace_vo_offers ADD COLUMN IF NOT EXISTS renting_monthly NUMERIC(10,2)");
  await pool.query("ALTER TABLE moveadvisor_marketplace_vo_offers ADD COLUMN IF NOT EXISTS renting_months INTEGER DEFAULT 48");
...
async function listMarketplaceVoOffers(options = {}) {
  ...
  try {
    ...
    const countResult = await pool.query(
      `SELECT COUNT(*)::int AS total FROM moveadvisor_marketplace_vo_offers ${whereClause}`
    );
    ...
    const query = `
      ...
      LIMIT $1 OFFSET $2
    `;
    const rowsResult = await pool.query(query, [boundedLimit, boundedOffset]);
    ...
    const offers = (rowsResult.rows || []).map((row) => ({
      ...
      rentingMonthly: Number(toNumber(row.renting_monthly) || 0),
      rentingMonths: Number(toNumber(row.renting_months) || 48),
...
async function getMarketplaceVoOfferByUrl(sourceUrl) {
  ...
  try {
    const result = await pool.query(
      `SELECT id, title, brand, model, price, year, mileage, location, color, displacement,
              fuel, power, seller, has_guarantee_seal, portal_score, warranty_months,
              description, image_url, source_url, portal,
