...
const COBRO = { leadId: LEAD, email: CLIENTE, importe: 20999, fee: 3000, sessionId: "cs_1", cobroRef: "pi_1" };
...
describe("qué se factura de un depósito", { concurrency: 1 }, () => {
  test("solo lo nuestro", async () => {
    await depositoRecibido(COBRO);
    assert.equal(facturasEmitidas.length, 1);
    assert.equal(Number(facturasEmitidas[0].importe), 3000,
...
/**
 * El estado del dinero, que es lo que mira el ERP.
 *
 * El ERP no deja soltar el dinero si `escrow_estado` no dice «retenido», y ese
 * campo se quedaba en «pendiente» con el dinero ya dentro. El botón «Liberar el
 * pago al vendedor» contestaba que el cliente todavía no había pagado, y como el
 * aviso salía detrás del panel abierto, desde la pantalla no pasaba nada.
 */
describe("el estado del depósito", { concurrency: 1 }, () => {
  test("al llegar el dinero, queda retenido", async () => {
    let sql = "";
    const anterior = Pool.prototype.query;
    Pool.prototype.query = async (q, params) => {
      const t = String(q || "");
      if (/UPDATE moveadvisor_market_leads/i.test(t) && /deposit_paid_at/.test(t)) sql = t;
      return anterior(q, params);
    };
    try {
      await depositoRecibido({ leadId: "imp-1", email: CLIENTE, importe: 20999, fee: 3000, sessionId: "cs_1" });
    } finally {
      Pool.prototype.query = anterior;
    }
    assert.match(sql, /escrow_estado = 'retenido'/,
      "sin esto, el ERP dice que el cliente no ha pagado y no deja soltar el dinero");
  });

  test("y se marca en la misma sentencia que el cobro, no en otra", async () => {
    // Dos sentencias serían dos verdades a medias si la segunda falla: dinero
    // cobrado y estado sin mover, que es exactamente el fallo que hubo.
    let sql = "";
    const anterior = Pool.prototype.query;
    Pool.prototype.query = async (q, params) => {
      const t = String(q || "");
      if (/escrow_estado = 'retenido'/.test(t)) sql = t;
      return anterior(q, params);
    };
    try {
      await depositoRecibido({ leadId: "imp-2", email: CLIENTE, importe: 20999, fee: 3000, sessionId: "cs_2" });
    } finally {
      Pool.prototype.query = anterior;
    }
    assert.match(sql, /deposit_paid_at = NOW\(\)/);
    assert.match(sql, /deposit_paid_at IS NULL/,
      "y con la condición puesta: un segundo aviso no puede pisar un estado final");
  });
});
