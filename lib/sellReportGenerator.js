...
Para damageFactor: 1.00 = sin daños; 0.97 = cosmético mínimo; 0.91 = carrocería moderada; 0.84 = mecánico/estructural importante; 0.70 = daño grave múltiple. Sé preciso según la descripción real.`;
  ...
  try {
    ...
    // Aquí no hay descubrimiento de modelos, así que un nombre con número es
    // una bomba de relojería: los dos que había -2.0-flash y 1.5-flash- están
    // retirados y esto devolvía 404 siempre. Los alias "-latest" no caducan.
    for (const modelName of ["gemini-flash-latest", "gemini-2.5-flash"]) {
      try {
...
// Paleta PopCar sobre papel: negro, amarillo y blanco.
//
// AMBER es el amarillo de marca llevado a una luminosidad legible. Se usa once
// veces como texto, y el #FFC400 puro sobre papel blanco no se lee — menos aún
// impreso o fotocopiado. Para barras y rellenos está AMARILLO, que sí es el
// tono de marca: ahí el color no tiene que sostener ninguna letra.
const INK      = hex("#111111");
const AMBER    = hex("#6B5200");
const AMARILLO = hex("#FFC400");
const TEAL     = hex("#111111");
const GRAY     = hex("#5E5E59");
const LIGHT    = hex("#96968F");
const BG       = hex("#F7F7F3");
const BORDER   = hex("#E4E4DF");
const WHITE    = rgb(1, 1, 1);
const GREEN    = hex("#1A6B3C");
const RED      = hex("#B4502E");
const WARN     = hex("#6B5200");
const WARNBG   = hex("#FFF6D9");
...
function runhead(page, vLabel, pageNum, rb, ib) {
  fr(page, MX, 17, 3, 11, AMARILLO);
  dt(page, MARCA.nombre, MX + 7, 18, 10, rb, INK);
...
function footer(page, text, ib) {
  const t = text || `${MARCA.nombre} \xB7 ${MARCA.sitio} \xB7 Valido 30 dias desde su emision.`;
...
async function buildPdf(rd) {
  ...
  // Cabecera: fondo blanco, barra amarilla y el nombre en negro
  fr(p1, MX, 21, 4, 16, AMARILLO);
  dt(p1, MARCA.nombre, MX + 9, 22, 14, rb, INK);
  ...
  // Fallback notice
  if (rd.usedFallback) {
    fr(p1, MX, y, CW, 26, WARNBG, hex("#C79A00"), 0.8);
  ...
  footer(p1, `${MARCA.nombre} \xB7 ${MARCA.sitio} \xB7 Valido 30 dias desde su emision.`, ib);
  ...
  if (rd.usedFallback || rd.byPortal.length === 0) {
    ...
    fr(p2, MX, y, CW, 64, WARNBG, hex("#C79A00"), 0.8);
    ...
    if (rd.histogram.length) {
      ...
      rd.histogram.forEach(function(bucket) {
        ...
        if (bW > 0) fr(p2, MX + 72, y, bW, 14, isOpt ? AMBER : hex("#E4E4DF"));
  ...
  const disc = `AVISO LEGAL: Este informe tiene caracter informativo y no constituye una tasacion pericial oficial. Los precios son estimaciones basadas en datos de mercado publicos en el momento de la consulta. ${MARCA.razonSocial} no garantiza la venta al precio indicado ni asume responsabilidad por decisiones tomadas a partir de este informe. Valido 30 dias desde la fecha de emision.`;
  wrapText(disc, 95).forEach(function(l, i) { dt(p5, l, MX + 12, y + 10 + i * 11, 7.5, ib, GRAY); });

  footer(p5, `${MARCA.nombre} \xB7 ${rd.generatedAt} \xB7 ${MARCA.sitio}`, ib);
