...
const { plantilla, parrafo, datos, boton, esc } = require("./correo");
...
/**
 * De dónde vino el comprador.
 *
 * Las mismas cinco que guardan los leads y los eventos del embudo, y las mismas
 * a propósito: el informe agrupa por `utm_source`, y si aquí se guardaran otras
 * —o menos— las visitas no se podrían sumar con lo demás.
 *
 * Hasta ahora la visita no guardaba ninguna. El que llega de coches.net, pide
 * cita y no deja lead entraba como si hubiera aparecido de la nada, y ese es
 * justo el camino que hace el comprador del portal: pulsa el enlace corto, ve
 * el coche y pide hora. Sin esto no se puede contestar si el portal trae gente
 * o solo cuesta dinero, que es la única pregunta que decide si se sigue
 * pagando.
 *
 * Se guardan las cinco y no solo la fuente: el día que se haga una campaña, lo
 * que no se guardó no se puede recuperar.
 */
const UTM = ["utm_source", "utm_medium", "utm_campaign", "utm_content", "utm_term"];

/** Lo que llegue, recortado a lo que cabe y sin nulos. */
function laUtm(body) {
  const out = {};
  for (const c of UTM) out[c] = String(body?.[c] ?? "").slice(0, 255);
  return out;
}
...
/**
 * Las columnas de la UTM en la solicitud.
 *
 * Van en un ALTER y no solo dentro del CREATE porque la tabla ya existe en
 * producción: un `CREATE TABLE IF NOT EXISTS` no toca una tabla que ya está, y
 * las columnas nuevas nunca llegarían.
 */
const ENSURE_UTM_EN_LA_SOLICITUD = `
  ALTER TABLE vehicle_visit_requests
    ${UTM.map((c) => `ADD COLUMN IF NOT EXISTS ${c} VARCHAR(255) NOT NULL DEFAULT ''`).join(',\n    ')}`;
 ...
 */
/**
 * El correo con el enlace para confirmar.
 *
 * Con la maqueta de todos los demás —logotipo, botón— y no un párrafo suelto con
 * un enlace: es el primer correo que recibe alguien que no nos conoce, y un
 * texto pelado pidiendo que pulse un enlace es lo que parece un engaño y lo que
 * los filtros mandan a spam.
 *
 * Con la **franja** entera —«de 10:00 a 14:00»— y no la hora de empezar: «a las
 * 10:00» le hacía creer que le esperaban a las diez en punto.
 */
function elCorreoDeConfirmacion({ vehicleTitle, dia, franja, cuando, enlace }) {
  const coche = nt(vehicleTitle) || "el coche";
  const filas = [["Vehículo", esc(coche)]];
  if (nt(dia)) filas.push(["Día", esc(dia)]);
  if (nt(franja)) filas.push(["Franja", esc(franja)]);
  if (!nt(dia) && nt(cuando)) filas.push(["Cuándo", esc(cuando)]);
  return {
    subject: `Confirma tu visita a ${coche}`,
    html: plantilla({
      titulo: "Confirma tu visita",
      cuerpo:
        parrafo(`Has pedido ver <strong>${esc(coche)}</strong>. <strong>Todavía no está reservada</strong>: pulsa el botón para confirmar que este correo es tuyo y se la pasamos a quien tiene el coche.`) +
        datos(filas) +
        boton("Confirmar la visita", enlace) +
        parrafo(`El enlace vale ${HORAS_DE_VALIDEZ} horas. Si no has sido tú, no hagas nada: sin pulsarlo no se reserva nada y nadie se entera.`, 14),
    }),
  };
}
