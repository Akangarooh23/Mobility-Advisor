...
describe("el enlace a la ficha de un coche", { concurrency: 1 }, () => {
  ...
  test("el precio de la vista previa es el del coche puesto aquí", async () => {
    const res = await pide("as_1", [COCHE_ALEMAN]);
    // Lo que cuesta traerlo y lo que ganamos, no los 28.000 del anuncio alemán.
    const puesto = precioPuestoAqui(COCHE_ALEMAN.price, COCHE_ALEMAN.market_price_es);
    assert.match(res.cuerpo, new RegExp(puesto.toLocaleString("es-ES").replace(".", "\\.")));
