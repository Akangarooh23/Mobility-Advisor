...
const { MARCA, COLOR, remitente, respuestaA } = require("../marca");
const { plantilla, parrafo, boton, enlace, urlSegura, textoPlano } = require("../correo");
...
async function findImportMatches(alert, pool, already) {
  ...
  const conds = ["country = 'DE'", "import_published = TRUE", "price IS NOT NULL", "COALESCE(is_active, TRUE) = TRUE"];
  const vals = [];
...
function buildOfferCard(o) {
  ...
  // Sin flex ni degradados: en un correo no se ven igual en dos clientes
  // seguidos. Una imagen, el título, y el precio grande.
  const imgTag = o.image
    ? `<img src="${urlSegura(o.image)}" width="540" style="width:100%;height:180px;object-fit:cover;display:block" alt="">`
    : "";

  return `
    <table role="presentation" cellpadding="0" cellspacing="0" border="0" style="width:100%;border:1px solid ${COLOR.linea};border-radius:10px;margin:0 0 16px 0">
      ${imgTag ? `<tr><td>${imgTag}</td></tr>` : ""}
      <tr><td style="padding:16px 18px">
        ${o.isImport ? `<p style="margin:0 0 6px 0;font-size:11px;font-weight:700;color:${COLOR.amarilloTexto}">IMPORTACIÓN</p>` : ""}
        <p style="margin:0 0 4px 0;font-size:16px;font-weight:700;color:${COLOR.negro}">${esc(o.title)}</p>
        <p style="margin:0 0 12px 0;font-size:13px;color:${COLOR.textoSuave}">${esc(meta)}</p>
        ${price ? `<p style="margin:0 0 14px 0;font-size:22px;font-weight:800;color:${COLOR.negro}">${esc(price)}</p>` : ""}
        ${boton("Ver la oferta", link)}
      </td></tr>
    </table>`;
}
...
function buildEmailHtml(alert, offers) {
  const cards = offers.slice(0, CABEN).map(buildOfferCard).join("");
  // El titular cuenta las que hay, no las que caben: el asunto ya dice el total
  // y decir aquí otro número deja al cliente con un correo que se contradice
  // solo —«70 nuevas ofertas» fuera y «5 nuevas ofertas» dentro—.
  const plural = offers.length === 1 ? "nueva oferta" : "nuevas ofertas";
  const resto = offers.length - CABEN;

  return plantilla({
    titulo: `${offers.length} ${plural} para tu alerta`,
    cuerpo:
      parrafo(esc(alert.title || "Alerta de mercado"), 14) +
      cards +
      (resto > 0 ? parrafo(`Aquí van las ${CABEN} más recientes; quedan ${resto} por ver.`, 14) : "") +
      enlace("Ver todas las ofertas", `${APP_BASE_URL}/marketplace-vo`),
...
async function sendEmail(to, subject, html) {
  ...
  const resp = await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({ from, reply_to: respuestaA(), to: [to], subject, html, text: textoPlano(html) }),
  });
