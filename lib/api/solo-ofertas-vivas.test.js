...
describe("ni un coche dañado, ni uno sin mirar", () => {
  ...
  test("el recálculo lee las dos columnas, no solo is_damaged", () => {
    // Un NULL en is_damaged no distingue «mirado y limpio» de «sin mirar»: sin
    // damage_checked_at, el recálculo publicaría los que nadie ha mirado.
    const fs = require("fs");
    const path = require("path");
    ...
    assert.match(consulta, /is_damaged/, "la consulta no lee is_damaged");
    assert.match(consulta, /damage_checked_at/, "la consulta no lee damage_checked_at");
