...
const { porQueNoSePuedeTasarSuCoche } = require("../antes-de-tasar");
...
function getCheckoutPool() {
  ...
  _checkoutPool = new Pool({ connectionString: connString, ssl: SSL_POSTGRES });
...
async function billingCheckoutHandler(req, res) {
  ...
  if (requestedPlanId === "valuation") {
     ...
     */
    /*
     * Si el coche es suyo, primero su ficha técnica.
     *
     * Va **antes** de la gratuita y antes de Stripe a propósito: es lo que
     * decide si se le cobra o se le gasta su única tasación gratis. Bloquear
     * después sería cobrarle por un número que no podemos sostener.
     *
     * Una tasación vale lo que vale su fiabilidad: la versión es lo que dice
     * con qué coches se compara el suyo, y de la ficha técnica es de donde sale
     * la versión de verdad. Si no se puede comprobar, tampoco se tasa — y se le
     * dice que no se le ha cobrado nada.
     */
    const sinFicha = await porQueNoSePuedeTasarSuCoche(
      getCheckoutPool(),
      customerEmail,
      normalizeText(body.plate),
    );
    if (sinFicha) {
      return res.status(409).json({ error: sinFicha, falta_la_ficha_tecnica: true });
    }
