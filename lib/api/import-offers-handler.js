...
const { serviciosParaElCliente, tarifasDeEntrega } = require("../servicios");
...
function mapOffer(r, garantias) {
  ...
  return {
    ...
    /**
     * Lo que puede contratar aparte: entrega, seguro y reacondicionado.
     *
     * Van sin precio. La entrega lo tendra cuando haya tabla de zonas cargada;
     * el reacondicionado no lo puede tener nunca hasta que el coche llegue y se
     * mire, y el seguro no lo tendra hasta que haya correduria. Un servicio sin
     * precio se ensena como «a consultar» y **no suma** al total: no se puede
     * sumar lo que no se sabe.
     */
...
module.exports = async function importOffersHandler(req, res) {
  ...
  if (offerId) {
    try {
      ...
      /**
       * Lo que puede contratar aparte, y lo que cuesta llevarselo a su casa.
       *
       * Las tarifas van enteras y no resueltas: aqui todavia no se sabe en que
       * provincia vive. Se resuelven en la pantalla, cuando la dice, sin tener
       * que volver a preguntar al servidor cada vez que la cambia.
       */
      const tarifas = await tarifasDeEntrega(pool);
      return res.status(200).json({
        ok: true,
        offer: mapOffer(r.rows[0], garantias),
        servicios: serviciosParaElCliente(tarifas, ""),
        entregaTarifas: tarifas,
      });
    } catch (error) {
