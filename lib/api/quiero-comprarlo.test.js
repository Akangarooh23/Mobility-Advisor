...
before(() => {
  Pool.prototype.query = async function (sql, p = []) {
    ...
    // El encargo, bloqueado aparte: es el candado que decide quien gana.
    if (/FROM erp_encargos_venta e WHERE e\.id = \$1 FOR UPDATE/.test(t)) {
      return r(encargo && p[0] === encargo.id
        ? [{ venta_estado: encargo.venta_estado ?? null, venta_booking_id: encargo.venta_booking_id ?? null }]
        : []);
    }
    if (/UPDATE vehicle_visit_bookings\s+SET status = 'cancelled'/.test(t)) {
      const anuladas = otras.filter((o) => o.offer_id === p[0] && o.id !== p[1] && ["pending", "confirmed"].includes(o.status));
      for (const o of anuladas) o.status = "cancelled";
      return r(anuladas.map((o) => ({ ...o })));
    }
...
describe("quiero comprarlo", () => {
  ...
  test("las demás visitas a ese coche quedan anuladas, y se les dice", async () => {
    /*
     * Al vendedor se le decía «no lo enseñes a nadie más» y a quien tenía hora
     * para el miércoles, nada: le seguía llegando el recordatorio y se
     * presentaba a ver un coche ya reservado.
     */
    otras.push({
      id: "b-2", offer_id: "idcar-veh-1", status: "confirmed", vehicle_title: "Volkswagen T-Roc",
      starts_at: new Date(Date.now() + 2 * 86400000).toISOString(),
      buyer_email: "otra@example.com", buyer_name: "Marta",
    });
    await compra();
    assert.equal(otras[0].status, "cancelled");
    const suyo = correos.find((c) => c.to === "otra@example.com");
    assert.match(suyo.subject, /ya esta reservado/);
    assert.ok(!suyo.html.includes("12345678Z"), "a nadie más le llega el DNI del comprador");
  });

  test("y las que ya estaban anuladas o pasadas se quedan como están", async () => {
    otras.push({ id: "b-3", offer_id: "idcar-veh-1", status: "cancelled", buyer_email: "otra@example.com" });
    await compra();
    assert.equal(otras[0].status, "cancelled");
    assert.equal(correos.filter((c) => c.to === "otra@example.com").length, 0);
  });
