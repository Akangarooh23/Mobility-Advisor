  ...
  if (step === 'pre_reg_consent') {
    if (!/^s[ií]?$|^yes$|^acepto$/i.test(lo)) {
      return {
        step: 'rejected',
        ctx,
        reply: `Sin tu consentimiento no puedo continuar. Para consultas escríbenos a ${MARCA.correoSoporte}.\n¡Hasta pronto! 👋`
      };
    }
...
module.exports = async function whatsappHandler(req, res) {
  ...
  try {
    ...
    await sendWhatsApp(from, `Ha ocurrido un error. Por favor inténtalo de nuevo o contáctanos en ${MARCA.correoSoporte}`, sendFromId).catch(() => {});
  }
};
