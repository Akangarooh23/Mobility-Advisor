...
/**
 * Si una hora es una de las que se le propusieron.
 *
 * Se compara por instante y no por texto: la misma hora puede venir escrita de
 * dos formas —con milisegundos, con zona— y dos textos distintos pueden ser el
 * mismo momento.
 *
 * Sin esta comprobacion, el enlace del correo seria una forma de ponerse la
 * hora que uno quiera sobre un calendario que nadie ha acordado.
 */
function esUnaHoraPropuesta(horas, iso) {
  const cuando = new Date(iso).getTime();
  if (Number.isNaN(cuando)) return false;
  return (Array.isArray(horas) ? horas : []).some((h) => new Date(h).getTime() === cuando);
}
 ...
 */
/**
 * La hora de la visita: «10:00».
 *
 * Una visita es a una hora. Solo una reserva vieja —de cuando se reservaba la
 * franja entera— dura más, y entonces se dice el rango para no mentir.
 */
function laFranja(inicio, fin) {
  const hora = (d) => new Date(d).toLocaleTimeString("es-ES", { hour: "2-digit", minute: "2-digit", timeZone: "Europe/Madrid" });
  const a = hora(inicio);
  if (!fin || Number.isNaN(new Date(fin).getTime())) return a;
  if (!(new Date(fin).getTime() - new Date(inicio).getTime() > 60 * 60 * 1000)) return a;
  const z = hora(fin);
  return a === z ? a : `de ${a} a ${z}`;
}
...
function deReserva(b, { sitioUrl = "" } = {}) {
  ...
  return {
    ...
    // La franja entera si la tiene: «10:00» a secas hace creer que la cita es a
    // las diez en punto cuando la ventana es de cuatro horas.
    appointment_time: laFranja(b.starts_at, b.ends_at),
    ...
    gestionar_url: b.token_buyer
      ...
      : "",
    // En el coche de un particular, el seguimiento pregunta si se lo queda: y
    // «Quiero comprarlo» lleva a donde se empieza la compra.
    comprar_url: b.token_buyer && b.seller_email && String(b.offer_id || "").startsWith("idcar-")
      ? `${String(sitioUrl).replace(/\/$/, "")}/quiero-comprarlo?id=${encodeURIComponent(b.id)}&token=${encodeURIComponent(b.token_buyer)}`
      : "",
  };
}

module.exports = { ESTADO, PASO, comoSeLee, estadoDeReserva, deReserva, esUnaHoraPropuesta, laFranja };
