...
const { leQuedaLaGratuita } = require("../tasacion");
const { porQueNoSePuedeTasar } = require("../antes-de-tasar");
const { avisaDeLaFicha } = require("../avisa-de-la-ficha");
const { elMotorDeLaFicha } = require("../el-motor-de-la-ficha");
...
function getAccountPgPool() {
  ...
  _accountPgPool = new Pool({ connectionString: connString, ssl: SSL_POSTGRES });
...
async function saveProfileToPostgres(email, profile) {
  ...
  try {
    ...
    if (result.rowCount === 0) {
      console.warn(`[saveProfileToPostgres] No rows updated for email: ${email}`);
    }
  } catch (e) {
    console.error("[saveProfileToPostgres] Error:", e.message);
    throw e;
  }
}
...
async function getInvoicesFromPostgres(email) {
  ...
  try {
    ...
    return result.rows.map((row) => ({
      ...
      status: row.status || "",
      // La direccion del fichero no sale de aqui. Se le da la ruta que pide la
      // factura con su sesion; esa decide si la sirve, la genera o le manda a
      // Stripe. Antes se devolvia la direccion publica del PDF y el panel la
      // pintaba tal cual: el enlace valia para cualquiera que lo tuviera.
      pdfUrl: `/api/invoice-pdf?id=${encodeURIComponent(row.id)}`,
    }));
  } catch {
    return null;
  }
...
module.exports = async function billingAccountHandler(req, res) {
  ...
  // La regla de quién es quien pide vive en un solo sitio, `identidad.js`: manda
  // la sesión y nunca la URL. Estaba escrita aquí y solo aquí, y por eso el
  // manejador de la factura en PDF no la tenía.
  const { userId: sessionUserId, email } = await identidadDeLaPeticion(req, { cuerpo: body });
  const identity = {
  ...
  if (req.method === "GET") {
    if (normalizeText(req.query?.scope).toLowerCase() === "garage") {
      ...
      const vehicles = await listGarageVehicles(identity);

      /*
       * Las cinco puertas de cada coche, solo si se piden.
       *
       * Cuesta una consulta por coche, así que no viaja por omisión: la web
       * pinta el garaje sin ellas y no debe pagarlas. La app sí las pide, para
       * poder enseñar lo completo que está cada IDCar **antes** de que haya
       * ningún encargo de venta — que es cuando sirve de algo saberlo.
       */
      if (String(req.query?.puertas || "") === "1") {
        const porCoche = await puertasDeLosCoches(vehicles.map((v) => v?.id));
        return res.status(200).json({
          ok: true,
          vehicles: vehicles.map((v) => ({ ...v, puertas: porCoche[normalizeText(v?.id)] || null })),
        });
      }
    ...
    if (pgPlan) {
      ...
      } else if (pgPlan.planId === "pro") {
        account.billingState.planLabel = "Pro";
    ...
    if (pgProfile) {
      account.profile = {
        ...(account.profile || {}),
        fullName:         pgProfile.fullName        || account.profile?.fullName        || "",
        phone:            pgProfile.phone           || account.profile?.phone           || "",
        companyName:      pgProfile.companyName     || account.profile?.companyName     || "",
        taxId:            pgProfile.taxId           || "",
        billingStreet:    pgProfile.billingStreet   || "",
        billingPostalCode:pgProfile.billingPostalCode|| "",
        billingProvince:  pgProfile.billingProvince || "",
        billingAddress:   pgProfile.billingAddress  || "",
        clientType:       pgProfile.clientType      || "individual",
        iban:             pgProfile.iban            || "",
        email,
      };
    }
    ...
    const needsStripeVerification = stripeKey && (["plus", "pro"].includes(currentPlanId) && !ACTIVE_STATUSES.has(currentStatus));
    ...
    // Si a este usuario le queda su tasacion gratuita. Va aqui y no en un
    // endpoint propio porque la pantalla de venta ya pide esta cuenta antes de
    // ensenar el boton: una llamada mas solo para saber el precio seria otra
    // espera delante del cliente.
    return res.status(200).json({
      ok: true,
      account,
      tasacionGratuita: await leQuedaLaGratuita(identity),
      billingCatalog: {
  ...
  if (action === "update_profile") {
    ...
    try {
      ...
      return res.status(500).json({ ok: false, error: "Error al guardar el perfil: " + e.message });
    }
  ...
  if (action === "garage_add") {
    ...
    /*
     * Si ha subido su ficha técnica, que el ERP la lea ya.
     *
     * El lector vive allí y se había enganchado a la subida del ERP, que es el
     * lado que no usa nadie: la ficha la sube el cliente desde aquí. Por ese
     * camino no se leía nunca, y seguía haciendo falta que alguien pulsara un
     * botón — justo lo que se venía a quitar.
     *
     * Se espera —en Vercel lo que no se espera se corta— pero con un tope
     * corto y sin mirar lo que conteste: el coche del cliente ya está
     * guardado, y perder la lectura es una molestia nuestra, no suya.
     */
    await avisaDeLaFicha(persistedVehicle);
  ...
  if (action === "appointment_delete") {
    ...
    if (!appointmentId) return res.status(400).json({ ok: false, error: "missing_appointmentId" });
    await deleteAppointment(identity, appointmentId);
    const appointments = await listAppointments(identity);
    return res.status(200).json({ ok: true, appointments, message: "Cita eliminada." });
  }

  /*
   * ¿Se puede tasar ese coche suyo?
   *
   * La pregunta existe para poder pararlo **antes** de tasar. Bloquear solo al
   * guardar no servía de nada: la tasación ya se había hecho, el cliente ya
   * tenía el número en pantalla y lo único que se perdía era apuntarlo.
   */
  /*
   * El motor que dice su ficha tecnica, para ordenarle la lista de versiones.
   *
   * La version decide con que coches se compara el suyo al tasarlo, y la elige
   * de una lista de cuarenta. Con la cilindrada y los kilovatios del papel se
   * pueden poner delante las que su motor puede tener.
   *
   * Nunca falla: sin ficha leida devuelve el motor vacio y la lista sale como
   * salia. Quedarse sin editar el coche por esto seria mucho peor.
   */
  if (action === "motor_de_la_ficha") {
    const vehicleId = String((body.vehicleId ?? body.vehicle_id) || "").trim();
    const motor = await elMotorDeLaFicha(getAccountPgPool(), vehicleId);
    return res.status(200).json({ ok: true, motor });
  }

  if (action === "puede_tasarse") {
    const falta = await porQueNoSePuedeTasar(getAccountPgPool(), (body.valuation || body)?.vehicleId);
    return res.status(200).json({ ok: true, puede: !falta, message: falta });
  }
  ...
  if (action === "valuation_add") {
     ...
     *
     * La tasación compara su coche con los que son como él, y la versión es lo
     * que dice cuáles son: una gama tiene tres «1.5» que no valen lo mismo. Esa
     * versión la elige él de una lista, y si elige la que no es, el número no
     * sale mal por poco — sale comparado con **otros coches**, y de ese número
     * sale luego el precio de salida.
     *
     * Va aquí y no solo en la pantalla porque un botón apagado no es un
     * bloqueo: se salta con la pantalla anterior abierta o llamando a la
     * dirección. La tasación suelta de la web no pasa por esto, que allí no hay
     * coche dado de alta al que pedirle papeles.
     */
    const peticion = body.valuation || body;
    const falta = await porQueNoSePuedeTasar(getAccountPgPool(), peticion?.vehicleId);
    if (falta) {
      return res.status(409).json({ ok: false, error: "falta_la_ficha_tecnica", message: falta });
    }
    const valuations = await addValuation(identity, peticion);
