...
// { alpha: 0, balance: false } = no-op, bit-identical to pre-Ola-2 behavior.
// Change one at a time; each produces a separate drift event in the golden tests.
const POOL_CONFIG = { alpha: 0, balance: false };
...
function computeUsageImpact(offers, userKm, userYear, medianPrice, priceField, brand) {
  ...
  if (!medianPrice || !userKm || !userYear) {
    return { usageImpact: 0, usedDefault: false, slopeKm: null, slopeYear: null };
  }

  const candidates = offers.filter(
    (o) => o.mileage > 0 && o.year > 0 && Number.isFinite(o[priceField]) && o[priceField] > 0
  );
  ...
  const defaultImpact = () => {
    ...
    return { usageImpact: Math.round(Math.max(-cap, Math.min(cap, raw))), usedDefault: true, slopeKm: null, slopeYear: null };
  };
...
async function getMarketPriceSnapshot(options = {}) {
    ...
    if (!brandKnown) {
      return {
        ...
        market: {
          ...
          kmImpact: 0, ageImpact: 0, priceTrend: null, absorptionRate: null,
  ...
  // limit: 1500 — más candidatos para selectBalancedPool y OLS. Ola 1 usaba 400;
  // subir a 1500 da acceso a coches más antiguos que el ORDER BY updated_at DESC
  // aparta (el mercado actualiza el stock nuevo más frecuentemente que el stock viejo).
  const baseQuery = { desiredType, modelCandidates, maxMileage: mileage, location, limit: 1500 };
  ...
  // 1st try: fuel + transmission + year + powerCv (most precise)
  let { offers, source, totalUniverse } = await listInventoryOffers({ ...baseQuery, fuel, transmission, targetYear: year, minPowerCv: powerCvMin, maxPowerCv: powerCvMax });
  const cascadeRelaxed = { power: false, transmission: false, fuel: false, year: false };

  // If powerCv was specified and too few results: relax powerCv before other filters
  if (offers.length < 10 && powerCvMin) {
    ({ offers, source, totalUniverse } = await listInventoryOffers({ ...baseQuery, fuel, transmission, targetYear: year }));
    cascadeRelaxed.power = true;
  }

  // 2nd try: relax transmission if result too small to be meaningful
  if (offers.length < 10 && transmission) {
    ({ offers, source, totalUniverse } = await listInventoryOffers({ ...baseQuery, fuel, targetYear: year }));
    cascadeRelaxed.transmission = true;
  }

  // 3rd try: relax fuel filter
  if (!offers.length && fuel) {
    ({ offers, source, totalUniverse } = await listInventoryOffers({ ...baseQuery, fuel: "", targetYear: year }));
    cascadeRelaxed.fuel = true;
  }

  // 4th try: relax fuel + year (brand + model only)
  if (!offers.length) {
    ({ offers, source, totalUniverse } = await listInventoryOffers({ ...baseQuery, fuel: "", targetYear: null }));
    cascadeRelaxed.fuel = true;
    cascadeRelaxed.year = true;
  }

  const priceField = desiredType === "renting" ? "monthlyPrice" : "price";
  const rawPrices = offers
  ...
  const privateOffers = offers.filter(isPrivateSeller);
  const dealerOffers  = offers.filter((o) => !isPrivateSeller(o));
  ...
  const { usageImpact, usedDefault: usageUsedDefault, slopeKm, slopeYear } =
    computeUsageImpact(offers, mileage, year, marketMedian, priceField, brand);

  // Price trend: compares entry prices of recent vs older listings
  const priceTrend = computePriceTrend(offers, priceField);
  ...
  return {
    ...
    _pool: offers
      ...
      // Orden determinista: diff de recapturas muestra solo cambios reales de mercado.
      .sort((a, b) => a.price - b.price || a.mileage - b.mileage || a.year - b.year),
  };
}
