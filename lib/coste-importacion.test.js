...
const {
  TRANSPORTE, PAPELEO_ESTIMADO, sqlTipoDelImpuesto, impuestoEstimado, VALOR_FISCAL,
  FEE_POPCAR, FEE_POPCAR_BASE, PRECIO_MINIMO_COCHE,
...
describe("lo que paga el cliente", () => {
  ...
  test("el coche, el fee y el impuesto: nada más", () => {
    ...
    assert.equal(precioPuestoAqui(18000, 24000), 18000 + 3000 + impuestoEstimado(24000));
  });

  test("el fee que se le enseña lleva el IVA dentro", () => {
    // 3.000 es el precio final. De ahí, 2.479 son nuestros y 521 de Hacienda.
    assert.equal(FEE_POPCAR, 3000);
    assert.equal(FEE_POPCAR_BASE, Math.round(3000 / 1.21));
    assert.ok(FEE_POPCAR_BASE < FEE_POPCAR, "el IVA no está saliendo de dentro");
  });
  ...
  test("el fee es el mismo para un coche de 12.000 que para uno de 40.000", () => {
    ...
    const barato = precioPuestoAqui(12000, 12000) - 12000 - impuestoEstimado(12000);
    const caro = precioPuestoAqui(40000, 40000) - 40000 - impuestoEstimado(40000);
...
describe("el fee frente a lo que nos cuesta", () => {
  ...
  test("y lo que queda del fee es lo que ganamos, sin el IVA", () => {
    ...
    assert.equal(margenDelServicio(), FEE_POPCAR_BASE - costeDelServicio());
    assert.ok(margenDelServicio() < FEE_POPCAR - costeDelServicio(),
...
describe("los mismos números en el flujo que los calcula", () => {
  ...
  test("el impuesto va sobre el precio español también en el flujo", () => {
    assert.ok(sql.includes(`*${VALOR_FISCAL}*COALESCE(c.es_median,0)`),
