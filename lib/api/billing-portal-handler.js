...
module.exports = async function billingPortalHandler(req, res) {
  ...
  // A dónde devuelve Stripe al cerrar el portal. La reserva era example.com:
  // sin `origin` —una llamada de servidor a servidor, o un navegador que no
  // manda la cabecera— el cliente acababa en un dominio de ejemplo de la IANA
  // después de gestionar su suscripción. Ahora cae en la marca, como el
  // checkout, que ya lo hacía así.
  const returnUrl =
    normalizeText(process.env.STRIPE_PORTAL_RETURN_URL) ||
    `${origin || MARCA.sitioUrl}/panel/cuenta`;
