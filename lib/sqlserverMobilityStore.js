...
function addGarageVehicleByEmailSqlServer(email = "", vehicle = {}) {
  ...
  const statements = [
    ...
    `  INSERT INTO dbo.MoveAdvisorUserVehicles
         (Id, UserEmail, Title, Brand, Model, [Year], Plate, Mileage, Fuel, PolicyCompany, Notes, CreatedAt, UpdatedAt)
       VALUES
         (${sqlString(vehicleId)}, ${sqlString(normalizedEmail)}, ${sqlString(normalizedVehicle.title)},
          ${sqlString(normalizedVehicle.brand)}, ${sqlString(normalizedVehicle.model)}, ${sqlString(normalizedVehicle.year)},
  ...
  photos.forEach((photo) => {
    ...
    statements.push(`
      INSERT INTO dbo.MoveAdvisorUserVehicleFiles (VehicleId, FileType, FileName, FileSize, CreatedAt)
      VALUES (${sqlString(vehicleId)}, N'photo', ${sqlString(photo.name)}, ${Number(photo.size || 0)}, ${sqlString(nowIso)});
    `);
  });
  ...
  documents.forEach((document) => {
    ...
    statements.push(`
      INSERT INTO dbo.MoveAdvisorUserVehicleFiles (VehicleId, FileType, FileName, FileSize, CreatedAt)
      VALUES (${sqlString(vehicleId)}, N'document', ${sqlString(document.name)}, ${Number(document.size || 0)}, ${sqlString(nowIso)});
    `);
  });
    ...
    BEGIN
      ...
      BEGIN
        ...
        SET UserEmail = ${sqlString(normalizedEmail)},
            ...
            Status = ${sqlString(normalizeText(appointment?.status || "Pendiente"))},
      ...
      BEGIN
        ...
        VALUES
          (${sqlString(id)}, ${sqlString(normalizedEmail)}, ${sqlString(vehicleId)}, ${sqlString(appointmentType)},
           ...
           ${sqlString(normalizeText(appointment?.status || "Pendiente"))}, ${sqlString(normalizeText(appointment?.requestedAt) || toEsDateTimeText(nowIso))},
