...
// ── Fuel compatibility table ──────────────────────────────────────────────────
// Replaces substring matching (.includes) which had two bugs:
//   1. 'gas' ⊂ 'gasolina' → "Gas" portal label matches gasolina subjects (and vice-versa)
//   2. 'gasolina' ⊂ 'gasolina gas' → bifuel offers ("Gasolina/Gas") enter gasolina pools
//
// Key rules:
//   - híbrido enchufable is comparable for híbrido subjects (PHEV pricing reference)
//   - gas/gnc/glp are comparable for each other (GNC and GLP are priced together)
//   - gasolina is NOT compatible with gas subjects (different residual value curve)
//   - bifuel ('gas gasolina', 'gasolina gas') is NOT compatible with pure gasolina
//
// Surface map — which filter is binding per feature path:
//   sell report (getMarketPriceSnapshot):
//     readPostgresInventory → NO fuel filter in SQL → JS isFuelCompatible is the only barrier
//   ERP backoffice search (INVENTORY_PROVIDER=sqlserver):
//     buildSqlServerWhereClause → getFuelCompatTokens → exact OR in SQL + JS isFuelCompatible
//   marketplace browsing (listMarketplaceVoOffers):
//     SQL IN (compatible tokens) → binding surface; JS layer not reached for excluded offers
//   user vehicles list (listUserPublishedVehiclesForMarketplace):
//     same as marketplace browsing
const FUEL_COMPAT = {
  [GASOLINA]:           [GASOLINA],
  [DIESEL]:             [DIESEL],
  [HIBRIDO]:            [HIBRIDO, HIBRIDO_ENCHUFABLE],
  [HIBRIDO_ENCHUFABLE]: [HIBRIDO_ENCHUFABLE],
  [ELECTRICO]:          [ELECTRICO],
  'gas':                ['gas', 'gnc', 'glp'],   // GNC/GLP are comparable; gasolina is not
  'gnc':                ['gas', 'gnc', 'glp'],
  'glp':                ['gas', 'gnc', 'glp'],
};

function isFuelCompatible(offerToken, subjectFuel) {
  const allowed = FUEL_COMPAT[subjectFuel];
  if (!allowed) return true;  // unknown subject → sin dato = pasa
  return allowed.includes(offerToken);
}

// Returns compatible token list for SQL builders; null for unknown subject.
function getFuelCompatTokens(subjectFuel) {
  return FUEL_COMPAT[subjectFuel] ?? null;
}

module.exports = { inferFuelFromVersion, isFuelCompatible, getFuelCompatTokens };
