...
async function sendViewingLeadEmails({ name, email, message, vehicle_title }) {
  ...
  const from = process.env.RESEND_FROM_EMAIL || MARCA.remitentePorDefecto;
  ...
  const internalHtml = `
    <div style="font-family:sans-serif;max-width:560px;margin:0 auto;color:#1e293b">
      ...
      <hr style="border:none;border-top:1px solid #e2e8f0;margin:24px 0">
      <p style="font-size:12px;color:#64748b">${MARCA.nombre} ERP</p>
    </div>`;
  ...
  const results = await Promise.allSettled([
    fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({ from, to: email, subject: `Solicitud recibida — ${vehicle_title || MARCA.nombre}`, html: clientHtml }),
    }),
    fetch("https://api.resend.com/emails", {
      method: "POST",
      headers: { Authorization: `Bearer ${apiKey}`, "Content-Type": "application/json" },
