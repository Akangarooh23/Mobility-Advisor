...
module.exports = async function billingWebhookHandler(req, res) {
  ...
  if (eventType === "checkout.session.completed") {
    ...
    if (email) {
      updateBillingState(email, {
        planId: resolvedPlanId || normalizeText(resolveAccount(email)?.billingState?.planId).toLowerCase() || "",
  ...
  if (eventType === "customer.subscription.updated" || eventType === "customer.subscription.created" || eventType === "customer.subscription.deleted") {
    ...
    if (email) {
      ...
      updateBillingState(email, {
        planId: normalizeText(resolvedPlan?.id).toLowerCase() || normalizeText(currentAccount?.billingState?.planId).toLowerCase() || "",
        planLabel: normalizeText(resolvedPlan?.label) || normalizeText(currentAccount?.billingState?.planLabel) || "Plan MoveAdvisor",
        stripeCustomerId: customerId,
        stripeSubscriptionId: subscriptionId,
        status: normalizeStripeSubscriptionStatus(eventData?.status),
