...
async function generateInvoicePdf(invoiceRecord, userProfile = {}) {
  ...
  if (suplidos.length) {
    ...
    for (const su of suplidos) {
      ...
      dt(fmtEur(importe), 480, sy, 9, ri, c("#111111"));
      sy += 14;
    }
  }

  fr(370, totY + 30, 175, 22, c(COLOR.amarillo));
  dt(sumaSuplidos ? "TOTAL FACTURA:" : "TOTAL:", 380, totY + 37, 11, rb, c(COLOR.negro));
  dt(fmtEur(amount), 480, totY + 37, 11, rb, c(COLOR.negro));

  // Y lo que ha transferido de verdad. Sin esta línea, la factura y el ingreso
  // son dos cifras distintas y ningún papel dice por qué.
  if (sumaSuplidos) {
    dt("Total pagado (factura + suplidos):", 300, totY + 58, 9, ri, c(COLOR.textoSuave));
    dt(fmtEur(amount + sumaSuplidos), 480, totY + 58, 9, ri, c(COLOR.textoSuave));
  }

  // Pagada stamp
  dt("PAGADA", 60, Math.max(totY + 78, sy + 10), 11, rb, c(COLOR.negro));
...
async function generateDepositoPdf(datosDep, userProfile = {}) {
  ...
  dt("IMPORTE", 470, tY + 7, 8.5, rb, c(COLOR.textoSuave));
  ln(50, tY + 24, 545, tY + 24);

  let y = tY + 34;
  for (const l of reparto.lineas) {
    dt(l.concepto.slice(0, 52), 60, y, 9.5, ri, c("#111111"));
    dt(l.aQuien, 300, y, 9, ri, c(COLOR.textoSuave));
    dt(fmtEur(l.importe), 470, y, 9.5, ri, c("#111111"));
    y += 16;
  }
  ln(50, y + 4, 545, y + 4);

  fr(370, y + 12, 175, 22, c(COLOR.amarillo));
  dt("TOTAL RECIBIDO:", 380, y + 19, 11, rb, c(COLOR.negro));
  dt(fmtEur(total), 470, y + 19, 11, rb, c(COLOR.negro));
