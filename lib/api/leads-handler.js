...
async function sendConfirmationEmail(lead) {
  ...
  const from = process.env.ALERT_EMAIL_FROM || "CarsWise <support@carswiseai.com>";
  if (!apiKey) return;
  ...
  if (req.method === "PATCH") {
    ...
    try {
      ...
      if (action === "confirm") {
        ...
        // Send confirmation email (non-blocking — don't fail the request if email errors)
        sendConfirmationEmail(lead).catch((e) => console.error("[leads-handler] confirm email error:", e.message));
