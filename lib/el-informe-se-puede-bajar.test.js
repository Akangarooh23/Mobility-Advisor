...
describe("el PDF llega del correo al panel", () => {
  ...
  test("pero cuando no lo hay, lo dice en vez de no enseñar nada", () => {
    /*
     * Un hueco mudo deja al cliente buscando un botón que no existe. Pasó: la
     * pregunta —«¿dónde está para descargarlo?»— llegó por otro lado, que es
     * justo lo que la pantalla tenía que haber contestado sola.
     */
    assert.match(PANTALLA, /El informe de esta tasación está en tu correo/);
  });
});
