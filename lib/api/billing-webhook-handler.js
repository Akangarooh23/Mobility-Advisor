...
async function depositoRecibido({ leadId, email, importe, fee, sessionId, cobroRef }) {
  ...
  try {
     ...
     *
     * El coche es del concesionario aleman y el impuesto es de Hacienda. Los dos
     * pasan por nuestra cuenta y **ninguno es ingreso nuestro**, asi que van
     * fuera de la base de IVA y aparte en la factura.
     *
     * Si no se dijera, esa factura tendria un total de 3.630 € al lado de una
     * transferencia de 21.940, y la diferencia no tendria explicacion.
     */
    await pool.query(`
    ...
    try {
      const r = await pool.query(
        `SELECT escrow_coche, escrow_impuesto FROM moveadvisor_market_leads WHERE id = $1`,
        [leadId]
      );
      const f = r.rows[0] || {};
      const coche = Math.round(Number(f.escrow_coche) || 0);
      const imp = Math.round(Number(f.escrow_impuesto) || 0);
      if (coche) suplidos.push({ concepto: "Precio del coche (vendedor en Alemania)", importe: coche });
