...
/**
 * Informe de estado — espejo de referencia de PopCar Check.
 *
 ...
 *
 *  - el informe. Vive en `condition_reports` de PopCar Check, donde un CHECK
...
/** Y aquellos en los que existe un informe emitido, que ya no se puede perder. */
...
function urlDeVuelta(origen) {
  const base = normalizeText(process.env.PUBLIC_SITE_URL) || MARCA.sitioUrl;
...
/**
 * Pregunta a PopCar Check por los expedientes que puedan haber cambiado.
...
async function refrescarEstados(pool, filas) {
  ...
  if (!pendientes.length || !popcarCheck.estaConfigurado()) return filas;

  const refrescadas = await Promise.all(
    pendientes.map(async (fila) => {
      const r = await popcarCheck.leerEstadoDeSesion(fila.capture_session_id);
      ...
      /**
       * Un informe que ya estaba listo no se degrada a caducado. Nunca.
       *
       * Cuando el token de captura vencía, la consulta devolvía `410` y este
       * código lo traducía a «caducada»: al día siguiente de hacer la captura
       * desaparecían del IDCar la descarga del PDF y la vista en 3D de un
       * expediente terminado. El origen se ha arreglado en PopCar Check —el
       * token protege el enlace de captura, no el informe—, pero esto se queda
       * como red: un documento emitido es del usuario y ninguna respuesta de
       * otro servicio debería poder quitárselo de la pantalla.
       *
       * Lo que sí puede es desaparecer de verdad: un `404` sigue anulándolo,
       * porque entonces la sesión ya no existe al otro lado.
       */
      if (LISTO.has(fila.status) && nuevoEstado === "caducada") return fila;
...
module.exports = async function conditionReportHandler(req, res) {
  ...
  try {
    ...
    if (metodo === "GET") {
       ...
       *
       * Va por aquí y no contra PopCar Check directamente porque el
      ...
      if (aDescargar) {
        ...
        if (formato === "glb" || formato === "usdz") {
          const modelo = await popcarCheck.descargarModelo3d(aDescargar, formato);
          if (!modelo.ok) {
        ...
        const pdf = await popcarCheck.descargarInformePdf(aDescargar);
        if (!pdf.ok) {
    ...
    if (!popcarCheck.estaConfigurado()) {
    ...
    const creada = await popcarCheck.crearSesionDeCaptura({
      vehicleId,
    ...
    await pool.query(
      ...
      [
        vehicleId,
        popcarCheck.uuidDeVehiculo(vehicleId),
        datos.session_id,
        datos.capture_url || "",
        email,
