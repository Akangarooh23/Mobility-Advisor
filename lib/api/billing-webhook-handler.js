...
async function syncPlanToPostgres(email, planId, status) {
  const pool = getWebhookPgPool();
  if (!pool || !email) return;
  try {
    await pool.query(
      `UPDATE moveadvisor_users SET plan_id = $1, plan_status = $2, plan_updated_at = NOW() WHERE lower(email) = lower($3)`,
      [planId || "free", status || "inactivo", email]
    );
  } catch (e) {
