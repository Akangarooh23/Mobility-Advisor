...
module.exports = async function mandatoFirmadoHandler(req, res) {
  ...
  const porQueNo = M.porQueNoSePuedeSubir({ tipo, tamano });
  ...
  try {
    ...
    const ruta = await uploadBase64ToSupabase(
      contenido, tipo, `mandatos/${encargoId}/${Date.now()}-${nombre}`
    );
    if (!ruta) {
      return res.status(502).json({ ok: false, error: "No hemos podido guardar el archivo. Prueba otra vez." });
    }

    await pool.query(M.SQL_GUARDA_DOCUMENTO, [encargoId, nombre, tipo, ruta, tamano, email]);
    ...
    return res.status(200).json({
      ok: true,
      data: { firmado: true, firmado_at: marca.rows[0].firmado_at, ruta },
    });
  } catch (err) {
