...
/** Lo que se acepta: un papel firmado es un PDF o una foto. */
...
const SQL_SU_ENCARGO = `
  SELECT e.id, e.mandato_id, e.acepto_el_precio, e.firmado_at
 ...
 *
 * `libre_desde` sale de la firma y solo si aceptó el precio: es lo que decide
  ...
  UPDATE erp_encargos_venta
     SET firmado_at = NOW(),
         ...
         libre_desde = CASE WHEN acepto_el_precio
                            THEN NOW() + INTERVAL '${DIAS_HASTA_SALIR_GRATIS} days'
                            ELSE NULL END,
         updated_at = NOW()
 ...
 */
function porQueNoSePuedeSubir({ tipo, tamano } = {}) {
  if (!esUnTipoQueVale(tipo)) {
    return 'Ese archivo no vale. Sube el mandato en PDF o una foto del papel firmado.';
  }
