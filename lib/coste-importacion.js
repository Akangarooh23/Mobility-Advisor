 ...
 * impuesto y salieron 2.491. Mil setenta y un euros que hubo que poner de
 * nuestro margen, y no era un caso raro — con un tipo fijo del 4,75 %, la
 * segunda banda más baja de cuatro, **todos** los SUV grandes salen así.
 *
 * Las bandas son por CO₂: 0 hasta 120 g/km, 4,75 hasta 159, 9,75 hasta 199 y
 * 14,75 por encima. Cuando el anuncio trae el CO₂ se usa esa. **Hoy no lo trae
 * ninguno** —cero de 25.498 ofertas alemanas—, así que se estima con lo que sí
 * viene: el combustible, la potencia, la cilindrada y si lleva tracción total.
 *
 * **Y la banda se aplica sobre el valor fiscal, no sobre el precio español.**
 * Ese es el otro error, y se veía menos: el tipo se aplicaba sobre el precio de
 * un usado comparable en España, que está muy por encima de lo que Hacienda
 * llama valor de mercado —tablas de precios medios por el coeficiente de
 * antigüedad—. En el Sorento, 29.899 € contra unos 17.600.
 *
 * Con las dos correcciones, al Sorento se le estiman 2.646 € contra los 2.491
 * que costó: 155 € de más, del lado bueno.
 *
 * Sigue siendo una aproximación. Para hacerlo exacto hacen falta el CO₂ de cada
 * coche, la fecha de primera matriculación —con solo el año, uno de diciembre y
 * otro de enero caen en coeficientes distintos— y las tablas oficiales.
 */
...
/**
 * Lo que vale el coche para Hacienda, sobre lo que vale en un concesionario.
 *
 * El impuesto no se calcula sobre lo que cuesta el coche en el mercado: se
 * calcula sobre el precio del modelo **cuando era nuevo** según las tablas del
 * BOE, multiplicado por un coeficiente de depreciación por años de uso. En los
 * coches que traemos —cuatro a ocho años— eso sale en torno al 60 % del precio
 * de un usado comparable aquí.
 *
 * Es una aproximación de una tabla que no tenemos. Se queda corta en coches muy
 * nuevos y larga en muy viejos, y los dos extremos son raros en importación.
 */
const VALOR_FISCAL = 0.6;
...
const ES_TRACCION_TOTAL = /\bsuv\b|4x4|awd|4matic|quattro|xdrive|allrad|4motion/i;
...
function co2Estimado({ fuel, power_cv, title, displacement } = {}) {
 ...
 */
/**
 * Lo que se estima de impuesto para un coche.
 *
 * La banda que le toca sobre lo que vale para Hacienda, que no es lo que vale
 * en un concesionario. Las dos correcciones juntas: sin la banda, un SUV grande
 * paga como un utilitario; sin el factor, un utilitario paga como si valiera un
 * 60 % más de lo que Hacienda le reconoce.
 */
function impuestoEstimado(precioEspanol, coche = null) {
  return tipoDelImpuesto(coche) * (Number(precioEspanol) || 0) * VALOR_FISCAL;
}

function precioPuestoAqui(precioAleman, precioEspanol, garantia = 0, coche = null) {
  const referencia = Number(precioEspanol) || 0;
  return (Number(precioAleman) || 0) + FEE_POPCAR
    + impuestoEstimado(referencia, coche) + (Number(garantia) || 0);
}
...
function partidasDeTraerlo(precioAleman, precioEspanol, coche = null) {
  ...
  return [
    ...
    { concepto: "Impuesto de matriculación", importe: impuestoEstimado(referencia, coche), firme: false },
  ];
}
...
function desgloseParaElCliente(precioAleman, precioEspanol, garantia = null, coche = null) {
  ...
  const lineas = [
    ...
    { concepto: "Servicio PopCar · IVA incluido", importe: FEE_POPCAR, aQuien: "a nosotros" },
    {
      ...
      importe: impuestoEstimado(referencia, coche),
      aQuien: "a Hacienda",
    },
  ];
...
function sqlTipoDelImpuesto(alias = "") {
  ...
  const total = `COALESCE(${c}title,'') ~* '(\\ysuv\\y|4x4|awd|4matic|quattro|xdrive|allrad|4motion)'`;
  /*
 ...
 *
 * Con el mismo valor fiscal que la versión de JavaScript: el impuesto no se
 * calcula sobre lo que cuesta el coche en un concesionario, sino sobre las
 * tablas de Hacienda depreciadas, que salen en torno al 60 % de eso.
 */
function sqlPrecioPuestoAqui(alias = "", garantia = 0) {
  const c = alias ? `${alias}.` : "";
  const g = typeof garantia === "string" ? garantia : Math.round(Number(garantia) || 0);
  return `(COALESCE(${c}price,0) + ${FEE_POPCAR} + ${g} + (${sqlTipoDelImpuesto(alias)})*${VALOR_FISCAL}*COALESCE(${c}market_price_es,0))`;
}
