...
async function identifyByPhone(pool, phone) {
  ...
  // 1. Check pre_clientes (previous WhatsApp contacts)
  ...
  // 2. Check existing leads by contact_phone (buyers who contacted before)
...
async function findOffer(pool, ref) {
  ...
  const byId = await pool.query(
    `SELECT id, brand, model, year, price, mileage, fuel_type
...
async function saveLead(pool, { phone, preClienteId, userEmail, tipo, subTipo, subFuente, ofertaId, criterios, prioridad, transcript }) {
  ...
  `, [phone, preClienteId || null, userEmail || null, tipo, subTipo || null, subFuente || null, ofertaId || null, JSON.stringify(criterios || {}), prioridad || 'normal', JSON.stringify(transcript || [])]);
}
...
async function notifyOps(subject, lines) {
  ...
  const html = `<pre style="font-family:sans-serif;font-size:14px">${lines.join('\n')}</pre>`;
  await fetch('https://api.resend.com/emails', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${key}` },
    body: JSON.stringify({ from: 'CarsWise Bot <bot@carswiseai.com>', to: [ops], subject, html })
  }).catch(() => {});
}
...
function actionReply(tipo, offerTitle, ofertaId) {
  ...
  const replies = {
    ...
    VISITA:       `Puedes reservar tu visita directamente aquí:\n${url}\n\nO nuestro equipo te llamará para coordinarla 📅`,
...
async function processStep(pool, session, phone, body, transcript) {
  ...
  // ── VO + Buy shared: offer ref → source → action ───────────────────────────
  ...
  if (step === 'vo_action' || step === 'buy_action') {
    ...
    return { step: 'done', ctx, reply: `${actionReply(tipo, ctx.offerTitle, ctx.ofertaId)}\n\n¿Hay algo más en lo que pueda ayudarte? *SÍ* / *NO*` };
  }

  // ── Buy: search without concrete vehicle ───────────────────────────────────
  if (step === 'buy_has_ref') {
  ...
  if (step === 'buy_search_budget') {
    ...
    await notifyOps(`[WhatsApp Bot] BÚSQUEDA`, [`Teléfono: ${phone}`, `Nombre: ${ctx.name || '—'}`, `Criterios: marca=${ctx.criterios.brand}, presupuesto=${ctx.criterios.budget}`]);
  ...
  if (step === 'renting_has_offer') {
    if (lo === '1') return { step: 'renting_offer_ref', ctx, reply: '¿Cuál es la referencia o descripción de la oferta de renting?' };
    if (lo === '2') {
  ...
  if (step === 'sell') {
    if (lo === '1') {
      return { step: 'done', ctx, reply: 'Puedes publicar tu vehículo desde tu cuenta en:\nhttps://www.carswiseai.com\n\nSi necesitas ayuda escríbeme y un asesor te guía paso a paso.' };
    }
  ...
  if (step === 'tasacion_km') {
    ...
    return { step: 'done', ctx, reply: 'Perfecto, ya tengo los datos. Nuestro equipo de tasación te contactará en menos de 24h con una valoración 💰\n\n¿Algo más? *SÍ* / *NO*' };
  }
  ...
  if (step === 'service_type') {
    ...
    if (!st) return { step, ctx, reply: 'Elige:\n1️⃣ Taller / mantenimiento\n2️⃣ Seguro\n3️⃣ Garantía\n4️⃣ Tramitación\n5️⃣ Otro' };
    ctx.subTipo = st;
  ...
  if (step === 'done' || step === 'rejected') {
    if (/^s[ií]?$|^yes$/i.test(lo)) {
      if (flow === 'vo') {
        return { step: 'vo_offer_ref', ctx, reply: '¿Sobre qué vehículo te puedo ayudar? Dame la referencia o descripción.' };
      }
  ...
  // Fallback
  return { step, ctx, reply: 'No te he entendido. Por favor elige una opción del menú.' };
}

// ─── Main handler (called from api/market.js) ─────────────────────────────────
module.exports = async function whatsappHandler(req, res) {
  ...
  // Twilio sends form-encoded body (parsed by Vercel as req.body)
  ...
  // Determine flow from destination number
  const voNumber = String(process.env.TWILIO_WA_NUMBER_VO || '').replace('whatsapp:', '');
  const flow     = (voNumber && to === voNumber) ? 'vo' : 'general';

  // Load existing session
  let session = await getSession(pool, from);

  if (!session) {
    // First contact — identify the user
    ...
    if (userInfo.type === 'unknown') {
      ...
      if (flow === 'vo') {
        step     = 'vo_offer_ref';
        greeting = `¡Hola${fn ? ', ' + fn : ''}! 👋 ¿Cuál es el vehículo que te interesa? Dame la referencia o descríbemelo (marca, modelo, año).`;
      } else {
        step     = 'menu';
        greeting = `¡Hola${fn ? ', ' + fn : ''}! 👋\n\n${MAIN_MENU}`;
      }
  ...
  return sendTwiML(res, result.reply);
};
