...
module.exports = async function fianzaConfirmarHandler(req, res) {
  ...
  /**
   * `deposito` es el nombre de ahora; `fianza`, el de antes.
   *
   * Se aceptan los dos. Cuando la fianza del 30 % pasó a ser el depósito del
   * coche entero, la pasarela empezó a etiquetar sus sesiones como `deposito` y
   * esto se quedó exigiendo `fianza`: el cliente pagaba, volvía al panel, esto
   * contestaba 400 y su solicitud seguía diciendo «pendiente de depósito» con
   * el botón de pagar delante. El aviso de Stripe ya acepta los dos nombres,
   * así que el fallo solo se veía cuando el webhook no llegaba, que es
   * justamente para lo que existe este camino.
   *
   * Y se siguen aceptando los dos porque las sesiones viejas llevan el nombre
   * viejo: quien pagó ayer tiene que poder confirmarse hoy.
   */
  const plan = nt(meta.plan_id).toLowerCase();
  if (plan !== "deposito" && plan !== "fianza") {
    return res.status(400).json({ ok: false, error: "Esa sesión de pago no es de un depósito." });
  }
