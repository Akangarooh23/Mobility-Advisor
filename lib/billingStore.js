      ...
      try {
        ...
        const mand = await pool.query(
          `SELECT id, mandato_id, firmado_at,
                  clausula_id, clausula_enviada_at, clausula_firmada_at, precio_referencia
         ...
         * lleva andado.
         */
        if (enc && enc.clausula_enviada_at) {
          precioPorLead[s(r.id)] = {
            encargo_id: enc.id,
            aceptada: Boolean(enc.clausula_firmada_at),
            // El importe, ya escrito: el panel no hace cuentas con dinero.
            importe: Number(enc.precio_referencia) > 0
              ? `${Math.round(Number(enc.precio_referencia)).toLocaleString("es-ES")} €`
              : "",
          };
        }
