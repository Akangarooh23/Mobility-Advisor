...
async function listMarketplaceVoOffers(options = {}) {
  ...
  try {
    ...
    if (fuel) {
      // TODO (commit separado): usar getFuelCompatTokens + IN para que PHEV aparezca en búsqueda
      // híbrido y GNC en búsqueda gas. Requiere decisión de producto (buscador ≠ pool de comparables).
...
async function listUserPublishedVehiclesForMarketplace(filters = {}) {
  ...
  try {
    ...
    if (filters.fuel) {
      // TODO (commit separado): misma decisión de producto que listMarketplaceVoOffers
...
async function getMarketplaceShowcase() {
  ...
  const [conce, part, rent, imp, flex, auto, ocasion] = await Promise.all([
    ...
    q(`SELECT ${mkCols} FROM moveadvisor_market_offers WHERE portal='ocasionplus' AND COALESCE(is_active,TRUE)=TRUE AND price IS NOT NULL AND COALESCE(image_url,'')<>'' LIMIT 1`),
  ]);
  ...
  const sections = [
    ...
    ocasion && { key: "portal-ocasionplus", label: "Ofertas Portales · OcasiónPlus", nav: { type: "buy-known" }, offer: baseOffer(ocasion, firstImage(ocasion.image_url, ocasion.images), ocasion.price) },
  ].filter(Boolean);
