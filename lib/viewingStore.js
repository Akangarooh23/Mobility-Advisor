...
const { plantilla, parrafo, datos, boton, textoPlano } = require("./correo");
...
async function sendResendEmail({ to, subject, html }) {
  ...
  const resp = await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({ from: remitente(), reply_to: respuestaA(), to: [to], subject, html, text: textoPlano(html) }),
  });
  if (!resp.ok) {
