...
module.exports = async function importOffersHandler(req, res) {
  ...
  if (fuel)         { values.push(fuel.toLowerCase());         conditions.push(`lower(COALESCE(fuel,'')) = $${values.length}`); }
  if (color)        { values.push(color.toLowerCase());        conditions.push(`lower(COALESCE(color,'')) = $${values.length}`); }
