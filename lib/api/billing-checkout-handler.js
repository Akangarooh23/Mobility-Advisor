...
module.exports = async function billingCheckoutHandler(req, res) {
  ...
  if (requestedPlanId === "fianza") {
    ...
    try {
      const r = await pool.query(
        `SELECT id, vehicle_title, deposit_quoted, deposit_paid_at
    ...
    const successUrl = `${origin || "https://www.popcar.tech"}/panel?fianza=ok&session_id={CHECKOUT_SESSION_ID}`;
    const cancelUrl  = `${origin || "https://www.popcar.tech"}/panel?fianza=cancel`;
    if (!stripeSecretKey) return res.status(200).json({ ok: true, simulated: true, url: "" });
    ...
    try {
      ...
      const stripeResponse = await fetch("https://api.stripe.com/v1/checkout/sessions", {
        ...
        body: encodeForm({
          ...
          "line_items[0][price_data][product_data][name]": `Fianza de importacion · ${normalizeText(solicitud.vehicle_title) || "vehiculo"}`,
          "line_items[0][price_data][product_data][description]": "30% para reservar el coche y hacer el pedido. Se devuelve si no se hace el pedido.",
          "line_items[0][quantity]": "1",
          customer: stripeCustomerId,
          "metadata[plan_id]": "fianza",
