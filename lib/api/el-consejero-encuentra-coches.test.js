...
describe("la busqueda, de punta a punta", () => {
  test("el inventario aplica los criterios y vuelve a preguntar si no sale nada", async () => {
    ...
    // El tercer argumento son los valores que de verdad hay en cada columna,
    // que es lo que permite comparar con `=` en vez de con `LIKE`.
    assert.match(fuente, /condicionesDelConsejero\(criterios, 1, enLaBase\)/,
...
describe("lo que se le puede enseñar a un cliente", () => {
  test("se pide foto, precio con sentido y que sea un coche", () => {
    /*
     * Contado sobre las 1.579.332 ofertas visibles del pool: 360.891 sin
     * ninguna foto -el 23 %-, 3.437 por debajo de 500 €, 2.889 por encima de
     * 200.000 y 4.041 con kilómetros imposibles. En el consejero eso son
     * tarjetas con un hueco donde va el coche, y una autocaravana a 0 €
     * encabezando «lo que mejor encaja».
     */
    const { condiciones } = condicionesDelConsejero({ soloPresentables: true }, 0);
    const todo = condiciones.join(" ");

    assert.match(todo, /image_url/);
    assert.match(todo, /price BETWEEN 500 AND 200000/);
    assert.match(todo, /mileage IS NULL OR mileage BETWEEN 0 AND 500000/);
    assert.match(todo, /es_coche IS NOT FALSE/);
  });

  test("y no lleva parámetros: son condiciones fijas, no lo que pide nadie", () => {
    const { valores } = condicionesDelConsejero({ soloPresentables: true }, 0);
    assert.deepEqual(valores, []);
  });

  test("la tasación no lo lleva: un comparable sin foto sigue siendo un precio", () => {
    const { condiciones } = condicionesDelConsejero({ maxPrice: 20000 }, 0);
    assert.doesNotMatch(condiciones.join(" "), /image_url/);
  });
});
