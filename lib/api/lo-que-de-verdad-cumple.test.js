...
describe("la carroceria que eligio", () => {
  ...
  test("sin saber que coche es, no pasa", () => {
    /*
     * Esto costo dos intentos. Dejarlo pasar parecia lo generoso, pero el
     * WHERE ya filtra por carroceria: una oferta SIN carroceria no viene de la
     * busqueda buena, viene de una de emergencia que ha soltado ese criterio.
     * Dejandola pasar se colo un Audi A3 en una busqueda de SUV.
     */
    assert.equal(porQueNoVale({ bodyType: "" }, losLimites(PIDE_COMPACTO)), "no dicen que tipo de coche es");
  });
  ...
  test("y la lista de palabras es corta para no llevarse coches buenos", () => {
    ...
    for (const coche of [
      ...
      /*
       * Se caen por no tener carroceria, que es otra cosa. Lo que no puede
       * pasar es que se caigan por PARECER de otro tipo: eso seria la lista
       * de palabras disparando donde no debe.
       */
      assert.notEqual(porQueNoVale(coche, losLimites(PIDE_COMPACTO)), "es otro tipo de coche", coche.model);
    }
  });

  test("y a quien pidio un coupe, el TT le vale", () => {
    const elTT = { bodyType: "", brand: "Audi", model: "TT Coupé" };
    assert.notEqual(porQueNoVale(elTT, losLimites({ bodyType: "coupe" })), "es otro tipo de coche");
  });
