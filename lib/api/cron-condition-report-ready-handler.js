...
function baseDelSitio() {
  return (normalizeText(process.env.PUBLIC_SITE_URL) || "https://www.carswiseai.com").replace(
    /\/$/,
    ""
  );
}
...
async function enviar(destinatario, correo) {
  ...
  const from =
    ...
    "CarsWise <onboarding@resend.dev>";
  ...
  try {
    ...
    for (const fila of rows) {
      ...
      if (!LISTO.has(estado) && carswiseCheck.estaConfigurado()) {
        ...
        if (consulta.ok) estado = normalizeText(consulta.datos?.status) || estado;
