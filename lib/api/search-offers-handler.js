...
function mapear(f) {
  ...
  return {
    ...
    modelo: f.model || "",
    version: f.version || "",
    titulo: f.title || [f.brand, f.model, f.version].filter(Boolean).join(" "),
    precio: f.price === null ? null : Number(f.price),
    precioMensual: f.monthly_price === null ? null : Number(f.monthly_price),
    anio: f.year ?? null,
    km: f.mileage ?? null,
    combustible: f.fuel || "",
    cambio: f.transmission || "",
    carroceria: f.body_type || "",
    color: f.color || "",
    potencia: f.power_cv ?? null,
    provincia: f.province || "",
    vendedor: f.seller_type || "",
    imagen: f.image_url || imagenes[0] || "",
  };
}
