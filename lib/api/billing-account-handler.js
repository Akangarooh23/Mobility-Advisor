...
async function getProfileFromPostgres(email) {
  ...
  try {
    const result = await pool.query(
      `SELECT name, phone FROM moveadvisor_users WHERE lower(email) = lower($1) LIMIT 1`,
      [email]
    );
    if (!result.rows.length) return null;
    const row = result.rows[0];
    return {
      fullName: normalizeText(row.name),
...
async function saveProfileToPostgres(email, profile) {
  ...
  try {
    await pool.query(
      `UPDATE moveadvisor_users SET name = $2, phone = $3 WHERE lower(email) = lower($1)`,
      [email, normalizeText(profile?.fullName), normalizeText(profile?.phone)]
    );
  } catch {}
}
