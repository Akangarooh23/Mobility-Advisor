...
async function listMarketplaceVoOffers(options = {}) {
  ...
  try {
    ...
    const whereClause = onlyActive ? "WHERE o.is_active = TRUE" : "";
    ...
    const query = `
      SELECT
        ...
        o.image_url, o.source_url, o.portal,
        ...
        o.image_urls, o.has_stock_management,
        COALESCE(u.units_available, 0) AS units_available,
        COALESCE(u.available_colors, ARRAY[]::text[]) AS available_colors
      FROM moveadvisor_marketplace_vo_offers o
      LEFT JOIN LATERAL (
        SELECT
          COUNT(*)::int FILTER (WHERE status = 'available') AS units_available,
          ARRAY_AGG(DISTINCT color ORDER BY color) FILTER (WHERE status = 'available' AND color IS NOT NULL AND color <> '') AS available_colors
        FROM moveadvisor_marketplace_vo_units
        WHERE offer_id = o.id
      ) u ON TRUE
      ${whereClause}
      ORDER BY o.portal_score DESC NULLS LAST, o.updated_at DESC NULLS LAST
      LIMIT $1 OFFSET $2
    `;
