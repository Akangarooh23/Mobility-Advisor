...
describe("pedir visita sin haber entrado", () => {
  ...
  test("en produccion NO se reserva, aunque mande un correo", async () => {
    /*
     * Lo que protege esta prueba no ha cambiado: sin haber probado quien es, de
     * aqui no sale una cita. Lo que ha cambiado es lo que se le contesta.
     *
     * Antes era un «inicia sesion» y se acababa ahi. Ahora hay un camino de al
     * lado —`route=solicitar`— que le manda un enlace al correo y no reserva
     * nada hasta que lo pulsa. El clic prueba lo mismo que probaba la sesion.
     */
    ...
    assert.equal(cuerpo?.error, "pide_por_correo");
    assert.match(String(cuerpo?.detalle ?? ""), /solicitar/,
      "hay que decirle por donde se pide, o el rechazo es un callejon sin salida");
  });
});
